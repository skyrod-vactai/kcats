...
#+begin_src kcats
  ...
  [ 🗑️ ] ;; get-children
  [] ;; pre-traverse
  [[association] [🗑️] 🩹 ▶️] ;; post-traverse
  [] ;; reassemble


  [5️⃣ ;; the pred remains untouched
   [✂️4️⃣ ✂️3️⃣] ;; affirmative - run the get-children and the pre-traverse 
   [✂️3️⃣ ✂️2️⃣] ;; negative - run the pre-traverse and post-traverse
   [🚜 ▶️ ✂️2️⃣ ✂️1️⃣] ;; combinator
  ] 🎒 🍫  🪆  ▶️ 
#+end_src
...
[[actual 2]
 ...
 [handled 🔳]
 [reason "type mismatch"]
 [trace [[[👥] 🛡️ [👥 👥 🔀 mod zero?] [🗑️ 🗑️ 🔳] [🗑️ 👥 👥 * >] [🗑️ 🗑️ ✅] ⚖️ [inc]]]]
 [type error]
 [unwound [⏳ ▶️]]]
...
#+begin_src kcats
  ...
  🔀 [🔀 [ first =] •🛡️ 📣 🧲 ▶️ ] 🚜 ▶️ ;; filter out rows with no link to this row (matching index)
  [[] [🔗] 🪜 ▶️ ;; connect all items in each row
  ...
  [📏 1 >=] 🧲 🚜 ▶️
  •🗑️ ;; don't need the by-variable lookup anymore
...
#+begin_src kcats
  ...
  [2 get [[word?]
          [string last \? =]] [▶️] 💯 ▶️]  🧲 ▶️
  ;; group by the variable
  [liberator [2 get] group] 🛡️ ▶️
  ;; group by row
  [liberator [first] group [first] sort] 🪄 
  ;[first] sort
  🔀
...
#+begin_src kcats
  ...
  [[object [[entity attribute value] 🔀 zip]]
   [variable? [[[word?] [string last \? =]] [▶️] 💯 ▶️]]
   [val [2 get]]
   [rowindex [0 get]]
   [format-link [[🍫 🗑️] 🪄
                 [🍫 🗑️ [4️⃣ 3️⃣ 2️⃣ 1️⃣] 🎒 "c{}.{} = c{}.{}" 🔀 format] 🚜 ▶️ ]]]
  [👥  
   ;; first make each into a mapping ;; then index each row
   [object] 🚜 ▶️ indexed
   ;; for each row, move the index into each value
   [🍫 [🔀 prepend] 🚜 ▶️] 🚜 ▶️ 👥 ;; save this data
   ;; join the rows together now that we have indices
   [] 🔀 [🔗] 🪜 ▶️ 
   ;; filter out non variables
   [val variable?] 🧲 ▶️ 
   ;; group by the variable
   liberator [val] group •🗑️ 
   🔀

   ;; just test one for now
 
   [👥 [ [rowindex] 🛡️ ▶️ ;; extract constraint num
    [val ;; extract variable name
     🔀 [get] •🛡️ 🪄 ;; look up in by-variable map
...
#+begin_src kcats
  [[🔳 🔳 🔳]
   [["c1.entity = c0.entity"] 🔳 🔳]
   [["c2.entity = c0.entity" "c2.entity = c1.entity"] 🔳 🔳]
   [["c3.entity = c0.entity" "c3.entity = c1.entity" "c3.entity = c2.entity"] 🔳 🔳]
   [🔳 🔳 ["c4.value = c3.value"]]
   [["c5.entity = c4.entity"] 🔳 ["c5.value = c2.value"]]
   [["c6.entity = c4.entity" "c6.entity = c5.entity"] 🔳 🔳]
   [["c7.entity = c4.entity" "c7.entity = c5.entity" "c7.entity = c6.entity"] 🔳 🔳]]
  [[author-first? [[3 value author-first?] [4 value author-first?]]]
   [author-last? [[2 value author-last?] [5 value author-last?]]]
   [author? [[4 entity author?] [5 entity author?] [6 entity author?] [7 entity author?]]]
   [birth-year? [[7 value birth-year?]]]
...
#+begin_src kcats
   33 [1 2 3] [+ odd?]
  🛡️ ;; run the predicate with no stack effect
...
[[actual a]
 ...
 [reason "type mismatch"]
 [type error]
 [unwound [association 🔗 [[[b 12]
                           [c 15]]] [[📸 🧦 [list?] 🪄 🧦 first] [🗑️ 🔳 🔀] [association] [[✂️1️⃣ 🔗]
                                                                                     🎒 🪜 ▶️ 🎁]
                                     🪆 ▶️ 🔗]
           🪜 ▶️ 🎁 🔗 [[d [[e 🔳]
                         [f [5 4 3 2 1]]]]] [[📸 🧦 [list?] 🪄 🧦 first] [🗑️ 🔳 🔀] [association] [[✂️1️⃣ 🔗]
                                                                                             🎒 🪜 ▶️ 🎁]
                                             🪆 ▶️ 🔗]
           🪜 ▶️ 🎁 🍫]]]
a 🔳 🔳
...
#+begin_src kcats
  [[a [[b 12]
       [c 15]]]
   [d [[e []]
       [f [5 4 3 2 1]]]]] [[association] [dump 🗑️] 🩹 ▶️ 🎁] walk ▶️ 
#+end_src
...
#+begin_src kcats
  [[a b]] 🎁 [[association] [dump 🗑️] 🩹 ▶️ 🎁] walk ▶️ 🍫
#+end_src
...
#+RESULTS:
#+begin_src kcats
[#b64 "core" #b64 "lO96m8LQJNw5Hpv7U0iHcLZEzZ-T2NNIgVCGaiLGSlg" 🔳]
#+end_src

#+begin_src kcats
  [fib] 🍫 decache ; string read
#+end_src

#+RESULTS:
#+begin_src kcats
thread 'main' panicked at src/types/container/environment.rs:244:62:
called `Result::unwrap()` on an `Err` value: [[actual [[asked [dictionary]] [handled 🔳] [reason "word is not defined"] [trace 🔳] [type error] [unwound [dictionary 🔀 [module [words] 🔀 update] •🛡️ ▶️ •🗑️]]]] [asked [dictionary]] [reason "type mismatch"] [type error]]
note: run with `RUST_BACKTRACE=1` environment variable to display a backtrace
#+end_src

#+begin_src kcats
  "123" encode dictionary [crypto] 🍫 module  [words] 🔀 update  [hash] confine
#+end_src
...
#+begin_src kcats
  ["foobar" -4 1 slice] [🗑️ "outofbounds"] 🩹 ▶️ 
#+end_src
...
#+RESULTS:
#+begin_src kcats
[[asked [[1️⃣ 2️⃣ c d e __] [a b c d] =]]
 [handled 🔳]
 [reason "match failed"]
 [trace [[1️⃣ 2️⃣ c d e __]]]
 [type error]
 [unwound [⛏️]]]
#+end_src

#+begin_src kcats
  [a b c d e f] [a 🏷️foo c _ _ _] ⛏️ 
#+end_src
...
[[asked [read]]
 [handled 🔳]
 [reason "Cannot have more a colon in a symbol"]
 [trace ["a:sdf"]]
 [type error]
 [unwound [word]]]
"a:sdf"
#+end_src
* use both for compare-by
#+begin_src kcats
-1 -2 [abs] both compare
#+end_src
