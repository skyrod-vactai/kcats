...
#+begin_src kcats :tangle src/kcats/core/execute.kcats :mkdirp yes
  [🪆 [[spec [[[program combinator]
       ...
       [doc "The Russian dolls denote a program recurrence. Constructs a
             recursive program, without having to give it a name.  Takes
             4 component programs as input: predicate, body, exit, and
             combinator; and returns a recursive program. This program
             will run predicate, if false the exit program runs and the
             recursive program is finished. If true, the body program
             runs, the whole recursive program is put on the stack and
             the combinator is run."]
...
#+begin_src kcats kcats :tangle src/kcats/core/collections.kcats :mkdirp yes
  ...
  [pad [[spec [[[item padding] [integer newsize] sized] [sized]]]
        ...
        [examples [[[[1 2 3] -5 0 pad] [[0 0 1 2 3]] "Pad a list at start with negative argument, to given size"]
                   ...
                   [[[1 2 3 4 5 6] 5 0 pad-left] [[1 2 3 4 5 6]] "Padding a list to smaller than original size, is a no-op"]]]]]
#+end_src
