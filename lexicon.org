...
#+begin_src kcats kcats :tangle src/kcats/core/collections.kcats :mkdirp yes
  ...
  [filter [[spec [[program sized] [list]]]
           [definition [[] sink ;; put empty results below list
                        [shield dip] decorate ;; run filter fn shielded and dipped under result
                        [swap] unwrap prepend ;; start by swapping the result back to the top
  ...
  [sort [[spec [[program sized] [list]]]
         [definition [[clone **1 pair] template
                      map sort-indexed]]
...
#+begin_src kcats
    ...
    ;; when the closure executes, capture the outer stack first
    wrap
    [[stack] [snapshot] divedown assign evaluate [stack] lookup restore] join dump execute
#+end_src

#+RESULTS:
#+begin_src kcats
Env: { stack: [dictionary_redacted], program: [] }
Env: { stack: [dictionary_redacted], program: [] }
Env: { stack: [dictionary_redacted], program: [] }
Env: { stack: [dictionary_redacted], program: [] }
Env: { stack: [dictionary_redacted], program: [] }
...
Env: { stack: [dictionary_redacted], program: [] }
Env: { stack: [dictionary_redacted], program: [] }
[[[[[plus2 [[definition [2 +]]]]] join]
  [words] update]
 dictionary_redacted]
[[actual [[plus2 [[definition [2 +]]]]]]
 [asked [KeyItem]]
 [handled yes]
 [reason "type mismatch"]
 [type error]
 [unwound [get [[join]] unwrap [something?] shield [take swap [get] dip [something?]
                                                    shield]
           loop drop [[[[[plus2 [[definition [2 +]]]]] join]
                       dictionary_redacted]] unwrap evert first [[words]] unwrap shielddown assign [[dictionary_redacted]]
           unwrap evert first
           [#b64 "g5nJOWpyglIeN2EgJOdFRVQ0ix76q42bTs2uG5w5J/s="] unwrap sink [dictmerge] shielddeep
           [[3 plus2]] unwrap [wrap] dipdown dump [program dictionary resolver] label environment
           wrap [[stack] [snapshot] divedown assign evaluate [stack] lookup restore]
           join dump execute]]]
[[plus2 [[definition [2 +]]]]] dictionary_redacted
#+end_src
