...
#+begin_src kcats :tangle src/kcats/core/execute-builtins.kcats :mkdirp yes
  ...
  [branch [[spec [[[program no-branch]
                   ...
                   [item condition]]
                  [*]]]
           [examples [[[5 yes [drop 3 *] [4 +] branch] [15] "Branch on true condition"]
...
#+begin_src kcats :tangle src/kcats/core/execute.kcats :mkdirp yes
  [loop [[spec [[program [item flag]] [*]]]
         [definition [[[*1 execute ;; execute the body
                       ...
                       branch]
                      pack]]
         [examples [[[10 yes [-2 * clone 50 <] loop execute] [160] "Looping"]
                    [[10 [] [-2 * clone 50 <] loop execute] [10] "Looping with false initial condition is no-op"]]]]]
  ...
  [recur [[spec [[[program rec2]
                  ...
                  [program pred]]
                 [*]]]
          [definition [[**2 [*4 *3 *2 *1 recur execute] **1] [pack]
                       shielddeep execute dropdown if]]
          [examples [[[3
                       [1 <=] shield
  ...
  [shielddown [[spec [[program item]
                      [item]]]
               [definition [shield [dropdown] join]]
  ...
  [shielddeep [[spec [[[program p] [item consumed] [item consumed]]
                      [[item result]]]]
               [definition [shield [[drop drop] dip] join]]
  ...
  [while [[spec [[[program body]
                  [program pred]]
                 [*]]]
          [definition [[*2 ;; run conditional
                        [**1 ;; run body
                         ,*2 *1 while ;; next iteration
                         execute]
                        []] ;; do nothing if condition is false
                       pack unwrap if]]
...
#+begin_src kcats
[label [[spec [[[sized labels]] [association]]]
        [definition [[] swap [wrap float assign] step]]
        [examples [[["Alice" 23 "123 Main St" [address age name] label] [[[address "123 Main St"]
                                                                          [age 23]
                                                                          [name "Alice"]]
                                                                         association]
                    "Label values on the stack as an association"]]]]]
[zip [[spec [[[dispenser values]
              [dispenser keys]]
             [list]]]
      [definition [[] sink [[something?] both?]
                   [[take] both swapdown pair sink [put] dipdown]
                   while drop drop]]
      [examples [[[[a b c] [1 2 3] zip] [[[a 1]
                                          [b 2]
                                          [c 3]]] "Zip two lists together into a single list of pairs"] [[[a b c d] [1 2 3]
                                                                                                          zip]
                                                                                                         [[[a 1]
                                                                                                           [b 2]
                                                                                                           [c 3]]] "Zip two lists of unequal size pads with Nothing values"]]]]]
[value [[spec [[[sized into-association]] [item]]]
        [definition [[count 1 =] [first second]
                     [[value] lookup]
                     if]]
        [examples [[[[[foo 1]] value]
                    [1] "The value of a single-entry association is the value of the key-value pair"]
                   [[[[type url]
                      [value "http://foo.com"]]
                     value]
                    ["http://foo.com"] "The value of an object is the value key"]]]]]
[type [[spec [[item] [item]]]
       [definition [[[[empty?] [[nothing] unwrap]]
                     ...
                     [[set?] [[set] unwrap]]
                     [[association?] [[[[type] lookup]
                                       [[count 1 =] [[first [type] unwrap =] [first second]
                                                     [first first]
                                                     if]
                                        [[]] if]
                                       [[association] unwrap]]
       ...
       [examples [[[[[foo 1]] association type] [[foo] unwrap]
                   "An association with one key assumes it is its type"]
                  [[1 type]
                   [[number] unwrap]
                   "Integers are of type number"]
                  [[1 type]
                   [[number] unwrap]
                   "Floats are of type number"]
                  [[[] type]
                   [[nothing] unwrap]
                   "Empty list is of type nothing"]
                  [["foo" encode type] [[bytes] unwrap]
                   "A byte array is of type bytes"]
                  [["foo" type]
                   [[string] unwrap]
                   "A string is of type string"]
                  [[[1 2 3] set type] [[set] unwrap]
                   "A set is of type set"]
                  [[[] environment type] [[environment] unwrap]
                   "An environment is of type environment"]
                  [[[[type foo]] association type] [[foo] unwrap]
                   "An association with a single type key, the value is its type"]
                  [[[[type foo]
                     [attr "blah"]]
                    association type]
                   [[foo] unwrap]
                  ...
                  [[[[attr1 foo]
                    ...
                    association type]
                   [[association] unwrap]
                   "An association with multiple keys and no type key, is of type association"]
                  [[[[type url]
                     [value "http://foo.com"]]
                    association type]
                   [[url] unwrap]
                   "An association with type/value keys, uses the type key for its type"]]]]]
[lookup [[spec [[[list keys]
                 sized]
                [item]]]
         [definition [[] [take swap [get] dip] while execute drop]]
         [examples [[[[[a b]
                       [c d]]
                      association [a] lookup]
                     [[b] unwrap]
                     "Lookup the value of a key in an association"]
                    [[[[a b]
                       [c d]]
                      [a] lookup]
                     [[b] unwrap]
                     "Lookup the value of a key, promoting a list to association"]
                    [[[[a b]
                       [c d]]
                      [e] lookup]
                     [[]] "Looking up a key that doesn't exist returns Nothing"]
                    [[[[outer [[a b]
                               [c d]]]] [outer c]
                      lookup]
                     [[d] unwrap]
                     "Lookup in a nested structure, with promotion"]]]]]
[update [[spec [[program [list keys]
                 [sized into-association]]
                [association]]]
         [definition [[[lookup] shield]
                      dip shielddown assign]]
         [examples [[[[[a 1]
                       [b 2]]
                      [b] [inc] update]
                     [[[a 1]
                       [b 3]]
                      association]
                     "Update a value in an association"]
                    [[[[a [[c 3]
                           [d 5]]]
                       [b 2]]
                      [a c]
                    ...
                    [[[[a [1 3 5 7]]
                     ...
                     [[[a [1 3 6 7]]
                       [b 2]]
                      association]
                     "Update a value in a mixed association/list structure"]
                    [["hi" [[a [[c 3]
                                [d 5]]]
                            [b 2]]
                      [a c]
                      [drop drop 10 15] update [a c]
                      lookup]
                     ["hi" 15]
                     "Update function can't destroy stack items"]
                    [[[[a 1]
                       [b 2]]
                      [d] [5] update]
                     [[[a 1]
                       ...
                       [d 5]]
                      association]
                     "Update creates new key when it doesn't exist"]
                    [[[[a [[c 3]
                           [d 5]]]
                       [b 2]]
                      [a e]
...
#+begin_src kcats :tangle src/kcats/stdlib/emoji.kcats :mkdirp yes
