...
#+begin_src kcats :tangle src/kcats/lexicon.kcats :mkdirp yes
  ...
  [capture [[spec [[program] [program]]]
            [definition [[snapshot wrap [swap [restore] dip] join] dip join]]
            [examples [[[[1 2 3 [+] capture [20 30] dip execute] shield] [5]]]]]]
...
#+begin_src kcats :tangle src/kcats/lexicon.kcats :mkdirp yes
  ...
  [sprint [[spec [[environment]
                  [environment]]]
           [definition [[[expression 0] lookup wrap [breakpoint] =] break
                        drop ;; the condition
                        [] [eval-step] [] if]]]] ;; advance past the breakpoint word if the program isn't complete
  ...
  [spawn [[spec [[program] [environment]]]
          [definition [[snapshot] dip
                       pair [stack expression] swap zip
                       environment]]
          [examples [[[1 2 3 [swap clone] spawn]
                      [1 2 3 [[expression [swap clone]] [stack [3 2 1]]] environment]]]]]]
#+end_src
...
#+begin_src kcats :tangle src/kcats/lexicon.kcats :mkdirp yes
  ...
  [print [[spec [[string] []]]
          [definition [[standard] dip bytes put drop]]]]
  ...
  [siphon [[spec [[[receptacle output] [program generator]] [[receptacle output]]]]
           [definition [[] ;; placeholder that gets dropped (next
                        ...
                        [drop ;; the last value
                         ...
                         [put] dip]
                        until
                        drop]] ;; the now-empty dispenser
           [examples [[[[integers 5 taker [] siphon] shield] [[0 1 2 3 4 []]]]]]]]
#+end_src
* Issues
