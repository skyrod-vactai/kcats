...
#+begin_src kcats :tangle src/kcats/core/compare.kcats :mkdirp yes
  ...
  [max-by [[spec [[program [item a] [item b]] [item]]]
           ...
           [examples [[[[[price 25] [name bar]]
                        [[price 15] [name foo]]
                        [[price] lookup] max-by] [[[price 25] [name bar]]]]]]]]
  ...
  [min-by [[spec [[program [item a] [item b]] [item]]]
           ...
           [examples [[[[[price 25] [name bar]]
                        [[price 15] [name foo]]
                        [[price] lookup] min-by] [[[price 15] [name foo]]]]]]]]
...
#+begin_src kcats :tangle src/kcats/stdlib/generators.kcats :mkdirp yes
  ...
  [[generator [[spec [program program]]
   ...
   [assemble [[spec [[program sized] [list]]]
   ...
   [parse [[spec [[[program partial-parser] [[program parser]]]]]
           ...
           [doc "A stream parser. On the stack should be a program that
                 takes 3 args: parsed object output buffer (on top), an
                 unparsed input buffer, and new input item. The function
                 should return a new input and output sequence. If there
                 is enough input to create a new output, the program
                 should do so, otherwise it only needs to combine the
                 new input with the existing input buffer. An empty
                 input should signal an end-of-stream condition and it's
                 up to the program how to interpret that."]
         
           [definition [🔳 🔳            ;; initial state - parsed object output (on top) and unparsed input 
                        [[👥 ☯️]         ;; there's no output
                         [🗑️ [▶️] ••🐋]  ;; exec parent generator 
                         [🔳] ⚖️         ;; leave an empty value to be consumed by outer ⏳ exiting
...
#+begin_src kcats :tangle src/kcats/core/dictionary-builtins.kcats :mkdirp yes
  ...
  [hashbytes [[spec [[bytes] [bytes]]]
              [examples [[[["foo" encode hashbytes] 2 times ▶️ =] [✔️]]
...
#+begin_src kcats :tangle src/kcats/core/dictionary.kcats :mkdirp yes
  ...
  [definition [[spec [[list] [program]]]
               [definition [[cache ✂️1️⃣ definition] 🎒 dictionary 🔀 lookup]]
...
#+begin_src kcats :tangle src/kcats/core/strings-builtins.kcats :mkdirp yes
  ...
  [parse-utf8 [[spec [[[sized current-output] [integer input] [bytes input-buffer]]
                      [[string updated-output-sequence] [bytes remaining-input]]]]
               [examples [[[#b64 "aGk" 226 "" parse-utf8] [#b64 "4g" "hi"]]
                          [[#b64 "aGVsbA" 111 "" parse-utf8] [#b64 "" "hello"]]]]]]
...
#+begin_src kcats :tangle src/kcats/stdlib/debug.kcats :mkdirp yes
  [[break [[spec [[[program condition] environment]
   ...
   [advance [[spec [[environment] [environment]]]
             [definition [👥 [program] lookup 📏 🔀 ;; count up the program length, we'll run until it's smaller than this
                          [[program] lookup 📏 ;; only stop if env's program empty or shorter than we started off
                           [[positive?] [<=]] [🛡️ ▶️] 💯 ▶️] 🛡️ 
