...
#+begin_src kcats :tangle src/kcats/lexicon.kcats
  ...
  [yank [[spec [[[item a] [item b]] [[item b]]]]
         [definition [swap discard]]]]
  ...
  [bail [[spec [[program] [*]]]
         [definition [[swap] [execute] [discard] if]]
         [examples [[[[] [inc] bail] [[]]]
                    [[1 [inc] bail] [2]]]]]]
  ...
  [taker [[spec [[] [program]]]
          [definition [[[positive?] [dec [generate] dive] [discard []] if]]]
  ...
  [map [[spec [[program list] [list]]]
        [definition [[[liberate] dip each collect] shielddowndown]] 
        [examples [[[[1 2 3] [inc] map] [[2 3 4]]]
                   ...
                   ;;[[7 9 [1 2 3] [discard discard] map] [7 9 [7 7 7]]]
                   ;; do it this way instead
                   [[1 [1 2 3] [1 swap +] map] [1 [2 3 4]]]
                   [[7 9 [1 2 3] [[7 9] dip + *] map] [7 9 [70 77 84]]]

                   [[[1 2 3] [discard 1 inc] map] [[2 2 2]]]
                   [[7 9 [+] [] map] [7 9 [+]]]]]]]

  [filter [[spec [[program list] [list]]]
           [definition [[[liberate] dip keep collect] shielddowndown]]
  ...
  [advance [[spec [[environment] [environment]]]
            [definition [[[expression] lookup count] shield swap ;; count up the expression length, we'll run until it's smaller than this
                         ...
                         [eval-step]  ;; evaluate the environment one step
                         while
                         yank ;; drop the expression length item
                        ]]]]
...
#+begin_src kcats :tangle src/kcats/lexicon.kcats
  ...
  [future [[spec [[program] [pipe]]]
           [definition [handoff swap ;; make a pipe
                        ...
                        [snapshot] join wrap [dive put discard] join
                        ; ;; now we have expr stack
                        pair
                        [stack expression] swap zip environment
                        animate]]
           [examples [[[1 [2 +] future take yank] [1 [3]]]]]]]
#+end_src
* Issues
