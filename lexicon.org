...
#+begin_src kcats :tangle src/kcats/stdlib/debug-step.kcats :mkdirp yes
  ;; To enable debug stepping through code, even when there are inner
  ;; environments, we need to avoid calling `evaluate` in a single step,
  ;; since that will advance too far in a single step.
  ;;
  ;; We do that by redefining `evaluate` such that it's not an axiom
  ;; word anymore, but rather a kcats program expressed in terms of
  ;; eval-step.
  [[evaluate [[spec [[environment] [environment]]]
              [definition [[finished? not] [eval-step] while]]]]]
  join
#+end_src
****  Examples
