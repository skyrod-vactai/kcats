...
#+begin_src kcats :tangle src/kcats/core/dictionary-builtins.kcats :mkdirp yes
  ...
  [namespaced [[spec [[word bytes] [word]]]]]

  [alias [[spec [[bytes word] [word]]]]]
...
#+begin_src kcats :tangle src/kcats/core/dictionary.kcats :mkdirp yes
  ...
  [let [[spec [[[program p]
        ...
        [definition [[[0] swap ;; add an alias (0 means don't bother with creating the alias)
                      draft inscribe wrap] dip ;; update dict and then wrap the hash as the module to be used 
  ...
  [inscribe2 [[spec [[program] []]]
              [definition [dictionary swap shielddown]]]]

  [stdmod [[spec [[[list wrapped-module-alias]] [program]]]
           [doc "reads a cached stdlib module from disk and puts it on the stack as a program"]
           [definition [unwrap decache string read]]
           [examples [[["123" encode [crypto] stdmod [hash] boomerang]
...
#+begin_src kcats :tangle src/kcats/stdlib/environment.kcats :mkdirp yes
  [[tos [[spec [[environment] [item]]]
   ...
   [spawn [[spec [[program [program module]] [environment]]]
           [definition [[dictionary swap shielddown] dip ;; p d 
                        [snapshot] dipdown ;; p d s
                        [program dictionary stack] label
                        environment]]
           [examples [[[1 2 3 [] [swap clone] spawn]
                       [1 2 3 [[program [swap clone]] [stack [3 2 1]]] environment]]]]]]

   [boomerang [[spec [[program [program module] [*]]]]
