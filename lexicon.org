...
#+begin_src kcats :tangle src/kcats/builtins.kcats
  ...
  [hash [[spec [[bytes] [bytes]]]
         [examples [[[["foo" hash] 2 times =] [true]]
                    [["foo" bytes hash
                      "fop" bytes hash
                      =]
                     [false]]]]]]
...
#+begin_src kcats
  [1 "two" +] [[discard 2] dip retry] recover
#+end_src

#+RESULTS:
: [[unwound [discard 2 [[[unwound [+]] [type error] [asked [number]] [reason "type mismatch"]]] unwrap retry]] [reason "not enough items on stack"] [type error] [asked [consume]]]
