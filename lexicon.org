...
#+begin_src kcats :tangle src/kcats/core/dictionary.kcats :mkdirp yes
    ...
    [let [[spec [[[program p]
          ...
          [definition [[draft dictionary swap [emit encode hashbytes] shield
                       ...
                       [program dictionary resolver] label ;; creates closure
                       ;; when the closure executes, capture the outer stack first
                       wrap
                       [[stack] [snapshot] divedown assign environment evaluate
                        [stack] lookup restore] join];; creates closure] 
          [examples [[[[[times5 [5 *]]
                        [doubledec [dec dec]]]
...
#+begin_src kcats
  ...
  ;; when the closure executes, capture the outer stack first
  wrap
  [[stack] [snapshot] divedown assign evaluate [stack] lookup restore] join execute
#+end_src
