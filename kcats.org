...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  impl TryDerive<coll::Sized> for Associative {
      fn try_derive(s: coll::Sized) -> Result<Self, Error> {
          match s {
              ...
              coll::Sized::String(i) => {
                  Err(Error::expected(fit!("associative"), coll::Sized::String(i)))
              }
              coll::Sized::Bytes(i) => {
                  Err(Error::expected(fit!("associative"), coll::Sized::Bytes(i)))
              }
  ...
  impl TryDerive<Item> for Associative {
      fn try_derive(i: Item) -> Result<Self, Error> {
          let s =
              Result::<coll::Sized, DeriveError<Item>>::derive(i.clone()).map_err(Error::derive)?;
