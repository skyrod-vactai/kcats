...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  impl<T> DeriveError<T> {
      ...
      pub fn update<F, U>(self, f: F) -> DeriveError<U>
      where
          F: FnOnce(T) -> U,
      {
          DeriveError {
  ...
  impl<I> Derive<Error> for DeriveError<I>
  where
      I: Derive<types::Item>,
  {
      fn derive(e: Error) -> DeriveError<I> {
          DeriveError::expected(fit!("fixme"), e.actual().cloned().unwrap_or_default().fit())
      }
  }
  ...
  pub trait RewrapExt: IntoIterator + Sized {
      ...
      /// Fallible: Uses TryDerive. Returns Result<Target, E>.
      fn try_rewrap<Target, U>(self) -> Result<Target, Error>
      where
          Target: FromIterator<U>,
          U: TryDerive<Self::Item>,
      {
          self.into_iter().map(U::try_derive).collect()
      }
  }
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  impl Derive<coll::Sized> for Result<Associative, DeriveError<coll::Sized>> {
      fn derive(i: coll::Sized) -> Result<Associative, DeriveError<coll::Sized>> {
          match i {
              ...
              coll::Sized::List(l) => <Result<Associative, DeriveError<coll::List>>>::derive(*l)
  ...
  impl Derive<coll::List> for Result<Associative, DeriveError<coll::List>> {
      fn derive(l: coll::List) -> Result<Associative, DeriveError<coll::List>> {
          l.iter()
              .cloned()
              .try_rewrap::<AssociationContent, Entry>()
              .map(Arc::new)
              .map(Associative::Assoc)
              .map_err(Fit::fit)
          //.map_err(|e| <DeriveError<coll::List>>::derive(e))
      }
  }
  ...
  impl TryDerive<&edn_format::Value> for Item {
      fn try_derive(item: &edn_format::Value) -> Result<Item, Error> {
          ...
          match item {
              ...
              edn_format::Value::Vector(v) => Ok(v.try_rewrap::<coll::List, Item>()?.fit()),
          ...
          if ks.is_empty() {
              ...
              match (i, k) {
                  ...
                  (a, k) => {
                      ...
                      let mut a: assoc::Association = Arc::new(a.to_iter().rewrap());
                      let am = a.mutate();
