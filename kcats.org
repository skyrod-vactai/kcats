...
#+end_src
*** Compiler
#+begin_src rust :tangle src/compile.rs :mkdirp yes
  ...
  pub fn compile(list: &cont::List) -> Chunk {
      ...
      for item in list.iter() {
          match item {
              Item::Word(w) => ops.push(Op::Call(w.clone())),
              _ => ops.push(Op::Push(item.clone())),
          }
      }
      ops.push(Op::Return);
  ...
  pub fn compile_recur(
      ...
      if !is_execute_first {
          let mut pred_chunk = compile(pred);
          pred_chunk.ops.pop();
          ...
          let mut true_chunk = compile(true_branch);
          true_chunk.ops.pop();
          ops.extend(true_chunk.ops);
          
          // Push recur node
          // We need the full structure here. 
          // We'll construct a Chunk with the full recur structure dynamically via execute/call.
          // Wait, if we push a Program, that Program contains a Frame with a newly compiled Chunk of this whole recur.
          // Or we can just dynamically reconstruct the list for the old way? 
          // Yes, we can push the source lists!
          let mut list = cont::List::new();
          ...
          let mut comb_chunk = compile(combinator);
          comb_chunk.ops.pop();
          ...
          let mut then_chunk = compile(false_branch);
          then_chunk.ops.pop();
      ...
      let mut comb_chunk = compile(&comb_rest);
      comb_chunk.ops.pop();
      ops.extend(comb_chunk.ops);
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  use crate::types::container::program::{self, Program, Frame};
  use crate::types::container::{
  ...
  pub fn eval_step(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      if let Some(o) = &op { println!("OP: {:?}, STACK: {:?}", o, env.stack); }
  ...
  pub fn self_insert(env: Environment) -> Sometime<'static, Environment> {
      env.fit()
  }
