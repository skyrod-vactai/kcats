...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  /// A trait that gives the name of the type - used in kcats specs for
  /// example
  pub trait TypeName {
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  //     fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
  //         match self {
  //             Sized::Assoc(a) => write!(f, "{:?}", a),
  //             Sized::Error(a) => write!(f, "{:?}", a),
  //             Sized::Dictionary(a) => write!(f, "{:?}", a),
  //             Sized::Words(a) => write!(f, "{:?}", a),
  //             Sized::DictEntry(a) => write!(f, "{:?}", a),
  //             Sized::Nothing => write!(f, "{:?}", a),
  ...
  impl PartialEq for Sized {
      fn eq(&self, other: &Self) -> bool {
          Item::derive(self.clone()) == Item::derive(other.clone())
      }
  }
  ...
  impl Sized {
      ...
      pub fn pop(self) -> (Self, Option<Item>) {
          match self {
              ...
              _ => {
                  let a = assoc::Associative::try_derive(&self).unwrap().take_simple();
                  (self, a)
              }
          }
      }
      ...
      pub fn put(self, other: Item) -> Result<Sized, Error> {
          match (self, other) {
              ...
              (a, i) => assoc::Associative::try_derive(&self)
                  .unwrap()
                  .put(i)
                  .map(Sized::derive),
          }
  ...
  impl TryDerive<Sized> for List {
      ...
      fn try_derive(s: Sized) -> Result<Self, Self::Error> {
          match s {
              Sized::List(l) => Ok(*l),
              i => match assoc::Associative::try_derive(&i) {
                  Ok(a) => Ok(List::derive_iter(a.to_iter().map(Item::derive))),
                  Err(i) => Err(Error::expected(fit!("list"), i)),
              },
          }
      }
  }
  ...
  impl TryDerive<Item> for Receptacle {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Nothing => Ok(Receptacle::Sized(Sized::Nothing)),
  ...
  impl Derive<Dispenser> for Item {
      fn derive(d: Dispenser) -> Self {
          match d {
              ...
              Dispenser::Sized(Sized::Nothing) => Item::Nothing,
  ...
  impl Derive<Receptacle> for Item {
      fn derive(r: Receptacle) -> Self {
          match r {
              ...
              Receptacle::Sized(Sized::Nothing) => Item::Nothing,
  ...
  impl TryDerive<Item> for Dispenser {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Nothing => Ok(Dispenser::Sized(Sized::Nothing)),
  ...
  /// Try to convert any reference to our types to T, and don't destroy the
  /// original in case it fails (so the caller could try again if
  /// needed.)
  impl<T: TryDerive<Dispenser, Error = E>, E> TryDerive<&Dispenser> for T
  where
      T: TryDerive<Dispenser>,
      E: Fit<Error>,
  {
      type Error = Error;
      fn try_derive(value: &Dispenser) -> Result<Self, Self::Error> {
          T::try_derive(value.clone()).map_err(Fit::fit)
      }
  }

  impl<T: TryDerive<Receptacle, Error = E>, E> TryDerive<&Receptacle> for T
  where
      T: TryDerive<Receptacle>,
      E: Fit<Error>,
  {
      type Error = Error;
      fn try_derive(value: &Receptacle) -> Result<Self, Self::Error> {
          T::try_derive(value.clone()).map_err(Fit::fit)
      }
  }

  impl<T: TryDerive<Sized, Error = E>, E> TryDerive<&Sized> for T
  where
      T: TryDerive<Sized>,
      E: Fit<Error>,
  {
      type Error = Error;
      fn try_derive(value: &Sized) -> Result<Self, Self::Error> {
          T::try_derive(value.clone()).map_err(Fit::fit)
      }
  }
  ...
  impl Derive<Associative> for coll::Sized {
      fn derive(a: Associative) -> coll::Sized {
          convert_enum!(
              ...
              Associative,
              coll::Sized,
              [Assoc, Dictionary, DictEntry, Error, Env, Nothing, Words]
          )
      }
  }
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  fn get_name<T: 'static>(_val: &T) -> &'static str {
      match TypeId::of::<T>() {
          t if t == TypeId::of::<Vec<u8>>() => "bytes",
          t if t == TypeId::of::<String>() => "string",
          t if t == TypeId::of::<&str>() => "string",
          t if t == TypeId::of::<Word>() => "word",
          t if t == TypeId::of::<coll::List>() => "list",
          t if t == TypeId::of::<Item>() => "item",
          t if t == TypeId::of::<coll::Set>() => "set",
          t if t == TypeId::of::<coll::Dispenser>() => "dispenser",
          t if t == TypeId::of::<coll::Receptacle>() => "receptacle",
          t if t == TypeId::of::<coll::Sized>() => "sized",
          t if t == TypeId::of::<assoc::Associative>() => "associative",
          t if t == TypeId::of::<assoc::Association>() => "association",
          t if t == TypeId::of::<Error>() => "error",
          t if t == TypeId::of::<Environment>() => "environment",
          t if t == TypeId::of::<dict::Dictionary>() => "dictionary",
          t if t == TypeId::of::<number::Number>() => "number",
          _ => std::any::type_name::<T>(),
      }
  }
  #[cfg(test)]
  mod tests {
