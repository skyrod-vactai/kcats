  ...
  impl PartialEq for Item {
      fn eq(&self, other: &Self) -> bool {
          match (self, other) {
              ...
              (Item::Associative(a), Item::Associative(b)) => a == b,
  ...
  mod serde {
      ...
      impl<'de> Visitor<'de> for ItemVisitor {
              ...
              A: de::MapAccess<'de>,
          {
              ...
              Ok(Item::Associative(assoc::Associative::Assoc(map.fit())))
          }
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Sized {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          try_convert_enum!(item, Item => Sized, [Bytes, String, List, Associative, Set, Program], |i| Error::expected(fit!("sized"), i) )
      }
  }
  ...
  impl TryDerive<Item> for Receptacle {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Associative(a) => Ok(Receptacle::Sized(Sized::Associative(a))),
  ...
  impl Derive<Dispenser> for Item {
      fn derive(d: Dispenser) -> Self {
          match d {
              ...
              Dispenser::Sized(Sized::Associative(a)) => Item::Associative(a),
  ...
  impl Derive<Receptacle> for Item {
      fn derive(r: Receptacle) -> Self {
          match r {
              ...
              Receptacle::Sized(Sized::Associative(a)) => Item::Associative(a),
  ...
  impl TryDerive<Item> for Dispenser {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Associative(a) => Ok(Dispenser::Sized(Sized::Associative(a))),
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Error {
      ...
      fn try_derive(i: Item) -> Result<Self, Self::Error> {
          match i {
              Item::Associative(assoc::Associative::Error(e)) => Ok(e),
...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
  ...
  impl Derive<Entry> for Item {
      fn derive(e: Entry) -> Self {
          Item::Associative(assoc::Associative::DictEntry(Box::new(e)))
      }
  }

  impl Derive<Words> for Item {
      fn derive(e: Words) -> Self {
          Item::Associative(assoc::Associative::Words(e))
      }
  }

  impl Derive<Dictionary> for Item {
      fn derive(d: Dictionary) -> Self {
          Item::Associative(assoc::Associative::Dictionary(Box::new(d)))
      }
  }
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  fn check_type(i: &Item, w: &Word) -> bool {
      ...
      match (w, i) {
          ...
          (w, Item::Associative(_)) if *w == *S_ASSOC => true,

          (w, Item::Associative(assoc::Associative::Error(_))) if *w == *S_ERROR => true,

          (w, Item::Associative(assoc::Associative::Dictionary(_))) if *w == *S_DICTIONARY => true,
          ...
          (w, Item::Associative(assoc::Associative::Env(_))) if *w == *S_ENVIRONMENT => true,
          (_, _) => false,
      }
  }
  ...
  impl From<&Item> for edn_format::Value {
      fn from(item: &Item) -> Self {
          match item {
              ...
              Item::Associative(assoc::Associative::Words(e)) => edn_format::Value::Symbol(
                  edn_format::Symbol::from_name(format!("{}_entries", e.len()).as_str()),
              ),
              Item::Associative(assoc::Associative::Env(e)) => (&e.representation()).into(),
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn is_error(i: Item) -> bool {
      matches!(i, Item::Associative(assoc::Associative::Error(_),))
  }
  ...
  pub fn is_environment(i: Item) -> bool {
      matches!(i, Item::Associative(assoc::Associative::Env(_)))
  }
  ...
  fn assoc_in(i: Option<Item>, ks: &[assoc::KeyItem], v: Item) -> Result<Item, Error> {
      ...
      if let [k, ks @ ..] = ks {
          match (i, k) {
              ...
              (Some(Item::Associative(a)), assoc::KeyItem::Int(k)) => {
  ...
  fn get_error(env: &Environment) -> Option<Error> {
      env.tos().and_then(|i| match i {
          Item::Associative(assoc::Associative::Error(e)) => Some(e.clone()),
          _ => None,
      })
  }
  ...
  fn unwind(mut env: Environment) -> Environment {
      ...
      let err = match err {
          Item::Associative(assoc::Associative::Error(mut e)) => {
  ...
  fn handle(mut env: Environment) -> Future<Environment> {
      ...
      match env.stack.front_mut() {
          None => {}
          Some(i) => match i {
              Item::Associative(assoc::Associative::Error(ref mut e)) => {
                  e.is_handled = true;
              }
              _ => {}
          },
