...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  impl PartialEq for ProgramNode {
      fn eq(&self, other: &Self) -> bool {
          match (self, other) {
              ...
              (
                  ProgramNode::Recur { pred: p1, then: t1, rec1: r1, rec2: c1, .. },
                  ProgramNode::Recur { pred: p2, then: t2, rec1: r2, rec2: c2, .. }
              ) => {
                  Arc::ptr_eq(p1, p2)
                      && Arc::ptr_eq(t1, t2)
                      && Arc::ptr_eq(r1, r2)
  ...
  pub struct Frame {
      ...
      pub next_idx: usize,
      pub source: Option<crate::types::Item>,
  }
  ...
  impl Frame {
      pub fn is_finished(&self) -> bool {
          match &*self.node {
              ...
              ProgramNode::Recur { .. } => self.next_idx >= 2, // 0 = evaluate pred, 1 = branch
          }
      }
          ...
          match &*self.node {
              ...
              ProgramNode::Recur { pred, then, rec1, rec2, .. } => {
                  if self.next_idx == 0 {
                      ...
                      if let ProgramNode::Seq(s) = &**rec1 {
                          for item in s.iter() {
                              true_branch_list.push_back(item.clone());
                          }
                      }
                   ...
                   source: None }]))));
                      ...
                      l.push_back(Item::Program(Box::new(Program(vec![Frame { node: std::sync::Arc::new(ProgramNode::Seq(true_branch_list)), next_idx: 0,
                   source: None }]))));
  ...
  impl Program {
      ...
      /// tree nodes (like Recur). It returns true if it performed an action
      ...
      pub fn advance_control_flow(&mut self, stack: &mut crate::types::container::stack::Stack) -> bool {
          ...
          if let ProgramNode::Recur { pred, then, rec1, rec2, self_ref } = &*frame.node {
              if frame.next_idx == 0 {
                  // Phase 0: Push pred onto the execution stack
                  frame.next_idx = 1;
                  ...
                  return true;
              } else if frame.next_idx == 1 {
                  // Phase 1: Check result from data stack
                  frame.next_idx = 2; // mark finished
                  
                  let is_true = stack.front().map(|i| crate::axiom::is_truthy(i)).unwrap_or(false);
                  if !is_true {
                      stack.pop_front();
                  }

                  if is_true {
                      let rec2_clone = rec2.clone();
                      let self_clone = self_ref.upgrade().expect("self_ref should be alive");
                      let rec1_clone = rec1.clone();
                                    let mut is_execute_first = false;
                      if let ProgramNode::Seq(s) = &*rec2_clone {
                          if !s.is_empty() {
                              if let Some(crate::types::Item::Word(w)) = s.front() {
                      ...
                      // We can just pop the finished frame now
                      self.0.pop();
                      ...
                      if is_execute_first {
                          ...
                          if let ProgramNode::Seq(s) = &*rec2_clone {
                              if s.len() > 1 {
                                  ...
                                  s_clone.pop_front();
                                  rest_node = Some(std::sync::Arc::new(ProgramNode::Seq(s_clone)));
                              }
                          }
                          if let Some(rn) = rest_node {
                              self.0.push(Frame { node: rn, next_idx: 0,
                   source: None });
                          }
                          self.0.push(Frame { node: self_clone, next_idx: 0,
                   source: None });
                          self.0.push(Frame { node: rec1_clone, next_idx: 0,
                   source: None });
                      } else { 
                          let mut l = crate::types::coll::List::new();
                          let mut prog = Program::default();
                          prog.push_frame(Frame { node: self_clone, next_idx: 0,
                   source: None });
                          l.push_back(crate::types::Item::Program(Box::new(prog)));
                          let push_recur_node = std::sync::Arc::new(ProgramNode::Seq(l));

                          self.0.push(Frame { node: rec2_clone, next_idx: 0,
                   source: None });
                          self.0.push(Frame { node: push_recur_node, next_idx: 0,
                   source: None });
                          self.0.push(Frame { node: rec1_clone, next_idx: 0,
                   source: None });
                      }
      ...
      pub fn prepend_with_source(&mut self, snippet: cont::List, source: Option<crate::types::Item>) {
          self.clean();
          if !snippet.is_empty() {
              self.0.push(Frame {
                  node: Arc::new(ProgramNode::Seq(snippet)),
      ...
      pub fn stacktrace(&self) -> cont::List {
          let s = self
              ...
              .iter()
              .rev()
              .filter_map(|frame| frame.source.clone().or_else(|| frame.current().cloned()))
              .collect::<cont::List>();
          //s.slice(0..(s.len() - 1))
          s
      }
          ...
          self.0
              ...
              .fold(0, |acc, f| {
                  match &*f.node {
                      ...
                      ProgramNode::Recur { .. } => acc + (2 - f.next_idx),
                  }
              })
      }
              ...
              Associative::DictEntry(mut de) => {
                  if let KeyItem::Word(ref w) = k {
                      ...
                      if w == "definition" {
                          ...
                          if let Ok(l) = l {
                              de.definition = dict::Executable::Derived(std::sync::Arc::new(crate::types::container::program::ProgramNode::Seq(l)));
...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
              ...
              assoc::KeyItem::Word(w) => match w.data.as_str() {
                  ...
                  "definition" => Some(match self.definition.clone() {
                      ...
                      Executable::Derived(d) => {
                          match &*d {
                              crate::types::container::program::ProgramNode::Seq(s) => s.clone().fit(),
                              _ => panic!("Expected Seq for Derived definition"),
                          }
                      },
                  }),
  ...
  pub enum Executable {
      ...
      Derived(Arc<crate::types::container::program::ProgramNode>),
  }
      ...
      fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
          match self {
              ...
              Executable::Derived(d) => {
                  ...
                  if let crate::types::container::program::ProgramNode::Seq(s) = &**d {
                      ds.entries(s.iter());
                  }
                  ds.finish()
              }
  ...
  impl TryDerive<Box<dyn Iterator<Item = Item>>> for Entry {
      fn try_derive(iter: Box<dyn Iterator<Item = Item>>) -> Result<Self, Error> {
          ...
          Ok(Entry {
              examples,
              definition: definition.unwrap_or(Executable::Derived(Arc::new(crate::types::container::program::ProgramNode::Seq(coll::List::default())))),
              spec,
              namespace,
              doc,
          })
  ...
  impl TryDerive<Item> for Executable {
      fn try_derive(i: Item) -> Result<Self, Error> {
          coll::List::try_derive(i).map(|l| Executable::Derived(Arc::new(crate::types::container::program::ProgramNode::Seq(l))))
      }
  }
  ...
  impl Derive<Entry> for assoc::Associative {
      fn derive(d: Entry) -> assoc::Associative {
          ...
          if let Executable::Derived(d) = d.definition {
              match &*d {
                  crate::types::container::program::ProgramNode::Seq(s) => {
                      a.insert(fit!("definition"), s.clone().fit());
                  },
                  _ => panic!("Expected Seq for Derived definition"),
              }
          }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn eval_step(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      let next_item = env.program.peek().cloned();
                              ...
                              match &dfn.definition {
                                  ...
                                  dict::Executable::Derived(d) => {
                                      ...
                                      env.program.push_frame(crate::types::container::program::Frame {
                                          node: d.clone(),
                                          next_idx: 0,
                                          source: Some(crate::types::Item::Word(word.clone())),
                                      });
                                      env.fit()
                                  }
                              }
...
#+begin_src rust :tangle src/lib.rs :mkdirp yes
  ...
  mod tests {
      ...
      use crate::axiom;
      use crate::derivation::{Fit, TryDerive, TryFit};
