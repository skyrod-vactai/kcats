...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
  ...
  impl Dict for Words {
      ...
      fn merge(&mut self, mut new: Words, namespace: &Namespace) {
  ...
  impl Default for Environment {
      ...
      fn default() -> Self {
          static INST: Lazy<Environment> = Lazy::new(|| {
              ...
              let modules = dict::CORE_NAMESPACES
                  .get()
                  .unwrap()
                  .iter()
                  .map(|(_, ns)| ns.clone())
                  .collect::<Vec<_>>();
              env.dictionary.modules = modules;
              env.dictionary.resolve();
              println!("starting dictionary: {:?}", env.dictionary.lingo);
