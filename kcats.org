...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  use crate::fit;
  use crate::types;
  use crate::types::container::error::{Error, Nested, NestedDeriveError};
  ...
  pub trait RewrapExt: ToIterator + Sized {
      ...
      where
          ...
          U: Derive<Self::Item>,
      {
          self.to_iter().map(U::derive).collect()
      }

      /// Fallible: Uses Derive specifically for DeriveErrors. Returns Result<Target, E>.

      fn try_rewrap<Target, Inner>(self) -> Result<Target, NestedDeriveError<Self>>
      where
          Self::Item:
              Clone + Fit<Result<<Target as IntoIterator>::Item, NestedDeriveError<Self::Item>>>,
          Target: FromIterator<<Target as IntoIterator>::Item> + IntoIterator,
      {
          let original = self.clone();
          self.to_iter()
              .map(Fit::fit)
              .collect::<Result<Target, NestedDeriveError<Self::Item>>>()
              .map_err(|inner_err| NestedDeriveError {
                  src: original,
                  expected: inner_err.expected,
                  inner: inner_err,
              })
      }
  }

  impl<I: ToIterator + Clone> RewrapExt for I {}

  pub trait ToIterator: Clone {
      type Item;
  ...
  // Blanket impl for anything that's Clone + IntoIterator
...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  impl TryDerive<String> for Word {
      fn try_derive(s: String) -> Result<Self, Error> {
          serialize::parse(s).and_then(|mut l| {
              if l.len() == 1 {
                  match l.pop_front().unwrap() {
                      Item::Word(w) => Ok(w),
                      _ => Err(Error::parse("Words cannot contain delimiters")),
                  }
              } else if l.is_empty() {
          ...
          match i {
              ...
              i => {
                  ...
                  match s {
                      ...
                      i => Err(Error::expected(fit!("string"), Item::derive(i))),
                  }
              }
          }
      }
  ...
  /// conflicts with other useful impls. The main one we need is this
  /// one, for when we have a trait bounds of Fit<Item> and it might get
  /// passed an Item already.
  impl TryDerive<Item> for Item {
      fn try_derive(i: Item) -> Result<Self, Error> {
          Ok(i)
      }
  }
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
      ...
      fn join(self, other: Sized) -> Result<Self::Output, Error> {
          ...
          Ok(match (self, other) {
              ...
              (Sized::Words(a), Sized::List(b)) => {
                  // maybe it's an assoc
                  let ba: assoc::Association = assoc::Association::try_from_iter(b.iter().cloned())?;
      ...
      fn has(&self, other: &Item) -> bool {
          ...
          match (self, other) {
              ...
              (s, other) => {
                  ...
                  //}
                  let c = Result::<assoc::Associative, NestedDeriveError<Sized>>::derive(s.clone())
                      .unwrap();
      ...
      pub fn pop(self) -> (Self, Option<Item>) {
          match self {
              ...
              s => {
                  ...
                  let mut c =
                      Result::<assoc::Associative, NestedDeriveError<Sized>>::derive(s).unwrap();
                  let x = c.take_simple();
                  (Sized::derive(c), x)
              }
          }
  ...
  impl TryDerive<Sized> for List {
      fn try_derive(s: Sized) -> Result<Self, Error> {
          match s {
              Sized::List(l) => Ok(*l),
              s => Ok(List::derive_iter(s.to_iter().map(Item::derive))),
          }
      }
  }
  ...
  impl TryDerive<Item> for List {
      fn try_derive(i: Item) -> Result<Self, Error> {
          Sized::try_derive(i).and_then(List::try_derive)
      }
  }

  impl Derive<Item> for Result<Sized, NestedDeriveError<Item>> {
      fn derive(i: Item) -> Self {
          match i {
              ...
              i => Err(NestedDeriveError::expected(fit!("sized"), i)),
  ...
  mod serde {
      ...
      use crate::serialize::Display;
      use crate::traits::*;
      use crate::types::container::{associative as assoc, error::NestedDeriveError};
      ...
      impl Serialize for Sized {
              ...
              S: serde::Serializer,
          {
              match self {
                  ...
                  s => {
                      let a =
                          Result::<assoc::Associative, NestedDeriveError<Sized>>::derive(s.clone())
                              .unwrap();
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  use crate::traits::*;
  use crate::types::container as coll;
  use crate::types::container::{error::NestedDeriveError, Count, Join, Mutey};
  use crate::types::number::Int;
  use crate::types::*;
  ...
  impl TryDerive<coll::Sized> for Associative {
      fn try_derive(s: coll::Sized) -> Result<Self, Error> {
          Result::<Associative, NestedDeriveError<coll::Sized>>::derive(s).map_err(Error::derive)
      }
  }

  impl Derive<Item> for Result<Associative, NestedDeriveError<Item>> {
      fn derive(i: Item) -> Result<Associative, NestedDeriveError<Item>> {
          match i {
              ...
              Item::List(l) => l
                  .into_iter()
                  .try_rewrap::<AssociationContent, Entry>()
                  .map(Arc::new)
                  .map(Associative::Assoc)
                  .map_err(NestedDeriveError::derive),
              i => Err(NestedDeriveError::expected(fit!("associative"), i)),
          }
      }
  }

  impl Derive<coll::Sized> for Result<Associative, NestedDeriveError<coll::Sized>> {
      fn derive(i: coll::Sized) -> Result<Associative, NestedDeriveError<coll::Sized>> {
          match i {
  ...
  impl TryDerive<Item> for Associative {
      fn try_derive(i: Item) -> Result<Self, Error> {
          Result::<assoc::Associative, NestedDeriveError<Item>>::derive(i).map_err(Error::derive)
      }
  }

  impl Derive<coll::List> for Result<Associative, NestedDeriveError<coll::List>> {
      fn derive(l: coll::List) -> Result<Associative, NestedDeriveError<coll::List>> {
          l.iter()
              .cloned()
              .try_rewrap::<AssociationContent, Entry>()
              .map(Arc::new)
              .map(Associative::Assoc)
              .map_err(Fit::fit)
          //.map_err(|e| <NestedDeriveError<coll::List>>::derive(e))
      }
  }
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  pub struct NestedDeriveError<Src> {
      pub src: Src,
      pub inner: Option<Box<dyn std::error::Error>>,
      pub expected: Word,
  }

  impl<T> Debug for NestedDeriveError<T>
  where
      T: Debug,
  {
      fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
          write!(
              f,
              "DeriveError[src: {:?}, expected: {:?}, inner: {:?}]",
              self.src, self.expected, self.inner
          )
      }
  }
  impl<T> NestedDeriveError<T> {
      pub fn expected(expected: Word, src: T) -> NestedDeriveError<T> {
          NestedDeriveError {
              src,
              expected,
              inner: None,
  ...
  impl<Src> NestedDeriveError<Src> {
      pub fn swap<Dest>(self, updated: Dest) -> NestedDeriveError<Dest> {
          NestedDeriveError {
              src: updated,
      ...
      pub fn update<Dest, F>(self, f: F) -> NestedDeriveError<Dest>
      where
          F: FnOnce(Src) -> Dest,
      {
          NestedDeriveError {
      ...
      pub fn clear_inner(self) -> NestedDeriveError<Src> {
          NestedDeriveError {
              src: self.src,
              expected: self.expected,
              inner: None,
  ...
  /// Convert DeriveError to Error
  impl<I> Derive<NestedDeriveError<I>> for Error
  where
      I: Fit<Item>,
  {
      fn derive(de: NestedDeriveError<I>) -> Error {
          Error::expected(de.expected, Some(de.src.fit()))
      }
  }

  impl<T, I> Derive<Result<T, NestedDeriveError<I>>> for Result<T, Error>
  where
      I: Fit<Item>,
  {
      fn derive(r: Result<T, NestedDeriveError<I>>) -> Self {
          r.map_err(Error::derive)
      }
  }

  /// Convert Error to DeriveError
  impl<I> Derive<Error> for NestedDeriveError<I>
  where
      I: Derive<Item>,
  {
      fn derive(e: Error) -> NestedDeriveError<I> {
          NestedDeriveError::expected(fit!("fixme"), e.actual().cloned().unwrap_or_default().fit())
      }
  }
  ...
  impl Nested<Error> for Error {
      fn nest(mut self, other: Error) -> Self {
          self.data.mutate().insert(fit!("cause"), other.fit());
          self
      }
  }
  ...
  impl<T> Derive<NestedDeriveError<T>> for Item
  where
      T: Fit<Item>,
  {
      fn derive(e: NestedDeriveError<T>) -> Item {
          //return an Error
          Item::Error(e.fit())
      }
  }

  impl<T> From<NestedDeriveError<T>> for Item
  where
      T: Fit<Item>,
  {
      fn from(e: NestedDeriveError<T>) -> Item {
          //return an Error
          Item::Error(e.fit())
      }
  }

  impl<T> From<NestedDeriveError<T>> for Error
  where
      T: Fit<Item>,
  {
      fn from(de: NestedDeriveError<T>) -> Error {
          //return an Error
          let err = Error::expected(de.expected.fit(), de.src.fit());
          if let Some(inner) = de.inner {
              err.nest(std::error::Error::from(inner))
          } else {
              err
          }
      }
  ...
  {
      fn from(de: NestedDeriveError<T>) -> Error {
          Error::expected(de.expected.fit(), de.src.fit())
      }
  }
...
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  ...
  pub trait DeriveMode: Sized + 'static {
      type Detail<Bounce>;

      fn predicate_failed<Bounce>(src: Bounce, expected: Word) -> Self::Detail<Bounce>;
      fn nest_error<Bounce>(outer: Bounce, inner: Bounce) -> Bounce;
  }
  ...
  impl DeriveMode for Fast {
      type Detail<Src> = Src;

      #[inline(always)]
      fn predicate_failed<Src>(src: Src, _expected: Word) -> Src {
          src
      }

      fn nest_error<Src>(outer: Src, inner: Src) -> Src {
          outer // nesting not allowed here, we short circuit and return the outer original value
      }
  }
  ...
  impl DeriveMode for Detailed {
      type Detail<Err> = Error;

      fn predicate_failed<Err>(src: Err, expected: Word) -> Error {
          Error::expected(expected, Default::default())
      }

      fn nest_error<Err>(outer: Err, inner: Err) -> Err {
          outer.nest(inner)
      }
  }
  ...
  pub trait TryDerive<T, M: DeriveMode = Detailed>: Sized {
      fn try_derive(value: T) -> Result<Self, M::Detail<T>>;
  }

  //impl TryDeriveSoft<F, F> for T
  ...
  pub trait TryFit<T>: Sized {
      fn try_fit(self) -> Result<T, Error>;
  }
  ...
  where
      U: TryDerive<T>,
  {
      fn try_fit(self) -> Result<U, Error> {
          U::try_derive(self)
      }
  }
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  use crate::traits::*;
  use crate::types::container::{
      self as coll, associative as assoc,
  ...
  impl Derive<edn_format::Value> for Result<Item, NestedDeriveError<edn_format::Value>> {
      fn derive(item: edn_format::Value) -> Result<Item, NestedDeriveError<edn_format::Value>> {
  ...
  impl TryDerive<Result<edn_format::Value, edn_format::ParserError>> for Item {
      fn try_derive(r: Result<edn_format::Value, edn_format::ParserError>) -> Result<Item, Error> {
          match r {
              Ok(expr) => Ok(expr.try_fit()?),
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  use crate::types::container::{
      self as coll, associative as assoc, dictionary as dict,
      environment::Environment,
      error::{self, Error, NestedDeriveError},
  ...
  /// stack item.
  fn f_stack1<F, Output, SpecInput, E>(f: F) -> impl Fn(Environment) -> Sometime<'static, Environment>
  where
      ...
      Output: Fit<Item>,
  {
      move |mut env: Environment| {
          ...
          match x {
              Ok(i) => match SpecInput::try_derive(i) {
                  Ok(i) => {
                      let res = f(i);
                      match res {
                          Ok(r) => {
  ...
  pub fn pop(mut env: Environment) -> Sometime<'static, Environment> {
      match coll::Sized::try_derive(env.pop()) {
          Ok(it) => {
              let (c, i) = it.pop();
              env.pop_prog();
              env.push(c);
  ...
  fn empty(s: Item) -> Result<coll::Sized, Error> {
      let s = coll::Sized::try_derive(s).map_err(Error::derive)?;
      Ok(s.empty().fit())
  }
  ...
  fn unpack(env: &mut Environment) -> Result<(), Error> {
      ...
      fn destructure(
          ...
          let data: Result<coll::Sized, NestedDeriveError<Item>> = data.clone().fit();
          let pattern: Result<coll::Sized, NestedDeriveError<Item>> = pattern.clone().fit();
          match (data, pattern) {
              (Ok(data), Ok(pattern)) => {
                  // We got containers
                  match (data, pattern) {
                      ...
                      (a, b) => {
                          ...
                          match (
                              Result::<assoc::Associative, NestedDeriveError<coll::Sized>>::derive(a),
                              Result::<assoc::Associative, NestedDeriveError<coll::Sized>>::derive(b),
                          ) {
                              (Ok(data), Ok(pattern)) => {
