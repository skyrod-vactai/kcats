...
#+begin_src rust :tangle src/types.rs :mkdirp yes
              ...
              Ok(mut l) => {
                  if l.len() == 1 {
                      match l.pop_front().unwrap() {
                          Item::Word(w) => Ok(w),
                          _ => Err(Error::expected(fit!("word"), Item::String(Box::new(original_s)))),
                      }
                  } else if l.is_empty() {
      ...
      fn try_derive(value: Item) -> Result<Self, Error> {
          match value {
              ...
              i => {
                  let s = coll::Sized::try_derive(i.clone())?;
                  match s {
                      coll::Sized::String(s) => s.try_fit().map_err(|_| Error::expected(fit!("word"), i.clone())),
                      s => {
  ...
  impl<T: TryCast<Item>> TryCast<Item> for Vec<T> {
      fn try_cast(value: Item) -> Result<Self, Item> {
          ...
          if let Ok(it) = it {
              let res: Result<Vec<T>, Item> = it.map(|item| {
      ...
      impl Serialize for Item {
              ...
              S: Serializer,
          {
              ...
              match self {
                  ...
                  Item::Words(w) => ser_map(serializer, Box::new(w.as_ref().clone().into_iter().map(|(k,v)| (k.fit(), v.fit())))),
                  Item::Nothing => {
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl Join<assoc::Associative> for List {
      ...
      fn join(self, other: assoc::Associative) -> Result<Self::Output, Self::Error> {
          ...
          let sa = assoc::Associative::Assoc(Arc::new(
              self.try_rewrap::<assoc::AssociationContent>()?,
          ));
  ...
  impl TryDerive<Item> for Set {
      fn try_derive(item: Item) -> Result<Self, Error> {
          ...
          s.try_rewrap::<HashSet<assoc::KeyItem>>()
              .map(Arc::new)
      }
  }
...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  pub enum ProgramNode {
      Seq(cont::List),
      Recur {
          pred: Arc<ProgramNode>,
          then: Arc<ProgramNode>,
          rec1: Arc<ProgramNode>,
          rec2: Arc<ProgramNode>,
          self_ref: std::sync::Weak<ProgramNode>,
      },
  }
  ...
  impl PartialEq for ProgramNode {
      fn eq(&self, other: &Self) -> bool {
          match (self, other) {
              (ProgramNode::Seq(a), ProgramNode::Seq(b)) => a == b,
              (
                  ProgramNode::Recur { pred: p1, then: t1, rec1: r1a, rec2: r2a, self_ref: s1 },
                  ProgramNode::Recur { pred: p2, then: t2, rec1: r1b, rec2: r2b, self_ref: s2 }
              ) => p1 == p2 && t1 == t2 && r1a == r1b && r2a == r2b && std::sync::Weak::ptr_eq(s1, s2),
              _ => false,
          }
      }
  }
  ...
  #[derive(Clone, Debug)]
  pub struct Frame {
      pub node: Arc<ProgramNode>,
      pub next_idx: usize,
  }

  impl PartialEq for Frame {
      fn eq(&self, other: &Self) -> bool {
          Arc::ptr_eq(&self.node, &other.node) && self.next_idx == other.next_idx
      }
  }
  ...
  impl Frame {
      pub fn is_finished(&self) -> bool {
          match &*self.node {
              ProgramNode::Seq(snippet) => self.next_idx >= snippet.len(),
              ProgramNode::Recur { .. } => self.next_idx >= 4,
          }
      }
      ...
      pub fn next(&mut self) -> Option<&Item> {
          match &*self.node {
              ProgramNode::Seq(snippet) => {
                  let i = snippet.get(self.next_idx);
                  self.next_idx += 1;
                  i
              }
              ProgramNode::Recur { .. } => None, // Handled dynamically in advance_control_flow
          }
      }
      ...
      pub fn peek(&self) -> Option<&Item> {
          if self.is_finished() {
              ...
              match &*self.node {
                  ProgramNode::Seq(snippet) => snippet.get(self.next_idx),
                  ProgramNode::Recur { .. } => None,
              }
          }
      }
      ...
      pub fn current(&self) -> Option<&Item> {
          ...
          match &*self.node {
              ProgramNode::Seq(snippet) => snippet.get(self.next_idx - 1),
              ProgramNode::Recur { .. } => None,
          }
      }
      ...
      pub fn remaining(&mut self) -> cont::List {
          match &*self.node {
              ProgramNode::Seq(snippet) => {
                  let last = snippet.len();
                  snippet.clone().slice(self.next_idx..last)
              }
              ProgramNode::Recur { pred, then, rec1, rec2, .. } => {
                  let mut true_branch_list = cont::List::new();
                  
                  // 1. `then`
                  if let ProgramNode::Seq(s) = &*then.clone() {
                      true_branch_list.append(s.clone());
                  } else {
                      true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: then.clone(), next_idx: 0 }]))));
                  }

                  // 2. `[pred] [then] [rec1] [rec2]`
                  true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: pred.clone(), next_idx: 0 }]))));
                  true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: then.clone(), next_idx: 0 }]))));
                  true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec1.clone(), next_idx: 0 }]))));
                  true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec2.clone(), next_idx: 0 }]))));
                  
                  // 3. `🪆`
                  true_branch_list.push_back(crate::fit!("🪆"));
                  
                  // 4. `rec2`
                  if let ProgramNode::Seq(s) = &*rec2.clone() {
                      true_branch_list.append(s.clone());
                  } else {
                      true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec2.clone(), next_idx: 0 }]))));
                  }

                  let mut l = cont::List::new();
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: pred.clone(), next_idx: 0 }]))));
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: Arc::new(ProgramNode::Seq(true_branch_list)), next_idx: 0 }]))));
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec1.clone(), next_idx: 0 }]))));
                  l.push_back(crate::fit!("⚖️"));
                  l
              }
          }
      }
  ...
  impl Program {
      ...
      /// advance_control_flow handles the internal state progression for dynamic
      /// tree nodes (like PushCycle). It returns true if it performed an action
      /// and should be called again.
      pub fn advance_control_flow(&mut self, stack: &mut crate::types::container::stack::Stack) -> Result<bool, crate::types::container::error::Error> {
          self.clean();
          let frame_node;
          let phase;
          {
              let Some(frame) = self.0.last_mut() else { return Ok(false); };
              match &*frame.node {
                  ProgramNode::Recur { .. } => {
                      frame_node = frame.node.clone();
                      phase = frame.next_idx;
                  }
                  _ => return Ok(false),
              }
          }
          ...
          match &*frame_node {
              ProgramNode::Recur { pred, then, rec1, rec2, self_ref } => {
                  if phase == 0 {
                      self.0.last_mut().unwrap().next_idx = 1;
                      self.0.push(Frame { node: pred.clone(), next_idx: 0 });
                      return Ok(true);
                  } else if phase == 1 {
                      let Some(b) = stack.pop_front() else {
                          return Err(crate::types::container::error::Error::stack_underflow());
                      };
                      if crate::axiom::is_truthy(&b) {
                          self.0.last_mut().unwrap().next_idx = 2;
                          self.0.push(Frame { node: then.clone(), next_idx: 0 });
                      } else {
                          self.0.last_mut().unwrap().next_idx = 4;
                          self.0.push(Frame { node: rec1.clone(), next_idx: 0 });
                      }
                      return Ok(true);
                  } else if phase == 2 {
                      self.0.last_mut().unwrap().next_idx = 3;
                      let self_clone = self_ref.upgrade().expect("self_ref should be alive");
                      stack.push_front(Item::Program(Box::new(Program(vec![Frame { node: self_clone, next_idx: 0 }]))));
                      return Ok(true);
                  } else if phase == 3 {
                      self.0.last_mut().unwrap().next_idx = 4;
                      self.0.push(Frame { node: rec2.clone(), next_idx: 0 });
                      return Ok(true);
                  }
              }
              _ => {}
      ...
      pub fn is_empty(&self) -> bool {
          self.0.iter().all(|f| f.is_finished())
      }
      ...
      pub fn prepend(&mut self, snippet: cont::List) {
          ...
          if !snippet.is_empty() {
              self.0.push(Frame {
                  node: Arc::new(ProgramNode::Seq(snippet)),
                  next_idx: 0,
              })
          }
      }
      ...
      pub fn unwind_to_handle(&mut self, handle: &Item) -> (cont::List, bool) {
          let mut unwound = cont::List::new();
          let mut found = false;
          ...
          while let Some(f) = self.0.last_mut() {
              ...
              match &*f.node {
                  ProgramNode::Seq(items) => {
                      let mut local_found = false;
                      while f.next_idx < items.len() {
                          let item = &items[f.next_idx];
                          f.next_idx += 1;
                          if item == handle {
                              local_found = true;
                              break;
                          } else {
                              unwound.push_back(item.clone());
                          }
                      }
                      if local_found {
                          found = true;
      ...
      pub fn count(&self) -> usize {
          self.0
              ...
              .fold(0, |acc, f| {
                  match &*f.node {
                      ProgramNode::Seq(snippet) => acc + (snippet.len() - f.next_idx),
                      ProgramNode::Recur { .. } => acc + (4 - f.next_idx),
                  }
              })
      }
      ...
      pub fn last(&self) -> Option<&Item> {
          self.0.first().and_then(|f| {
              match &*f.node {
                  ProgramNode::Seq(snippet) => snippet.last(),
                  ProgramNode::Recur { .. } => None,
              }
          })
      }
  ...
  impl Derive<cont::List> for Program {
      fn derive(s: cont::List) -> Program {
          Program(vec![Frame {
              node: Arc::new(ProgramNode::Seq(s)),
              next_idx: 0,
          }])
      }
  }
  ...
  mod tests {
      ...
      fn test_simple() {
          let mut p = Program(vec![Frame {
              node: Arc::new(ProgramNode::Seq(list!["foo", "bar"])),
              next_idx: 0,
          }]);
      ...
      fn test_layers() {
          let mut p = Program(vec![
              Frame {
                  node: Arc::new(ProgramNode::Seq(list!["foo", "bar", "baz"])),
                  next_idx: 1,
              },
              Frame {
                  node: Arc::new(ProgramNode::Seq(list!["quuz", "blork", "stoon"])),
                  next_idx: 2,
              },
          ]);
          let q = p.clone();
...
#+begin_src rust :tangle src/types/container/stack.rs :mkdirp yes
  ...
  impl StackData {
      ...
      pub fn get(&self, index: usize) -> Option<&Item> {
          let cached = self.tos.is_some() as usize
              + self.nos.is_some() as usize
              + self.nnd.is_some() as usize;
          if index < cached {
              match index {
  ...
  impl StackData {
      ...
      pub fn replace3(&mut self, item: Item) {
          self.tos = Some(item);
          self.nos = if !self.rest.is_empty() { Some(self.rest.pop_front().unwrap()) } else { None };
          self.nnd = if !self.rest.is_empty() { Some(self.rest.pop_front().unwrap()) } else { None };
      }

      pub fn swap(&mut self, a: usize, b: usize) {
          if a == b { return; }
          // Very simplistic swap for now, can be optimized later
  ...
  impl IntoIterator for Associative {
      ...
      fn into_iter<'a>(self) -> Self::IntoIter {
          match self {
              ...
              Associative::Words(d) => Box::new(d.as_ref().clone().into_iter().map(|(k,v)| (crate::types::container::associative::KeyItem::Word(k), crate::types::Item::DictEntry(Box::new(v))))),
...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
  ...
  impl Derive<SpecElement> for Item {
      fn derive(se: SpecElement) -> Item {
          if se.name.is_some() {
              crate::types::Item::List(Box::new(vec![se.elemtype.fit(), se.name.unwrap().fit()].into()))
          } else {
      ...
      pub fn get(&self, key: &assoc::KeyItem) -> Option<Item> {
          match key {
              assoc::KeyItem::Word(w) => match w.data.as_str() {
                  ...
                  "modules" => Some(crate::types::Item::List(Box::new(self.modules.iter().map(|m| crate::types::Item::Bytes(Box::new(m.as_ref().clone()))).collect()))),
  ...
  impl Dict for Words {
      ...
      fn builtins() -> Self {
          ...
          for (bw, bd) in BUILTIN_FUNCTIONS.iter() {
              let ns = core_nss.get(bw.data.as_str()).unwrap_or_else(|| panic!("Missing namespace for builtin: {}", bw.data.as_str()));
              let entry = Entry {
                  definition: bd.clone(),
                  examples: None,
                  spec: None,
  ...
  impl IntoIterator for Dictionary {
      ...
      fn into_iter(self) -> Self::IntoIter {
          let v: Vec<(assoc::KeyItem, Item)> = vec![
              ...
              (
                  fit!("modules"),
                  crate::types::Item::List(Box::new(self.modules.iter().map(|m| crate::types::Item::Bytes(Box::new(m.as_ref().clone()))).collect())),
              ),
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  impl Environment {
      ...
      pub fn check_input_spec(&self, specs: &dict::StackSpec) -> Result<(), Error> {
          fn all_ok(res: &[(&dict::SpecElement, &Item, bool)]) -> bool {
  ...
  impl Default for Environment {
      ...
      fn default() -> Self {
          static INST: Lazy<Environment> = Lazy::new(|| {
              ...
              env = [
                  // "collections", // actually we probably want collections in default dictionary? wait maybe not
              ]
              .into_iter()
...
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  ...
  where
      I::Item: Fit<(crate::types::container::associative::KeyItem, crate::types::Item)>,
  {
      fn derive_iter(iter: I) -> Self {
          iter.into_iter().map(|x| x.fit()).collect()
      }
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  impl From<&Item> for edn_format::Value {
      fn from(item: &Item) -> Self {
          fn from_assoc(a: assoc::Associative) -> edn_format::Value {
              let mut av = a.clone().into_iter().collect::<Vec<(assoc::KeyItem, Item)>>();
  ...
  impl TryDerive<Result<edn_format::Value, crate::types::container::error::Error>> for Item {
      fn try_derive(r: Result<edn_format::Value, crate::types::container::error::Error>) -> Result<Item, Error> {
          match r {
  ...
  pub fn parse(s: String) -> Result<coll::List, Error> {
      ...
      parser.into_iter().map(|x| x.unwrap().try_fit()).collect::<Result<crate::types::container::List, _>>()
  }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  use crate::types::container::program::{self, Program, ProgramNode};
  use crate::types::container::{
  ...
  pub fn eval_step(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      //println!("Program: {:?}", env.program);

      while match env.program.advance_control_flow(&mut env.stack) {
              ...
              Item::Word(word) => {
                  if word.quoted {
                      ...
                      if let Some(dfn) = definition {
                          ...
                          //let w = word.clone();
                          env.push_err(Error::undefined((*word).fit()));
                          env.fit()
                      }
                  }
              }
  ...
  pub fn f_recur(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      let rec2_node = Arc::new(ProgramNode::Seq(
          coll::List::try_derive(combinator).unwrap_or_default(),
      ));
      let rec1_node = Arc::new(ProgramNode::Seq(
          coll::List::try_derive(false_branch).unwrap_or_default(),
      ));
      let then_node = Arc::new(ProgramNode::Seq(
          coll::List::try_derive(true_branch).unwrap_or_default(),
      ));
      let pred_node = Arc::new(ProgramNode::Seq(
          coll::List::try_derive(pred).unwrap_or_default(),
      ));

      let cycle_node = Arc::new_cyclic(|weak| ProgramNode::Recur {
          pred: pred_node,
          then: then_node,
          rec1: rec1_node,
          rec2: rec2_node,
...
#+begin_src rust :tangle src/lib.rs :mkdirp yes
          ...
          unsafe {
              ...
              match serialize::parse_input(&mut env_val, program) {
                  Ok(_) => {
                      ...
                      let result =
                          serialize::auto_format(env_val.stack.to_list().iter().emit().as_str(), 10, 40);
  ...
  mod tests {
      ...
      use crate::derivation::{TryDerive, TryFit, Fit};
