...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
      ...
      pub fn remaining(&mut self) -> cont::List {
          match &*self.node {
              ...
              ProgramNode::Recur { pred, then, rec1, rec2, .. } => {
                  ...
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec2.clone(), next_idx: 0 }]))));
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: rec1.clone(), next_idx: 0 }]))));
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: then.clone(), next_idx: 0 }]))));
                  l.push_back(Item::Program(Box::new(Program(vec![Frame { node: pred.clone(), next_idx: 0 }]))));
                  l.push_back(crate::fit!("🪆"));
                  l
              }
          }
      }
  ...
  impl Program {
      ...
      pub fn advance_control_flow(&mut self, stack: &mut crate::types::container::stack::Stack) -> bool {
          self.clean();
          let frame_node;
          let phase;
          {
              let Some(frame) = self.0.last_mut() else { return false; };
              match &*frame.node {
                  ProgramNode::Recur { .. } => {
          ...
          match &*frame_node {
              ProgramNode::Recur { pred, then, rec1, rec2, self_ref } => {
                  if phase == 0 {
                      ...
                      let b = stack.pop_front().expect("stack spec guarantees presence");
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn evert(mut env: Environment) -> Sometime<'static, Environment> {
      let mut l = coll::List::try_derive(env.pop()).expect("stack spec guarantees List");
  ...
  pub fn eval_step(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      while env.program.advance_control_flow(&mut env.stack) {}
  ...
  pub fn f_recur(mut env: Environment) -> Sometime<'static, Environment> {
      ...
      let cycle_node = Arc::new_cyclic(|weak| {
          ProgramNode::Recur {
              pred: pred_node,
              then: then_node,
              rec1: rec1_node,
