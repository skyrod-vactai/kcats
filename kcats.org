...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  impl Derive<String> for Item {
      fn derive(i: String) -> Self {
          Item::Sized(coll::Sized::String(i))
      }
  }

  impl Derive<Bytes> for Item {
      fn derive(b: Bytes) -> Self {
          Item::Sized(coll::Sized::Bytes(b))
      }
  }
  ...
  mod serde {
      ...
      impl<'de> Visitor<'de> for ItemVisitor {
          ...
          where
              E: de::Error,
          {
              Ok(Item::Sized(coll::Sized::String(v.to_string())))
          }
          ...
          where
              E: de::Error,
          {
              Ok(Item::Sized(coll::Sized::Bytes(v)))
          }
              ...
              A: de::MapAccess<'de>,
          {
              ...
              Ok(Item::Sized(coll::Sized::Associative(
                  assoc::Associative::Assoc(map.fit()),
              )))
          }
      ...
      impl Serialize for Item {
              ...
              S: Serializer,
          {
              match self {
                  ...
                  Item::Sized(ref sized) => sized.serialize(serializer),
  ...
  impl TryDerive<Item> for Number {
      ...
      fn try_derive(i: Item) -> Result<Self, Self::Error> {
          ...
          match i {
              ...
              Item::Sized(cont::Sized::String(s)) => fromstr(s),
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl Derive<List> for Item {
      fn derive(l: List) -> Item {
          Item::Sized(coll::Sized::List(Box::new(l)))
      }
  }
  ...
  impl Container<Item> for Sized {
      fn has(&self, other: &Item) -> bool {
          ...
          match (self, other) {
              ...
              (Sized::Set(s), Item::Sized(Sized::Set(other))) => other.is_subset(s),
              (Sized::Set(s), other) => {
  ...
  impl TryDerive<Item> for List {
      ...
      fn try_derive(i: Item) -> Result<Self, Self::Error> {
          match i {
              Item::Sized(s) => List::try_derive(s),
              i => Err(Error::expected(fit!("list"), i)),
          }
      }
  }
  ...
  impl TryDerive<Item> for Receptacle {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Sized(s) => Ok(Receptacle::Sized(s)),
  ...
  impl Derive<Set> for Item {
      fn derive(l: Set) -> Self {
          Item::Sized(Sized::Set(l))
      }
  }

  impl Derive<Dispenser> for Item {
      fn derive(c: Dispenser) -> Self {
          convert_enum!(c, Dispenser, Item, [Sized, Out, Tunnel])
      }
  }

  impl Derive<Receptacle> for Item {
      fn derive(c: Receptacle) -> Self {
          convert_enum!(c, Receptacle, Item, [Sized, In, Tunnel])
      }
  }
  ...
  impl TryDerive<Item> for Dispenser {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              Item::Sized(s) => Ok(Dispenser::Sized(s)),
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Error {
      ...
      fn try_derive(i: Item) -> Result<Self, Self::Error> {
          match i {
              Item::Sized(coll::Sized::Associative(assoc::Associative::Error(e))) => Ok(e),
              Item::Sized(coll::Sized::String(_)) | Item::Sized(coll::Sized::Bytes(_)) => {
                  Err(Error::expected(fit!("error"), Item::default()))
              }
              Item::Sized(c) => c.into_iter().try_fit(),
...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Namespace {
      ...
      fn try_derive(i: Item) -> Result<Namespace, Error> {
          match i {
              Item::Sized(coll::Sized::Bytes(b)) => Ok(bytes_to_ns(b)),
              i => {
                  let s = coll::Sized::try_derive(i)?;
                  if s.is_empty() {
                      Ok(Default::default())
  ...
  impl Derive<Entry> for Item {
      fn derive(e: Entry) -> Self {
          Item::Sized(coll::Sized::Associative(assoc::Associative::DictEntry(
              Box::new(e),
          )))
      }
  }
  ...
  impl Derive<Words> for Item {
      fn derive(e: Words) -> Self {
          Item::Sized(coll::Sized::Associative(assoc::Associative::Words(e)))
      }
  }

  impl Derive<Dictionary> for Item {
      fn derive(d: Dictionary) -> Self {
          Item::Sized(coll::Sized::Associative(assoc::Associative::Dictionary(
              Box::new(d),
          )))
      }
  }
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  fn check_type(i: &Item, w: &Word) -> bool {
      ...
      match (w, i) {
          ...
          (w, Item::Out(_) | Item::Tunnel(_) | Item::Sized(_)) if *w == *S_DISPENSER => true,
          (w, Item::Tunnel(_) | Item::Sized(_)) if *w == *S_RECEPTACLE => true,
          ...
          (w, Item::Sized(coll::Sized::Bytes(_))) if *w == *S_BYTES || *w == *S_ORDERED => true,

          (w, Item::Sized(coll::Sized::String(_))) if *w == *S_STRING => true,
          (w, Item::Word(_)) if *w == *S_WORD => true,

          (w, Item::Out(_) | Item::Tunnel(_) | Item::In(_)) if *w == *S_PIPE => true,

          (w, Item::Sized(coll::Sized::List(_)) | Item::Sized(coll::Sized::Program(_)))
              if *w == *S_LIST || *w == *S_PROGRAM =>
          {
              true
          }

          (w, Item::Sized(coll::Sized::Associative(_))) if *w == *S_ASSOC => true,

          (w, Item::Sized(coll::Sized::Associative(assoc::Associative::Error(_))))
              if *w == *S_ERROR =>
          {
              true
          }

          (w, Item::Sized(coll::Sized::Associative(assoc::Associative::Dictionary(_))))
              if *w == *S_DICTIONARY =>
          {
              true
          }

          (w, Item::Sized(_)) if *w == *S_SIZED || *w == *S_ORDERED => true,

          (w, Item::Sized(coll::Sized::Associative(assoc::Associative::Env(_))))
              if *w == *S_ENVIRONMENT =>
          {
              true
          }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn is_string(i: Item) -> bool {
      matches!(i, Item::Sized(coll::Sized::String(_)))
  }

  pub fn is_bytes(i: Item) -> bool {
      matches!(i, Item::Sized(coll::Sized::Bytes(_)))
  }

  pub fn is_error(i: Item) -> bool {
      matches!(
          i,
          Item::Sized(coll::Sized::Associative(assoc::Associative::Error(_),))
      )
  }
  ...
  pub fn is_environment(i: Item) -> bool {
      matches!(
          i,
          Item::Sized(coll::Sized::Associative(assoc::Associative::Env(_)))
      )
  }
  ...
  fn assoc_in(i: Option<Item>, ks: &[assoc::KeyItem], v: Item) -> Result<Item, Error> {
      ...
      if let [k, ks @ ..] = ks {
          match (i, k) {
              ...
              (Some(Item::Sized(coll::Sized::List(l))), assoc::KeyItem::Int(k)) => {
                  assoc_vec(*l, ks, *k, v)
              }

              // An int key for an associative means an integer key, which is uncommon
              // but we'll support it
              (Some(Item::Sized(coll::Sized::Associative(a))), assoc::KeyItem::Int(k)) => {
  ...
  fn unassoc_in(i: Item, ks: &[assoc::KeyItem]) -> Result<Item, Error> {
      if let [k, ks @ ..] = ks {
          if ks.is_empty() {
              ...
              match (i, k) {
                  (Item::Sized(coll::Sized::List(mut l)), assoc::KeyItem::Int(k)) => {
              ...
              match r {
                  Ok(s) => match s {
                      ...
                      coll::Sized::List(l) => Bytes::try_derive((*l).clone()).unwrap_or_else(|_| {
                          (&Item::Sized(coll::Sized::List(l)))
                              .emit()
                              .as_bytes()
                              .to_vec()
                      }),
                      s => (&Item::Sized(s)).emit().as_bytes().to_vec(),
                  },
  ...
  fn get_error(env: &Environment) -> Option<Error> {
      env.tos().and_then(|i| match i {
          Item::Sized(coll::Sized::Associative(assoc::Associative::Error(e))) => Some(e.clone()),
          _ => None,
      })
  }
  ...
  fn unwind(mut env: Environment) -> Environment {
      ...
      let err = match err {
          Item::Sized(coll::Sized::Associative(assoc::Associative::Error(mut e))) => {
  ...
  fn handle(mut env: Environment) -> Future<Environment> {
      ...
      match env.stack.front_mut() {
          None => {}
          Some(i) => match i {
              Item::Sized(coll::Sized::Associative(assoc::Associative::Error(ref mut e))) => {
                  e.is_handled = true;
              }
              _ => {}
          },
  ...
  pub fn xor(i: Item, j: Item) -> ItemResult {
      match (i, j) {
          ...
          (Item::Sized(coll::Sized::Bytes(i)), Item::Sized(coll::Sized::Bytes(j))) => {
              Ok(xor_(i, j).fit())
          }
...
#+begin_src rust :tangle src/types/container/pipe/db.rs :mkdirp yes
  ...
  impl Db {
      ...
      fn insert_attribute(&self, id: uuid::Uuid, attribute: Item, value: Item) -> axiom::ItemResult {
          ...
          match value {
              Item::Sized(coll::Sized::Associative(a)) => {
                  let sub_id = uuid::Uuid::new_v4();
                  self.insert_item(a.fit(), sub_id)?;
                  self.conn.execute(q, params![id, attribute, sub_id])?;
              }
              Item::Sized(coll::Sized::Set(s)) => self.insert_iter(
                  Some((id, attribute)),
                  s.iter().map(|i| Item::derive(i.clone())),
              )?,
              Item::Sized(coll::Sized::List(l)) => self.insert_iter(Some((id, attribute)), *l)?,
              i => {
  ...
  pub fn is_value(i: &Item) -> bool {
      matches!(
          i,
          Item::Sized(coll::Sized::String(_))
              | Item::Sized(coll::Sized::Bytes(_))
              | Item::Int(_)
              | Item::Float(_)
              | Item::Word(_)
              | Item::Char(_)
  ...
  impl TryDerive<ValueRef<'_>> for Item {
      ...
      fn try_derive(value: ValueRef) -> Result<Self, Self::Error> {
          match value {
              ...
              ValueRef::Blob(b) => Ok(Item::Sized(coll::Sized::Bytes(b.to_vec()))),
  ...
  fn decode_string(s: String) -> axiom::ItemResult {
      if let Some(w) = s.strip_prefix("w|") {
          ...
          Ok(Item::Sized(coll::Sized::String(s.to_string())))
  ...
  impl rusqlite::ToSql for Item {
      fn to_sql(&self) -> Result<ToSqlOutput<'_>, DBError> {
          match self {
              ...
              Item::Sized(coll::Sized::String(s)) => Ok(EncodeAs::String(s.clone()).encode()),
              Item::Sized(coll::Sized::Bytes(b)) => b.to_sql(),
