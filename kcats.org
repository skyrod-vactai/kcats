  ...
  impl Sized {
      ...
      pub fn pop(self) -> (Self, Option<Item>) {
          match self {
              Sized::Assoc(mut a) => {
                  let v = a.take_simple();
                  (Sized::Assoc(a), v.map(Item::derive))
              }
              Sized::Words(mut a) => {
                  let v = a.take_simple();
                  (Sized::Words(a), v.map(Item::derive))
              }

              // The remaining Associative impls may require auto-demotion (eg,
              ...
              Sized::Dictionary(mut a) => {
                  let mut assoc: assoc::Association = assoc::Associative::Dictionary(a).fit();
                  let v = assoc.take_simple().map(Item::derive);
                  (assoc::Associative::Assoc(assoc), v);
              }
              Sized::Error(mut a) => {
                  let mut assoc: assoc::Association = assoc::Associative::Error(a).fit();
                  let v = assoc.take_simple().map(Item::derive);
                  (assoc::Associative::Assoc(assoc), v);
              }
              Sized::DictEntry(mut a) => {
                  let mut assoc: assoc::Association = assoc::Associative::DictEntry(a).fit();
                  let v = assoc.take_simple().map(Item::derive);
                  (assoc::Associative::Assoc(assoc), v);
              }
              Sized::Env(mut a) => {
                  let mut assoc: assoc::Association = assoc::Associative::Env(a).fit();
                  let v = assoc.take_simple().map(Item::derive);
                  (assoc::Associative::Assoc(assoc), v);
              }
              ...
              Sized::Nothing => (Sized::Nothing, None),
          }
      }
