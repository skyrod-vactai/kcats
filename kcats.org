...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  pub trait RewrapExt: IntoIterator + Clone + Sized {
      ...
      fn try_rewrap<Target, M>(self) -> Result<Target, <M as Mode<Self>>::Err>
      where
          M: Mode<Self>
              + Mode<Self::Item>
              + ModeErr<types::Word, Self, <M as Mode<Self>>::Err>
              + ModeNest<<M as Mode<Self>>::Err, <M as Mode<Self::Item>>::Err>,
          Self::Item: Clone + TryFit<<Target as IntoIterator>::Item, M>,
          Target: FromIterator<<Target as IntoIterator>::Item> + IntoIterator,
      {
          ...
          self.to_iter()
              ...
              .map_err(|inner_err| {
                  let outer = <M as ModeErr<_, _, _>>::expected(fit!("container"), original);
...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  pub trait DetailedMode<Src>: Mode<Src, Err = Error> + ModeErr<Word, Src, Error> {}
  impl<Src, T: Mode<Src, Err = Error> + ModeErr<Word, Src, Error>> DetailedMode<Src> for T {}
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
      ...
      fn into_iter(self) -> Self::IntoIter {
          match self {
              ...
              s => {
                  ...
                  let map = assoc::Associative::derive(s).unwrap();
  ...
  impl<M: Mode> TryDerive<Item, M> for Sized {
      fn try_derive(item: Item) -> Result<Self, M::Detail<Item>> {
          match item {
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  impl<M> TryDerive<Item, M> for Associative
  where
      M: Mode<Item> + ModeErr<Word, Item, <M as Mode<Item>>::Err>,
  {
      fn try_derive(i: Item) -> Result<Associative, <M as Mode<Item>>::Err> {
          match i {
              ...
              Item::List(l) => l
                  .try_rewrap::<AssociationContent, M>()
                  .map(Arc::new)
  ...
  impl<M: Mode> TryDerive<coll::Sized, M> for Associative {
      fn try_derive(i: coll::Sized) -> Result<Associative, M::Detail<coll::Sized>> {
          match i {
              ...
              coll::Sized::List(l) => {
                  <Associative as TryDerive<_, M>>::try_derive(l)
  ...
  impl<M: Mode> TryDerive<coll::List, M> for Associative {
      fn try_derive(l: coll::List) -> Result<Associative, M::Detail<coll::List>> {
          l.iter()
              .cloned()
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  use crate::derivation::{self as derive, *};
  use crate::serialize::Display;
  use crate::traits::*;
  ...
  impl<Source> derive::ModeErr<Word, Source, Error> for derive::Detailed
  where
      Source: Fit<Item>,
  {
      fn expected(expected: Word, src: Source) -> Error {
          Error::expected(expected, src)
      }
  }

  impl<Outer, Inner> derive::ModeNest<Outer, Inner> for derive::Detailed
  where
      Outer: Nested<Inner>,
      Inner: Fit<Error>,
  {
      fn nest_error(outer: Outer, inner: Inner) -> Outer {
          outer.nest(inner)
      }
  }

  impl<Src> derive::Mode<Src> for Detailed {
      type Err = Error;
  }

  impl<Src> derive::Mode<Src> for Fast {
      type Err = Src;
  }
...
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  ...
  ///Trait to specify how one type is converted to another, specifically
  /// how failures are dealt with.
  pub trait ModeErr<PredName, Source, Err>: Sized + 'static {
      fn expected(expected: PredName, src: Source) -> Err;
  }

  pub trait ModeNest<Outer, Inner>: Sized + 'static {
      fn nest_error(outer: Outer, inner: Inner) -> Outer;
  }

  pub trait Mode<Src>: Sized + 'static {
      type Err;
  }
  ...
  impl<PredName, Source> ModeErr<PredName, Source, Source> for Fast {
      #[inline(always)]
      fn expected(_expected: PredName, src: Source) -> Source {
          src
      }
  }

  impl<Outer, Inner> ModeNest<Outer, Inner> for Fast {
      #[inline(always)]
      fn nest_error(outer: Outer, inner: Inner) -> Outer {
  ...
  /// If derivation can't fail we have placeholder impl that doesn't do
  /// anything, because it'll never be asked to.
  impl<P, S> ModeErr<P, S, Infallible> for Infallible {
      fn expected(_expected: P, _src: S) -> Infallible {
          unreachable!("supposed to be infallible")
      }
  }

  impl<I> ModeNest<Infallible, I> for Infallible {
      fn nest_error(_o: Infallible, _i: I) -> Infallible {
          unreachable!("supposed to be infallible")
      }
  }
  ...
  pub trait TryDerive<Src, M: Mode<Src> = Detailed>: Sized {
      fn try_derive(value: Src) -> Result<Self, <M as Mode<Src>>::Err>;
  }
  ...
  pub trait TryFit<Target, M: Mode<Self> = Detailed>: Sized {
      fn try_fit(self) -> Result<Target, <M as Mode<Self>>::Err>;
  }
  ...
  impl<Src, Target, M: Mode<Src>> TryFit<Target, M> for Src
  where
      Target: TryDerive<Src, M>,
  {
      fn try_fit(self) -> Result<Target, <M as Mode<Src>>::Err> {
  ...
  impl<T, M: Mode<Self>> TryDerive<T, M> for T
  where
      T: Identity,
  {
      fn try_derive(value: T) -> Result<T, <M as Mode<Self>>::Err> {
          Ok(value)
      }
  }
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  impl TryDerive<edn_format::Value, Detailed> for Item {
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  /// stack item.
  fn f_stack1<F, Arg1, Output, ArgMode: Mode<Item>, Err>(
      f: F,
  ) -> impl Fn(Environment) -> Sometime<'static, Environment>
  where
      F: Fn(Arg1) -> Result<Output, Err> + 'static,
      Arg1: TryDerive<Item, ArgMode> + Fit<Item> + Clone,
  ...
  lazy_static! {
      ...
      let mut entries: Vec<(&str, &'static StepFn)> = vec![
          ...
          ("sort-indexed", Box::leak(Box::new(f_stack1::<_, _, _, Detailed, _>(sort_by_key)))),
          ...
          (
              "association",
              Box::leak(Box::new(f_stack1::<_, Item,  assoc::Associative, Detailed, Error>(
          ...
          (
              "character",
              Box::leak(Box::new(f_stack1::<_, Item, Char, _, _>(Char::try_derive))),
          ),
         ...
         // ("decide", Box::leak(Box::new(decide))),
          ...
          (
              "environment",
              Box::leak(Box::new(f_stack1::<_, Item, Environment, _, _>(Environment::try_derive))),
          ),
          ...
          ("number", Box::leak(Box::new(f_stack1::<_, Item, Number, _, _>(
              Number::try_derive,
          )))),
