...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  pub trait RewrapExt: IntoIterator + Clone + Sized {
      ...
      fn try_rewrap<Target, M: Mode>(self) -> Result<Target, M::Err<Self>>
      where
          M: Mode,
          Self: Fit<types::Item>,
          Self::Item: Clone + TryFit<<Target as IntoIterator>::Item>,
          Target: FromIterator<<Target as IntoIterator>::Item> + IntoIterator,
      {
          let original = self.clone();
          self.into_iter()
              .map(|x| x.try_fit_with::<M>())
              .collect::<Result<Target, _>>()
              .map_err(|inner_err| {
                  let outer = M::expected(fit!("container"), original);
                  M::nest_error(outer, inner_err)
              })
      }
  }

  impl<I: IntoIterator + Clone> RewrapExt for I {}
#+end_src
*** Internal data types
...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  impl TryDerive<String> for Word {
      fn try_derive_with<M: Mode>(s: String) -> Result<Self, M::Err<String>> {
          match serialize::parse::<M>(s.clone()) {
              Ok(mut l) => {
                  if l.len() == 1 {
                      match l.pop_front().unwrap() {
                          Item::Word(w) => Ok(w),
                          _ => Err(M::from(s, || {
                              Error::parse("Words cannot contain delimiters")
                          })),
                      }
                  } else if l.is_empty() {
                      Err(M::from(s, || Error::parse("Words cannot be empty")))
                  } else {
                      Err(M::from(s, || {
                          Error::parse("Words cannot contain whitespace")
                      }))
                  }
              }
              Err(e) => Err(M::from(s, e)),
          }
      }
  }
  ...
  impl TryDerive<&str> for Word {
      fn try_derive_with<M: Mode>(s: &str) -> Result<Self, M::Err<&str>> {
          Word::try_derive_with::<M>(s.to_string())
      }
  }
  ...
  impl fmt::Debug for Item {
      fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
          match self {
              ...
              Item::Standard => write!(f, "{:?}", Item::Standard),
              Item::Time => write!(f, "{:?}", Item::Time),
              Item::Word(w) => {
                  if w.quoted {
                      write!(f, "[{}]", Item::Word(w.clone()).emit())
                  } else {
                      write!(f, "{}", Item::Word(w.clone()).emit())
                  }
              }
  ...
  impl TryDerive<Item> for String {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          match i {
              Item::Word(w) => Ok(w.data.as_str().to_string().fit()),
              i => {
                  let s = coll::Sized::try_derive_with::<M>(i)?;
                  match s {
                      coll::Sized::String(i) => Ok(i),
                      i => Err(M::expected(fit!("string"), Item::derive(i))),
                  }
              }
          }
      }
  ...
  impl TryDerive<Item> for Word {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          match i {
              Item::Word(i) => Ok(i),
              i => {
                  let s = coll::Sized::try_derive_with::<M>(i.clone())?;
                  match s {
                      coll::Sized::String(s) => s.try_fit_with::<M>().swap_original(|_| i),
                      s => {
                          let l = coll::List::try_derive(s);
                          match l {
                              //TODO i think this was meant to support
                              // quoted words or strings as words?
                              Ok(mut l) => {
                                  if l.len() == 1 {
      ...
      fn try_derive(i: Item) -> Result<Self, Error> {
          match i {
              ...
              Item::Int(i) => match char::from_u32(i as u32) {
                  Some(c) => Ok(c),
                  None => Err(Error::expected(fit!("character-encoding"), i)),
              },
      ...
      impl Serialize for Item {
              ...
              S: Serializer,
          {
              ...
              match self {
                  ...
                  Item::Assoc(a) => ser_map(serializer, a.clone().to_iter()),
                  ...
                  Item::Words(w) => ser_map(serializer, w.clone().to_iter()),
                  Item::Nothing => {
...
#+RESULTS:
: error: Could not compile `cargoJVHPrj`.
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl Join<List> for assoc::Associative {
      ...
      fn join(self, other: List) -> Result<Self::Output, <Self as Join<List>>::Error> {
          ...
          let la = assoc::Associative::Assoc(Arc::new(
              other.try_rewrap::<assoc::AssociationContent, Detailed>()?,
          ));
  ...
  impl Join<assoc::Associative> for List {
      ...
      fn join(self, other: assoc::Associative) -> Result<Self::Output, Self::Error> {
          ...
          let sa = assoc::Associative::Assoc(Arc::new(
              self.try_rewrap::<assoc::AssociationContent, Detailed>()?,
          ));
  ...
  impl Receptacle {
      ...
      pub fn put(self, i: Item) -> Sometime<'static, Result<Receptacle, Error>> {
          match self {
              Receptacle::Sized(s) => Sometime::Now(s.put(i).map(|s| Receptacle::Sized(s))),
              Receptacle::In(p) => p.put(i).map(|r| r.map(|i| Receptacle::In(i))),
              Receptacle::Tunnel(t) => t.put(i).map(|r| r.map(|t| Receptacle::Tunnel(t))),
          }
      }
  }
      ...
      fn into_iter(self) -> Self::IntoIter {
          match self {
              ...
              s => {
                  ...
                  Box::new(map.to_iter().map(|kv| kv.fit()))
              }
          }
      }
  }
  ...
  impl TryDerive<Dispenser> for Sized {
      fn try_derive_with<M: Mode>(c: Dispenser) -> Result<Self, M::Err<Dispenser>> {
          //println!("from iterable {:?}", c);
          match c {
              Dispenser::Sized(s) => Ok(s),
              i => Err(M::expected(fit!("sized"), i)),
          }
      }
  }

  impl TryDerive<Receptacle> for Sized {
      fn try_derive_with<M: Mode>(c: Receptacle) -> Result<Self, M::Err<Receptacle>> {
          match c {
  ...
  impl TryDerive<Sized> for List {
      fn try_derive_with<M: Mode>(s: Sized) -> Result<Self, M::Err<Sized>> {
          match s {
              Sized::List(l) => Ok(*l),
  ...
  impl TryDerive<List> for Vec<dict::Namespace> {
      fn try_derive_with<M: Mode>(l: List) -> Result<Self, M::Err<List>> {
  ...
  impl TryDerive<List> for Bytes {
      fn try_derive_with<M: Mode>(l: List) -> Result<Self, M::Err<List>> {
          let v = l
              .iter()
              .cloned()
              .map(|i| number::Int::try_derive_with::<M>(i).map(|i| i as u8))
              .collect::<Result<Bytes, M::Err<List>>>()?;
          Ok(v)
      }
  }
  ...
  impl TryDerive<Item> for List {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          let s = Sized::try_derive_with::<M>(i)?;
          List::try_derive_with::<M>(s).map_err(|e| M::revert(e, Fit::fit))
      }
  }

  impl TryDerive<Item> for Sized {
      fn try_derive_with<M: Mode>(item: Item) -> Result<Self, M::Err<Item>> {
          match item {
  ...
  impl TryDerive<Item> for Receptacle {
      fn try_derive_with<M: Mode>(item: Item) -> Result<Self, M::Err<Item>> {
          match item {
              ...
              i => Err(M::expected(fit!("receptacle"), i)),
          }
      }
  }
  ...
  impl TryDerive<Receptacle> for Dispenser {
      fn try_derive_with<M: Mode>(c: Receptacle) -> Result<Self, M::Err<Receptacle>> {
          match c {
              ...
              i => Err(M::expected(fit!("iterable"), i)),
          }
      }
  }
  ...
  impl TryDerive<Item> for Box<dyn Iterator<Item = Item>> {
      fn try_derive_with<M: Mode>(item: Item) -> Result<Self, M::Err<Item>> {
          Ok(Sized::try_derive_with::<M>(item)?.into_iter())
      }
  }
  ...
  impl TryDerive<Item> for Set {
      fn try_derive_with<M: Mode>(item: Item) -> Result<Self, M::Err<Item>> {
          let s = Sized::try_derive_with::<M>(item)?;
          s.try_rewrap::<HashSet<assoc::KeyItem>, M>()
              .map(Arc::new)
              .map_err(|e| M::revert(e, Item::derive))
      }
  }
      ...
      impl Serialize for Sized {
              ...
              S: serde::Serializer,
          {
              match self {
                  ...
                  s => {
                      let a =
                          <assoc::Associative as TryDerive<_, Fast>>::try_derive(s.clone()).unwrap();
                      // Start serializing a map
                      let assoc = assoc::Association::derive(a);
...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  impl Frame {
      ...
      pub fn remaining(&mut self) -> cont::List {
  ...
  impl Program {
      ...
      pub fn last(&self) -> Option<&Item> {
          self.0.first().and_then(|f| f.snippet.last())
      }
  }
  ...
  mod tests {
      ...
      fn test_simple() {
          ...
          assert_eq!(p.pop().cloned(), Some(Item::Word(fit!("foo"))));
          assert_eq!(p.peek().cloned(), Some(Item::Word(fit!("bar"))));
      }
      ...
      fn test_layers() {
          ...
          let q = p.clone();
          assert_eq!(p.count(), 3);
          assert_eq!(p.pop().cloned(), Some(Item::Word(fit!("stoon"))));
          assert_eq!(p.count(), 2);
          assert_eq!(p.peek().cloned(), Some(Item::Word(fit!("bar"))));
          assert_eq!(p.pop().cloned(), Some(Item::Word(fit!("bar"))));
          assert_eq!(p.0.len(), 1);

          // test conversion to List
          assert_eq!(cont::List::derive(q), list!["stoon", "bar", "baz"])
      }
  }
#+end_src
**** Associative types
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
      ...
      fn try_derive(i: Item) -> Result<Self, Error> {
          match i {
              ...
              i => match coll::Sized::try_derive(i)? {
                  ...
                  coll::Sized::List(l) => Ok(KeyItem::List(l.iter().cloned().try_rewrap()?)),
                  s => {
  ...
  impl coll::Join<coll::List> for Association {
      ...
      fn join(self, other: coll::List) -> Result<Self::Output, Self::Error> {
          let la = Arc::new(other.iter().cloned().try_rewrap()?);
  ...
  impl coll::Join<Association> for coll::List {
      ...
      fn join(self, other: Association) -> Result<Self::Output, Self::Error> {
          let la: Association = Arc::new(self.iter().cloned().try_rewrap()?);
  ...
  impl TryDerive<Item> for Associative {
      fn try_derive_with<M: Mode>(i: Item, mode: &M) -> Result<Associative, M::Err<Item>> {
          match i {
              ...
              Item::List(l) => l
                  .try_rewrap::<AssociationContent, M>(mode)
                  .map(Arc::new)
                  .map(Associative::Assoc)
                  .map_err(|e| mode.map_err(e, |s| Item::List(Box::new(s)))),
              i => Err(M::expected(fit!("associative"), i)),
          }
      }
  }

  impl TryDerive<coll::Sized> for Associative {
      fn try_derive_with<M: Mode>(
          i: coll::Sized,
          mode: &M,
      ) -> Result<Associative, M::Err<coll::Sized>> {
          match i {
              ...
              coll::Sized::List(l) => {
                  Associative::try_derive_with(*l, mode)
                      .map_err(|e| mode.map_err(e, |s| coll::Sized::List(Box::new(s))))
  ...
  impl TryDerive<coll::List> for Associative {
      fn try_derive_with<M: Mode>(
          l: coll::List,
          mode: &M,
      ) -> Result<Associative, M::Err<coll::List>> {
          l.try_rewrap::<AssociationContent, M>(mode)
              .map(Arc::new)
              .map(Associative::Assoc)
      }
  }
  ...
  impl TryDerive<Item> for Entry {
      fn try_derive_with<M: Mode>(i: Item, mode: &M) -> Result<Entry, Error> {
          let s = coll::Sized::try_derive(i)?;
          if s.count() != 2 {
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  pub trait Nested<T> {
      fn nest(self, child: T) -> Self;
  }

  impl<T, E> Nested<Result<T, E>> for Result<T, E>
  where
      E: Nested<E>,
  {
      fn nest(self, child: Result<T, E>) -> Self {
          match (self, child) {
              (Err(s), Err(child)) => Err(s.nest(child)),
              (Ok(s), _) => Ok(s),
              (Err(s), Ok(_)) => Err(s),
          }
      }
  ...
  impl<E> Nested<E> for Error
  where
      E: Fit<Error>,
  {
      fn nest(mut self, other: E) -> Self {
          self.cause = Some(Box::new(other.fit()));
          self
      }
  }
      ...
      pub fn create<T: Fit<Item>>(asked: coll::List, reason: &str, actual: Option<T>) -> Error {
          ...
          Error {
              ...
              data: std::sync::Arc::new(
                  data.rewrap::<assoc::AssociationContent, assoc::Entry>()
                      .fit(),
              ),
              cause: None,
          }
  ...
  impl Dict for Words {
      ...
      fn insert_core_module(&mut self, lexicon: String) -> Result<Namespace, Error> {
          ...
          let items = serialize::parse(lexicon)?
              ...
              .and_then(|i| coll::List::try_derive(i))?;
      ...
      fn builtins() -> Self {
          ...
          for (bw, bd) in BUILTIN_FUNCTIONS.iter() {
              let ns = core_nss.get(bw.data.as_str()).unwrap();
              let entry = Entry {
                  definition: bd.clone(),
                  examples: None,
                  spec: None,
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  impl Environment {
      ...
      pub fn check_input_spec(&self, specs: &dict::StackSpec) -> Result<(), Error> {
          fn all_ok(res: &Vec<(&dict::SpecElement, &Item, bool)>) -> bool {
              res.iter().map(|(_, _, is_pass)| is_pass).all(move |x| *x)
          }
          ...
          let res: Vec<_> = specs
              ...
              .map(|(i, spec)| {
                  ...
                  (spec, item, check_type(item, &spec.elemtype))
              })
              .collect();
          ...
          if all_ok(&res) {
              ...
              //create an item to use as 'actual' in the error
              let actual: Vec<Item> = res
                  .into_iter()
                  .map(|(spec, item, pass)| {
      ...
      pub fn load_module(mut self, module: Item, is_using: bool) -> Environment {
          ...
          //println!("Env: {:?}", new_env);
          let mut dict: dict::Dictionary = self.pop().try_fit().unwrap();
          dict.resolve();
  ...
  impl Default for Environment {
      ...
      fn default() -> Self {
          static INST: Lazy<Environment> = Lazy::new(|| {
              ...
              .into_iter()
              .fold(env, |env, module| env.load_module(module, true));
              // println!(
...
#+begin_src rust :tangle src/crypto.rs :mkdirp yes
  ...
  impl TryDerive<Item> for signing::Keypair {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          let sk: signing::SecretKey = i.try_fit_with::<M>()?;
  ...
  impl TryDerive<Item> for signing::SecretKey {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          let a = assoc::Associative::try_derive_with::<M>(i)?;
          if a.get(&fit!("type")) == Some(fit!("elliptic-curve-key")) {
              let sk = signing::SecretKey::from_bytes(
                  &Bytes::try_derive(a.get(&fit!("secret")).ok_or_else(
                      || M::expected(fit!("secret"), a), //Error::expected(fit!("secret"), None::<Item>)
                  )?)?[..],
              )?;
              Ok(sk)
          } else {
              Err(M::expected(fit!("elliptic-curve-key"), a))
          }
      }
  }
  ...
  impl TryDerive<Item> for signing::PublicKey {
      fn try_derive_with<M: Mode>(i: Item) -> Result<Self, M::Err<Item>> {
          ...
          if a.get(&fit!("type")) == Some(fit!("elliptic-curve-key")) {
              ...
              Err(M::expected(fit!("public-key"), Item::derive(a)))
          }
      }
  }
...
#+end_src
*** Derivation
This describes how we convert from one type to another (which the interpreter does a lot)
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  use crate::types::container::error::{Error, Nested};
  use crate::types::*;

  /// A marker trait for types that can be blanket converted to themselves
  pub trait Identity {}

  /// Trait to specify how one type is converted to another,
  /// specifically how failures are dealt with.
  pub trait Mode: Sized + 'static {
      type Err<Src>;

      fn expected<Src: Fit<Item> + 'static>(expected: Word, src: Src) -> Self::Err<Src>;
      fn from<Src: Fit<Item> + 'static, Err: Fit<Error> + 'static>(
          src: Src,
          err: Err,
      ) -> Self::Err<Src>;
      fn nest_error<Outer: Fit<Item>, Inner: Fit<Item>>(
          outer: Self::Err<Outer>,
          inner: Self::Err<Inner>,
      ) -> Self::Err<Outer>;
      fn revert<Src, Dst>(err: Self::Err<Src>, f: impl FnOnce(Src) -> Dst) -> Self::Err<Dst>;
  }

  /// A unit struct to indicate we're deriving one type from another
  /// "fast" - if it fails we don't want a heavy informative error, just
  /// give the original back so we cantry something else. Use this when
  /// failure is somewhat "expected" and no explanation needs to be
  /// bubbled up.
  pub struct Fast;

  /// A unit struct to indicate we're deriving one type from another
  /// "detailed" - if it fails, give as much information as possible
  /// about what went wrong.
  pub struct Detailed;

  /// A function that returns [Error]
  pub type ErrThunk = Box<dyn FnOnce() -> Error>;

  impl Mode for Detailed {
      type Err<Src> = ErrThunk;

      fn expected<Src: Fit<Item> + 'static>(expected: Word, src: Src) -> Self::Err<Src> {
          Box::new(move || Error::expected(expected, src))
      }
      ...
      fn nest_error<Outer: Fit<Item>, Inner: Fit<Item>>(
          ...
          Box::new(move || outer().nest(inner()))
      }

      fn revert<Src, Dst>(err: ErrThunk, _f: impl FnOnce(Src) -> Dst) -> ErrThunk {
          err
      }
  }
  ...
  impl Mode for Fast {
      ...
      fn expected<Src: Fit<Item>>(_expected: Word, src: Src) -> Src {
          src
      }

      fn from<Src: Fit<Item>, Err: Fit<Error>>(src: Src, _err: Err) -> Self::Err<Src> {
          src
      }

      fn nest_error<Outer: Fit<Item>, Inner: Fit<Item>>(outer: Outer, _inner: Inner) -> Outer {
          outer
      }

      fn revert<Src, Orig>(err: Src, f: impl FnOnce(Src) -> Orig) -> Orig {
          f(err)
      }
  }

  /// A trait that allows one original value to be swapped for
  /// another. This is useful when doing derivations that have multiple
  /// steps. If the first step fails, we just return whatever we started
  /// with, but if the 2nd step fails, we have to go back to whatever
  /// was the input to the first step.
  pub trait SwapOriginal<F> {
      type Output;
      fn swap_original(self, f: F) -> Self::Output;
  }

  /// An implementation that updates the return value (Ok) of a result,
  /// otherwise does nothing.

  impl<O, N, K, F, M> SwapOriginal<F> for Result<K, M::Err<O>>
  where
      M: Mode,
      F: FnOnce(O) -> N, // just the inner src transform, not the whole Err
  {
      type Output = Result<K, M::Err<N>>;

      fn swap_original(self, f: F) -> Self::Output {
          self.map_err(|e| M::revert(e, f))
      }
  }
  ...
  pub trait TryDerive<Src>: Sized {
      fn try_derive_with<M: Mode>(value: Src) -> Result<Self, M::Err<Src>>;

      fn try_derive(value: Src) -> Result<Self, Error> {
          Self::try_derive_with::<Detailed>(value).map_err(|e| e())
      }
  }
  ...
  pub trait TryFit<Target>: Sized {
      fn try_fit_with<M: Mode>(self) -> Result<Target, M::Err<Self>>;

      fn try_fit(self) -> Result<Target, Error> {
          Self::try_fit_with::<Detailed>(self).map_err(|e| e())
      }
  }
  ...
  {
      fn try_fit_with<M: Mode>(self) -> Result<Target, M::Err<Src>> {
          Target::try_derive_with::<M>(self)
      }
  }
  ...
  where
      T: Identity + Fit<Item>,
  {
      fn try_derive_with<M: Mode>(value: T) -> Result<T, M::Err<T>> {
          Ok(value)
      }
  }

  // impl<Src, Target> Derive<Src> for Target
  // where
  //     Target: TryDerive<Src, Infallible>,

  // {
  //     fn derive(s: Src) -> Target {
  //         <Target as TryDerive<Src, Infallible>>::try_derive(s).unwrap()
  //     }
  // }

  // Blanket impl for everything that implements derive and returns a Result
  // impl<T, U, E> TryDerive<T> for U
  // where
  //     U: Derive<Result<U, E>>,
  // {
  //     fn try_derive(value: T) -> Result<U, Error> {}
  // }

  // impl<T, S> TryDerive<S> for T
  // where
  //     Result<T, Error>: Derive<S>,
  // {
  //     fn try_derive(value: S) -> Result<T, Error> {
  //         Result::<T, Error>::derive(value).map_err(Into::into)
  //     }
  // }
#+end_src
*** Serialization
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  impl TryDerive<edn_format::Value> for Item {
      fn try_derive_with<M: Mode>(
      ...
      ) -> Result<Item, M::Err<edn_format::Value>> {
          ...
          match item {
              ...
              edn_format::Value::Vector(v) => Ok(v
                  .try_rewrap::<coll::List, M>()
                  .map_err(|e| e.update(edn_format::Value::Vector).clear_inner())?
                  .fit()),
  ...
  impl TryDerive<Result<edn_format::Value, edn_format::ParserError>> for Item {
      fn try_derive_with<M: Mode>(
          r: Result<edn_format::Value, edn_format::ParserError>,
      ) -> Result<Item, M::Err<Result<edn_format::Value, edn_format::ParserError>>> {
          match r {
              Ok(val) => match val.clone().try_fit_with::<M>() {
                  Ok(val) => Ok(val),
                  Err(pe) => Err(M::from(Ok(val), pe)), //TODO: problem is edn_format structs are not items, maybe allow them to be convertible?
              },
  ...
  pub fn parse_edn(env: &mut Environment) -> Result<(), Error> {
      ...
      loop {
          ...
          match parser.next() {
              Some(Ok(v)) => {
                  obj_buffer.push_back((&v).try_fit()?);
              }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  lazy_static! {
      ...
      let mut entries: Vec<(&str, &'static StepFn)> = vec![
          ...
          (
              "character",
              Box::leak(Box::new(f_stack1(Char::try_derive))),
          ),
         ...
         // ("decide", Box::leak(Box::new(decide))),
          ...
          (
              "environment",
              Box::leak(Box::new(f_stack1(Environment::try_derive))),
          ),
          ...
          ("number", Box::leak(Box::new(f_stack1(
              Number::try_derive,
          )))),
          ...
          (
              "set",
              Box::leak(Box::new(f_stack1(coll::Set::try_derive))),
          ),
          ...
          ("word", Box::leak(Box::new(f_stack1(Word::try_derive)))),
  ...
  pub fn swap(mut env: Environment) -> Sometime<'static, Environment> {
      env.pop_prog();
      swap2(env, 0)
  }

  pub fn swapdown(mut env: Environment) -> Sometime<'static, Environment> {
      env.pop_prog();
      swap2(env, 1)
  }

  pub fn swapdeep(mut env: Environment) -> Sometime<'static, Environment> {
      env.pop_prog();
      swap2(env, 2)
  }
  ...
  pub fn is_list(i: Item) -> bool {
      <coll::Sized as TryDerive<_, Fast>>::try_derive(i)
  ...
  pub fn dipdown(mut env: Environment) -> Sometime<'static, Environment> {
      match coll::List::try_derive(env.pop()) {
          Ok(program) => {
  ...
  pub fn fail(env: &mut Environment) -> Result<(), Error> {
      let mut err = Error::try_derive(env.tos().unwrap()).map_err(Error::derive)?;
  ...
  pub fn dictmerge(env: &mut Environment) -> Result<(), Error> {
      ...
      let modified = dict::Dictionary::try_derive(env.tos().unwrap()).map_err(Error::derive)?;
      let mut existing =
          dict::Dictionary::try_derive(env.stack.get(1).unwrap()).map_err(Error::derive)?;
      let namespace =
          dict::Namespace::try_derive(env.stack.get(2).unwrap()).map_err(Error::derive)?;
  ...
  fn unpack(env: &mut Environment) -> Result<(), Error> {
      ...
      fn destructure(
          ...
          let data: Result<coll::Sized, Item> = data.clone().try_fit();
          let pattern: Result<coll::Sized, Item> = pattern.clone().try_fit();
          match (data, pattern) {
              (Ok(data), Ok(pattern)) => {
                  // We got containers
                  match (data, pattern) {
                      ...
                      (a, b) => {
                          ...
                          match (
                              Result::<assoc::Associative, coll::Sized>::try_derive(a),
                              Result::<assoc::Associative, coll::Sized>::try_derive(b),
                          ) {
                              (Ok(data), Ok(pattern)) => {
                              ...
                              (Ok(a), Err(b)) => Err(match_err(a, b.src)),
                              (Err(a), Ok(b)) => Err(match_err(a.src, b)),
                              (Err(a), Err(b)) => {
                                  let a = a.src;
                                  let b = b.src;
                                  if a == b {
              ...
              (Ok(d), Err(pe)) => {
                  let d = Item::derive(d);
                  if d == pe.src
                      || Word::try_derive(pe.src.clone())
                          .map(|w| is_wildcard(&w))
                          .unwrap_or_else(|_| false)
                  {
                      Ok((acc_assoc, acc_seq))
                  } else {
                      Err(match_err(pe.src.clone(), d))
                  }
              }
              (Err(de), Ok(p)) => {
                  let p = Item::derive(p);
          ...
          if let Some(d) = standard_env.dictionary.lingo.get(&w) {
              d.examples
                  ...
                  .filter_map(|ex| {
                      ...
                      match (p, exp_prog) {
                          (Ok(p), Ok(exp)) => {
                              if !contains_at_all(&p, &Item::Word(w.clone())) {
                                  Some(Error::create(
                                      list!(w.clone(), "test",),
...
#+begin_src rust :tangle src/main.rs :mkdirp yes
  ...
  #[tokio::main]
  async fn main() {
      // println!("Size of Item: {}", mem::size_of::<types::Item>());
      // println!("Size of Bytes: {}", mem::size_of::<types::Bytes>());
...
#+begin_src rust :tangle src/types/container/pipe.rs :mkdirp yes
  ...
  pub trait FutureTake {
      type Item;
      fn take_future<'a>(&'a mut self) -> types::Future<'a, Result<Option<Self::Item>, Error>>;
  }
...
#+begin_src rust :tangle src/types/container/pipe/fs.rs :mkdirp yes
  ...
  impl FutureTake for StaticFile {
      type Item = Bytes;
      fn take_future<'a>(&'a mut self) -> Future<'a, Result<Option<Self::Item>, Error>> {
  ...
  impl StaticFile {
      pub fn put<'a>(&'a mut self, i: Item) -> Future<'a, Result<(), Error>> {
          match self.file.as_mut() {
              Some(f) => {
...
#+begin_src rust :tangle src/types/container/pipe/net.rs :mkdirp yes
  ...
  impl Socket {
      pub fn put<'a>(&'a mut self, i: Item) -> types::Future<'a, Result<(), Error>> {
          //println!("Putting {:?}", i);
...
*** TODO Compilation
**** CANCELED initial exploration

- State "CANCELED"   from "TODO"       [2025-08-11 Mon 15:49] \\

I don't think we're going to do this - it's too complex and making it
truly fast would likely interfere with the language's flexibility.
...
**** TODO Second attempt
The way combinators work now in kcats (they don't execute, the just
combine snippets), we can in fact implement the word compile, which
would turn snippets into basically a Vec of rust fns ( fn (e:
Environment) -> Environment ) and should contain a reference to the
original list in case we want to treat it like a list later (like, for
serialization).

This saves the hashmap lookup cost, and otherwise should be
transparent, except for the sprinkling of the word =compile= in
combinators like if, map, loop, etc.

One other necessary change would be in recur to stop using splicing -
that undoes the compilation each iteration. Instead of =✂️4️⃣= we'd write =4️⃣
▶️=.
...
*** TODO Program.rs
**** TODO extend does an O(n) mem-move every time

#+begin_src rust
pub fn extend(&mut self, snippet: cont::List) {
    if !snippet.is_empty() {
        self.0.insert(0, Frame { … })   // ← shifts the whole Vec!
    }
}
#+end_src
Vec::insert(0, …) copies all existing frames one slot to the right for every call. If extend happens inside tight loops this quickly dwarfs everything else.

Fixes

Semantically swap the two operations:

• push() for “execute this snippet next”
• push_back() / VecDeque / keep a separate pending queue for “execute this snippet after the current call stack returns”

or keep Vec and only ever push/pop at the back.
***** TODO peek walks the frame stack every call
#+begin_src rust
let mut frameindex = self.0.len() - 1;
loop {
    if let Some(f) = self.0.get(frameindex) {
...
A branch-heavy loop plus bounds checks plus is_finished() for every
level, only to read one reference. If the interpreter does “peek,
dispatch, pop” thousands of times a second this is visible in perf.

Idioms to remove that cost

    Maintain the invariant “top frame is never finished”: clean one
    frame right after you see that next() returned None instead of
    calling clean() at the beginning of pop().  Then both peek and pop
    are a single last() / last_mut().
  ...
  #+end_src   
(peek never needs to scan.)
***** TODO Turn cont::List into an immutable slice
...
goes through whatever abstraction cont::List implements. If that is a
regular Vec<Item> behind a reference-counted handle you pay:

  +  a bounds check in get
  +  one Rc/Arc indirection to reach the buffer
  +  another bounds check by LLVM because it cannot see through the method
...
and pass around &'a [Item] when you construct a frame.  code[ip] then
compiles down to one mov + one bounds check that LLVM can usually
eliminate (because ip < code.len() is proven at push time).

***** INPROGRESS Inline the tiny helpers

- State "INPROGRESS" from "TODO"       [2025-08-11 Mon 16:06]
Hot helpers like is_finished, next, peek, pop are two–three instructions each. Add

#+begin_src rust
#[inline(always)]
#+end_src

on them or mark the whole impl Frame block with #[inline].
Without it Rust sometimes puts them out-of-line and you pay the call/ret overhead in the dispatch loop.

*** TODO Don't return pinned boxes on each eval-step
So i think instead of each step returning Future<Environment> and
using future::ready when it's pure functional, I should return an enum
with 2 variants: enum FEnv { IO(Pin<Box<dyn Future<Environment>>>),
Functional(Environment) } and then in the eval stepper function, i do
a match, and if it's an IO i await it, otherwise call the next step on
the Environment.

*** TODO Don't use im::Vector for the stack
Vec is faster. Convert vec to im::Vector if we're introspecting an environment.
...
Some of the above is out of date.

Let's see if we can write a performant "links" that returns all the places the given symbol occurs (forward in the list):
#+begin_src kcats
   [[book? title title?]
    [book? author author?]
    [author? first-name author-first?]
    [author? country "United States"]]
  indexed
   ;; maybe generate pairs of [c[i], c[i+1..]]?

   [📤 [•👥 🔀 pair] bail] generator
   ;;[] each
   7 taker collect

   ;; the inner function take 2 args: this row and subsequent rows,
   ;; should return: each item, as a pair: [item, links] as coordinates
   ;; (remember these are relative and not absolute)
...
[[[0 [book? title title?]]
  [[1 [book? author author?]]
   [2 [author? first-name author-first?]]
   [3 [author? country "United States"]]]]
 [[1 [book? author author?]]
  [[2 [author? first-name author-first?]]
   [3 [author? country "United States"]]]]
 [[2 [author? first-name author-first?]]
...
** DONE Make input spec check errors more helpful
- State "DONE"       from "INPROGRESS" [2025-07-15 Tue 16:05]
- State "INPROGRESS" from "TODO"       [2025-07-15 Tue 16:05]
Right now we get outputs like this:
...
and I think it should look more like this (where info from the spec is included

#+begin_src kcats
  [[actual [[[integer start] [✅ 0] 
             [integer end] [✅ 10] 
             [integer step] [[] 0.5]]]]
   [asked [match-input-spec?]]
   [handled 🔳]
   [reason "type mismatch"]
   [trace [-0.5]]
   [type error]
...
looks ok, but we broke a test for =pair?= which introspects errors:
...
** DONE Empty words or with spaces, should not be allowed
- State "DONE"       from "INPROGRESS" [2025-07-31 Thu 10:40]
- State "INPROGRESS" from "TODO"       [2025-07-22 Tue 18:42]
#+begin_src kcats
1 "a b c" word 2
#+end_src
...
converting str -> word should be try_derive, not derive, since not all strings are convertible to word.

Write me function that converts &str to  this type:
...
It's like a lisp or clojure symbol, but has slightly different rules for validity:

+ UTF8 and emoji are allowed

** TODO [#C] Performance redesign
*** Chat with chatgpt
**** Me
:PROPERTIES:
:CUSTOM_ID: message
...
I'm writing a stack based interpreter in rust similar to Joy. it's a
pretty slow design overall, and i want to get as much performance as I
can given the stack-based nature. My first attempt just basically uses a
Vec to store the stack, and my stack values are an Item enum, with lots
of different concrete types. To save memory I box the larger types (that
aren't already references). But i'm thinking this design of "get a few
stack items from the heap, work with them and put results back on the
heap" might be slow (i really don't have a good sense of what perf
bottlenecks really would be, maybe this is a non issue). And that some
structure that lets me keep a few stack items nearer to the CPU would be
much better because the top stack items are by definition the ones which
will be needed next. it's only as items get buried deeper that they can
be moved 'further away'. But standard Vec doesn't provide anything like
that. Is there a rust lib or idiom that would help here?
...
[[data:image/svg+xml;base64,]]

[[data:image/svg+xml;base64,]]

The short version​

1. Don't worry about “being near the CPU” -- a =Vec= *already keeps the
   whole stack in one contiguous chunk of memory*, so the most-recent
   elements are always in the last cache line you touched.
2. What /does/ cost you is
   • an extra indirection for every =Box=,\\
   • unpredictable branches when you pattern-match a big =enum=, and\\
   • heap allocations performed while the program is running.

Therefore the usual recipe for a fast Joy / Forth-like interpreter in
Rust is:

• keep the stack as =Vec<Value>= (contiguous, cache-friendly)\\
• make =Value= one machine word + a tag, so small scalars live inline
and only the “big” things are boxed once and shared\\
• optionally add a tiny register-sized “front buffer” to avoid even the
=Vec= bounds checks.

Below is the common layout and the optional front buffer.\\
After the code you'll find the rationale and some crate suggestions.

#+begin_src rust
// src/value.rs
use std::ptr::NonNull;

#[repr(u8)]            // <= one byte tag
pub enum Tag {
    Int,
    Float,
    Bool,
    Heap,              // everything that doesn’t fit in 56 bits
}
...
impl Value {
    const TAG_BITS: u64 = 0b111;            // low three bits
    const PAYLOAD_MASK: u64 = !Self::TAG_BITS;

    #[inline]
    pub fn int(i: i64) -> Self {
        // shift left so the tag fits
        Value(((i as u64) << 3) | (Tag::Int as u64))
    }
    #[inline]
    pub fn as_int(self) -> i64 {
        (self.0 as i64) >> 3
    }
    // Similar helpers for f64, bool);
    // For boxed things store the pointer as payload and set Tag::Heap.
    #[inline]
    pub fn ptr(p: NonNull<HeapObj>) -> Self {
        debug_assert!(p.as_ptr() as usize & 0b111 == 0); // alignment
        Value((p.as_ptr() as u64) | (Tag::Heap as u64))
    }
    #[inline]
    pub fn tag(self) -> Tag {
...
//------------------------------------------------------------------------------
// Optional register stack (front buffer)

const FRONT: usize = 8;             // 8 values stay in registers/L1
...
impl Stack {
    pub fn new() -> Self {
        Self { front: [Value(0); FRONT], len_front: 0, spill: Vec::new() }
    }

    #[inline(always)]
    pub fn push(&mut self, v: Value) {
        if self.len_front < FRONT {
            self.front[self.len_front] = v;
            self.len_front += 1;
        } else {
            self.spill.push(v);
        }
    ...
    pub fn pop(&mut self) -> Value {
        if self.len_front > 0 {
            self.len_front -= 1;
            self.front[self.len_front]
        } else {
            self.spill.pop().expect("stack underflow")
        }
    }
}
#+end_example

***** Why this is usually fast enough
:PROPERTIES:
:CUSTOM_ID: why-this-is-usually-fast-enough
:END:
Contiguous array = cache-friendly\\
=Vec= stores elements back-to-back. When you =push=/=pop= the CPU will
touch the last 1--2 cache lines repeatedly, exactly what you want.

One word + tag = no per-value allocation\\
=i64=, =f64=, =bool=, =char=, etc. are all represented /inside/ the
64-bit slot. Anything that does not fit (=String=, user records, arrays)
gets /one/ allocation and is referenced by pointer. You removed the
“Box-per-value” overhead.

No virtual dispatch\\
=match v.tag()= on four variants is predicted well by modern CPUs. The
expensive part is eliminated: dynamic trait objects and per-operation
heap traffic.

Optional front buffer\\
Bounds checks on =Vec= are /very/ cheap (=llvm= usually eliminates
them), but if you want to squeeze the last few percent you can keep a
small fixed-size ring (4-8 entries) in the interpreter struct. Push/pop
on that array needs no check at all and only spills when full.

***** Useful crates / idioms
:PROPERTIES:
:CUSTOM_ID: useful-crates-idioms
:END:
smallvec / arrayvec -- give you an inline capacity for secondary data
structures, not for the main value stack\\
bytemuck -- for safe transmutation between =Value= and raw bits\\
static-inline-global -- if you implement threaded code and want labels

***** When to look elsewhere
:PROPERTIES:
:CUSTOM_ID: when-to-look-elsewhere
:END:
If you sample the interpreter with =perf= / =cargo‐flamegraph= and see
that you are CPU-bound in instruction dispatch, switch from a =match=
dispatch loop to

• direct-threaded interpreter (computed-goto; doable in nightly Rust
with the =experimental_inline_asm_sym= feature), or\\
• “copy-the-bytecode into a big array of function pointers” and call
through that (works on stable).

Those bring larger wins than micro-tuning the stack once the obvious
indirections are gone.

***** Summary
:PROPERTIES:
:CUSTOM_ID: summary
:END:
Keep the stack contiguous, store scalars inline, box complex objects
once, and profile before redesigning.\\
The =Vec<Value>= approach with a tagged 64-bit =Value= is the prevailing
idiom for fast toy VMs in Rust, C, Lua, JavaScriptCore, etc.
...
what do you mean by 'tagged 64 bit Value' - right now my enum is 48
bytes, and it was as large as 224 because of not boxing and several
levels of enum hierarchy that add 8 bytes each.
...
Think “Java-script value”, “Lua value”, or “NaN-boxed value”: every
run-time value---an integer, a float, a pointer to a string, ...---fits
in one 64-bit machine word. A few of its bits carry a /tag/ that says
what it is, and the remaining bits are the payload.

Why 64 bits?

• On every desktop / server the pointer width is 64 bits, so one word is
the natural unit.\\
• A word is what the CPU can move, compare, or push/pop in a single
instruction.\\
• Keeping the interpreter's data path “one register wide” eliminates
copies and indirections.

Below is the conceptual picture and a concrete Rust layout.

--------------

***** 1 How many bits are actually free?
:PROPERTIES:
:CUSTOM_ID: how-many-bits-are-actually-free
:END:
• Heap pointers are always *aligned to at least 8 bytes* → their three
least-significant bits are /zero/. We can re-use those bits as a tag.\\
• IEEE-754 doubles reserve *2¹¹−2 patterns* for NaNs; libraries like
LuaJIT and WebKit store pointers or integers in those otherwise
“forbidden” bit patterns (this is called NaN boxing).\\
• Small signed integers fit in 61 bits (2⁶¹ ≈ 2.3e18) once we steal
three tag bits.

So with one word you can encode

Tag 000 → small signed int\\
Tag 001 → =f64= whose pattern is /not/ a pointer (normal number)\\
Tag 010 → boolean (payload bit = 0/1)\\
Tag 111 → “heap object”; payload = pointer with the low bits zero

You can of course pick any scheme.

--------------

***** 2 A minimal Rust implementation
:PROPERTIES:
:CUSTOM_ID: a-minimal-rust-implementation
:END:
#+begin_example rust
// value.rs
use std::ptr::NonNull;
...
pub enum Tag { Int = 0, Float = 1, Bool = 2, Heap = 7 }
...
impl Value {
    ...
    pub fn float(f: f64) -> Self {               // plain cast keeps bits
        Value(f.to_bits() & !Self::TAG_MASK | Tag::Float as u64)
    }
    pub fn boolean(b: bool) -> Self {
        Value(((b as u64) << 3) | Tag::Bool as u64)
    }
    pub fn ptr<T>(p: NonNull<T>) -> Self {
        // alignment ≥ 8 → ok to reuse bits
        debug_assert!(p.as_ptr() as usize & 0b111 == 0);
        Value(p.as_ptr() as u64 | Tag::Heap as u64)
    }
    ...
    #[inline]
    pub fn tag(self) -> Tag {
        unsafe { std::mem::transmute((self.0 & Self::TAG_MASK) as u8) }
    }
    pub fn as_int(self)  -> i64 { (self.0 as i64) >> 3 }
    pub fn as_bool(self) -> bool { (self.0 >> 3) & 1 != 0 }
    pub fn as_float(self) -> f64 { f64::from_bits(self.0 & !Self::TAG_MASK) }
    pub fn as_ptr<T>(self) -> NonNull<T> {
        NonNull::new((self.0 & !Self::TAG_MASK) as *mut T).unwrap()
    }
}
#+end_example

=Value= is 8 bytes, trivially copyable, lives in registers, and the VM's
operand stack is just =Vec<Value>=.

--------------

***** 3 Why your enum became 48--224 bytes
:PROPERTIES:
:CUSTOM_ID: why-your-enum-became-48224-bytes
:END:
=enum Item= in Rust is laid out like C/Swift “tagged unions”:

size = max(size_of(variant_i)) rounded up for alignment\\
+ 1 discriminant (1--8 bytes)

If one variant holds

#+begin_example rust
Array(Vec<Item>)        // Vec = 24 bytes on x86-64
#+end_example

and another holds

#+begin_example rust
Closure { env: Box<[Value]>, ip: usize }   // 16 bytes
#+end_example

then the /whole/ enum has to be at least 24 bytes. If you nest enums the
large variant is repeated, which is how you reached 48, 96, 224, ...

By moving “big” things behind a pointer and keeping only that pointer in
the enum, the largest variant becomes a single word, so the enum itself
shrinks to 16 bytes (pointer + tag). With the manual tagging above it
drops to 8 bytes.

--------------

***** 4 Is this worth the trouble?
:PROPERTIES:
:CUSTOM_ID: is-this-worth-the-trouble
:END:
Almost always, yes:

• Interpreter hot loops fit in L1/L2 cache.\\
• Stack frames push/pop one word instead of touching the allocator.\\
• =memcpy=/=copy_nonoverlapping= work on slices of =Value=s.\\
• You get predictable performance close to LuaJIT / V8 “baseline”
interpreters without going JIT.

If you prefer less low-level code, a compromise is:
...
This is 16 bytes---already a *3× shrink* from your 48-byte enum and
requires no hand tagging. The single heap indirection is usually
acceptable.
...
:CUSTOM_ID: take-away
:END:
A “tagged 64-bit value” means: one 64-bit word whose low bits encode the
type and whose high bits are the actual integer / float bits /or/ a
pointer to heap data. Using it turns every stack slot into one register
move and keeps the whole operand stack in a compact, cache-friendly
array.
...
is there a crate to help manage the tagged values?
...
There is no “standard” crate that everyone depends on, but there are a
handful of small crates that give you most of the plumbing for
pointer-/NaN-tagging so you don't have to write the bit-twiddling
yourself.

***** Crates you can use right now
:PROPERTIES:
:CUSTOM_ID: crates-you-can-use-right-now
:END:
- =nanbox= -- safe façade over a 64-bit NaN-boxed value.\\
  -- supports =i32=, =f64=, =bool=, =char=, and any =*mut T= pointer\\
  -- gives you =enum NanBoxed { Int(i32), Float(f64), … }= plus
  =into_raw()/from_raw()= if you want to keep it at the word level\\
  -- no =unsafe= in user code unless you extend it with your own
  payloads

- =tagged_ptr= -- minimal pointer-tagging helper.\\
  -- =TaggedPtr<T, const TAG_BITS: usize>= stores an aligned =*mut T=
  plus =TAG_BITS= bits of metadata\\
  -- you add your own enum and payload handling; good when you do /not/
  want NaN boxing and prefer the “steal the low 3 bits” scheme

- =enum_primitive_derive= or =num_enum= -- not about tagging per se, but
  they derive cheap conversions between =u8= and your =Tag= enum, which
  avoids the manual =transmute= dance.

- =tinyvec= / =arrayvec= -- if you decide to keep a small inline buffer
  for the operand stack these crates give you a safe “vector with N
  inline slots, spill to =Vec= when full”.

***** Choosing between them
:PROPERTIES:
:CUSTOM_ID: choosing-between-them
:END:
| use-case                                                                                                      | good choice |
|---------------------------------------------------------------------------------------------------------------+-------------|
| You want something that “just works” and already boxes =f64= → =nanbox=                                       |             |
| You only need pointer tagging, floats stay as plain =f64= → =tagged_ptr=                                      |             |
| You are happy to maintain 30 lines of unsafe but want zero deps → roll your own (like in the previous answer) |             |

***** Caveats
:PROPERTIES:
:CUSTOM_ID: caveats
:END:
- All crates above rely on =unsafe= internally; read their docs about
  soundness conditions (usually “pointers must keep the alignment they
  were created with”).
- NaN boxing breaks if you ever store signalling-NaNs or rely on exact
  bit patterns for =f64=; that is rarely an issue in interpreters, but
  good to be aware of.
- The crates are small (sub-1.0) projects; if the VM is production
  software you may still prefer an in-tree implementation you fully
  control.

In practice most hobby / research VMs end up in one of two camps:

1. Copy 60 lines from LuaJIT/WebKit (your own implementation).
2. Use =nanbox= for convenience during prototyping, then inline the code
   once the design stabilises.

Either way you get the same performance profile: one 64-bit word per
value, no per-push allocation, and the whole operand stack packed inside
a =Vec<Value>=.
...
hm ok good to know. And what about the constant push/pop of individual
instructions from the program? is Vec ok for this? The frequent
operations are, each interpreter loop pops off the next instruction,
which may extend the Vec with a new list of instructions (to be executed
first). I thought about changing to a design where program snippets are
immutable and we just move a pointer. i implemented that, but it wasn't
...
=Vec= is perfectly fine for the /data/ stack, but it is rarely the best
shape for the /instruction/ stream. The usual pattern in byte-code or
concatenative interpreters is

1. keep the program in an immutable array (=Vec<Instr>= or
   =Box<[Instr]>=)
2. keep an instruction pointer (=ip: usize=)
3. advance the pointer instead of =pop=-ing instructions out of the
   vector, and
4. when a word wants to execute an embedded quotation or splice in a new
   phrase, push the *current* state on a /call stack/ and jump to the
   new slice.

That design avoids the =memmove= you pay every time you remove the first
element of a =Vec=. The tiny pseudo-code below shows the idea.

#+begin_example rust
#[derive(Clone)]
struct Frame<'prog> {
    code: &'prog [Instr],   // immutable slice
    ip:   usize,            // next instruction inside that slice
}

struct VM<'prog> {
    run_stack: Vec<Frame<'prog>>,   // program / return stack
    data_stack: Vec<Value>,         // operand stack from earlier messages
}

impl<'p> VM<'p> {
    fn new(entry: &'p [Instr]) -> Self {
        Self {
            run_stack: vec![Frame { code: entry, ip: 0 }],
            data_stack: Vec::new(),
        }
    }

    fn step(&mut self) -> bool {
        let Frame { code, ref mut ip } = self.run_stack
            .last_mut()
            .expect("empty run-stack = program finished");

        let instr = match code.get(*ip) {
            Some(i) => *i,
            None    => { self.run_stack.pop(); return !self.run_stack.is_empty(); }
        };
        *ip += 1;

        match instr {
            Instr::PushInt(n) => self.data_stack.push(Value::int(n)),
            Instr::Add        => { /* … */ }

            // quotation ‑-> jump into a new slice, no copying
            Instr::ExecQuote(q) => self.run_stack.push(Frame { code: q, ip: 0 }),
        }
        true
    }
}
...
• =code= is immutable, so it stays in a contiguous cache-friendly
block.\\
• Advancing =ip= is one addition; no element removal, no reallocation.\\
• Splicing a quotation is a single =push= of a 16-byte =Frame= (two
registers) --- cheaper than inserting at the front of a =Vec=.\\
• Returning from a quotation is just =pop= on the run-stack.

When would you choose something else?

• If you really need constant-time insertion /at both ends/ of the
program stream, =VecDeque= is the standard choice. (It is still a ring
buffer in contiguous memory, but lets you pop from the front.)

• If profile data show dispatch, not queue management, is the hottest
spot, switch to direct-threading (computed gotos / function-pointer
tape). That gives a larger speed-up than changing container types.

But as long as you follow the immutable-code-with‐ip pattern, the
container itself is not the bottleneck; the interpreter's dispatch and
the value representation dominate.
...
ok that looks similar to what i implemented. see if you spot any perf
issues with what i did:
#+begin_src rust
  use crate::traits::*; use
  crate::types::container as cont; use
  crate::types::container::error::Error; use
  crate::types::container::SimpleTake; use crate::types::Item; /// A
  struct to hold program execution data efficiently #[derive(Clone,
  PartialEq, Debug)] pub struct Frame { snippet: cont::List, next_idx:
  usize, } impl Frame { pub fn is_finished(&self) -> bool { self.next_idx
  >= self.snippet.len() } pub fn next(&mut self) -> Option<&Item> { let i
  = self.snippet.get(self.next_idx); self.next_idx += 1; i } pub fn
  peek(&self) -> Option<&Item> { if self.is_finished() { None } else {
  self.snippet.get(self.next_idx) } } pub fn current(&self) ->
  Option<&Item> { if self.next_idx == 0 { return None; }
  self.snippet.get(self.next_idx - 1) } pub fn remaining(&mut self) ->
  cont::List { let last = self.snippet.len();
  self.snippet.slice(self.next_idx..last) } } #[derive(Clone, PartialEq,
  Default, Debug)] pub struct Program(Vec<Frame>); impl Program { fn
  clean(&mut self) { while self.0.last().map(|f|
  f.is_finished()).unwrap_or(false) { self.0.pop(); //println!("dropping
  frame: {:?}", x); } } ///return a reference to the current instruction
  and advance to /// next pub fn pop(&mut self) -> Option<&Item> {
  //println!("program: {:?}", self.0); self.clean(); // Next instruction
  is ready to go self.0.last_mut().and_then(|f| f.next()) } ///return a
  reference to the next instruction pub fn peek(&self) -> Option<&Item> {
  // might be in unclean state, and we don't want to mutate here // so we
  search for the next instruction without cleaning first if
  self.0.is_empty() { return None; } let mut frameindex = self.0.len() -
  1; loop { if let Some(f) = self.0.get(frameindex) { //println!("Peeking
  frame: {:?}", f); if f.is_finished() { //println!("Frame is finished");
  if frameindex == 0 { return None; } else { frameindex -= 1 }; } else {
  //println!("Frame not finished"); return f.peek(); } } else { return
  None; } } } pub fn is_empty(&self) -> bool { self.peek().is_none() } pub
  fn prepend(&mut self, snippet: cont::List) { self.clean(); if
  !snippet.is_empty() { self.0.push(Frame { snippet, next_idx: 0, }) } }
  pub fn extend(&mut self, snippet: cont::List) { if !snippet.is_empty() {
  self.0.insert( 0, Frame { snippet, next_idx: 0, }, ) } } pub fn
  stacktrace(&self) -> cont::List { let s = self .0 .iter() .rev()
  .filter_map(|frame| frame.current().cloned()) .collect::<cont::List>();
  //s.slice(0..(s.len() - 1)) s } /// TODO: this is probably not accurate
  pub fn count(&self) -> usize { self.0 .iter() .fold(0, |acc, f| acc +
  (f.snippet.len() - f.next_idx)) } pub fn last(&self) -> Option<&Item> {
  self.0.first().and_then(|f| f.snippet.last()) } }
...
The organisation is already “the right shape” (immutable code slices +
stack of frames). The remaining costs are mostly small, but since you
asked for a review, here is what sticks out in descending order of
potential impact.

***** 1 =extend= does an O(n) mem-move every time
:PROPERTIES:
:CUSTOM_ID: extend-does-an-on-mem-move-every-time
:END:
#+begin_example rust
pub fn extend(&mut self, snippet: cont::List) {
    if !snippet.is_empty() {
...
=Vec::insert(0, …)= copies all existing frames one slot to the right for
every call. If =extend= happens inside tight loops this quickly dwarfs
everything else.
...
- Semantically swap the two operations:
  ...
  • =push()= for “execute this snippet *next*”\\
  • =push_back()= / =VecDeque= / keep a separate /pending/ queue for
  “execute this snippet *after* the current call stack returns”

- or keep =Vec= and only ever push/pop at the back.

***** 2 =peek= walks the frame stack /every/ call
:PROPERTIES:
:CUSTOM_ID: peek-walks-the-frame-stack-every-call
:END:
#+begin_example rust
let mut frameindex = self.0.len() - 1;
loop {
...
1. Maintain the invariant “top frame is never finished”: clean one frame
   *right after* you see that =next()= returned =None= instead of
   calling =clean()= at the beginning of =pop()=.\\
   Then both =peek= and =pop= are a single =last()= / =last_mut()=.
...
***** 3 Turn =cont::List= into an /immutable slice/
:PROPERTIES:
:CUSTOM_ID: turn-contlist-into-an-immutable-slice
:END:
Every call to
...
and pass around =&'a [Item]= when you construct a frame.\\
=code[ip]= then compiles down to one =mov= + one bounds check that LLVM
can usually eliminate (because =ip < code.len()= is proven at push
time).

***** 4 Inline the tiny helpers
:PROPERTIES:
:CUSTOM_ID: inline-the-tiny-helpers
:END:
Hot helpers like =is_finished=, =next=, =peek=, =pop= are two--three
instructions each. Add
...
on them or mark the whole =impl Frame= block with =#[inline]=.\\
Without it Rust sometimes puts them out-of-line and you pay the call/ret
overhead in the dispatch loop.

***** 5 Return the value, not =&Item=
:PROPERTIES:
:CUSTOM_ID: return-the-value-not-item
:END:
=pop= returns =Option<&Item>=. That forces the snippet slice to stay
borrowed until the VM finishes using the reference, preventing you from
mutating the program in the same step without resorting to =unsafe= or
=RefCell=. Returning =Item= by value (it is an enum ≤ 16 bytes after the
tagging work) removes the lifetime juggling and lets the optimiser hoist
loads out of matches.

#+begin_example rust
pub fn pop(&mut self) -> Option<Item> {
    self.clean();
    self.0.last_mut().and_then(|f| {
        let out = f.next_idx < f.snippet.len()
               .then(|| f.snippet[f.next_idx].clone());
        f.next_idx += 1;
        out
    })
}
...
***** 6 Micro-cleanups (low impact but free)
:PROPERTIES:
:CUSTOM_ID: micro-cleanups-low-impact-but-free
:END:
- =is_finished()= can avoid the branch by returning the boolean
  directly:
...
- =count()= divides into zero if the current frame is finished; guard
  the subtraction.

- If you need =stacktrace= only for error reporting, build it on demand
  instead of every =Instr::Trace= instruction.

***** Bottom line
:PROPERTIES:
:CUSTOM_ID: bottom-line
:END:
Profiling shows that in stack interpreters the big wins come from
representation and dispatch. After you

1. shrink =Item= to one word,
2. ensure frames are pushed/popped at the back (no front insertion), and
3. remove the repeated scan in =peek=,

the remaining overhead of using plain =Vec= is in the low single-digit
percent range---almost always cheaper than the complexity of writing
your own container.
...
ok thanks for that. Another issue that's been a real pain: as I
mentioned i created an enum hierarchy to let me group functionality
(like, an enum for container, map, pipe, etc and abstract functionality
behind those). But there's the memory overhead, and enums do not support
overlapping functionality like traits do. However traits have their own
issues - object safety, difficulty starting with an Item, getting a
trait object and then doing all needed operations just with that, and
then converting back to Item which requires knowing the concrete type.
it can be done but it's difficult. Boxing helps but that can hurt
performance and casting between dyn<A + B> to dyn<A> isn't possible etc.
what do you recommend? I thought of making a big enum of all the
concrete types (40+) and using traits to operate on them. Have helper
functions to try to convert Item to the proper trait object. advice?
...
• /in-place/ storage (no extra allocation)\\
• /overlapping/ capabilities (a list is both “indexable” /and/
“printable”)\\
• and *zero run-time indirection*

plain trait objects cease to be attractive; a single, †flat†
=enum Value= plus pattern-matching is what almost every high-performance
interpreter ends up with:
...
pub enum Value {
    ...
    Float(f64),
    Bool(bool),
    Str(Rc<String>),       // already on the heap, keep pointer inline
    List(Rc<Vec<Value>>),
    Dict(Rc<HashMap<Value, Value>>),
    // …
}
#+end_example

Size: 16 bytes (8-byte payload + 1-byte tag padded to the next word).

--------------

***** Why not deep enums?
:PROPERTIES:
:CUSTOM_ID: why-not-deep-enums
:END:
#+begin_example
Copy codeenum Container { List(List), Dict(Dict) }
enum Item      { Scalar(Scalar), Container(Container), … }
#+end_example

Every extra layer forces the compiler to align the *largest* inner
variant, so you multiply the size and lose the cache locality you just
won back.

--------------

***** But I need “methods” like =len=, =index=, =display=!
:PROPERTIES:
:CUSTOM_ID: but-i-need-methods-like-len-index-display
:END:
Implement them /on the enum/ and delegate:
    ...
    pub fn add(&self, rhs: &Value) -> Result<Value, Error> {
        match (self, rhs) {
            ...
            (Value::Str(a),   Value::Str(b))   => {
                let mut s = (**a).clone(); s.push_str(b); Ok(Value::Str(s.into()))
            }
            _ => Err(Error::Type("add")),
        }
    }
}
#+end_example

The compiler turns each =match= into a jump table that is /fully
branch-predictable/ (four-to-six entries), so the cost is a couple of
nanoseconds---much cheaper than chasing a fat pointer to a vtable.

If the boiler-plate annoys you, generate it:

#+begin_example rust
use enum_dispatch::enum_dispatch;

#[enum_dispatch(Show, Length)]
#[derive(Clone)]
pub enum Value { … }

trait Show   { fn show(&self) -> String; }
trait Length { fn len(&self) -> Option<usize>; }

impl Show   for i64             { … }
impl Show   for Rc<String>      { … }
impl Length for Rc<String>      { … }
impl Length for Rc<Vec<Value>>  { … }
#+end_example

=enum_dispatch= expands this to exactly the same =match= code but keeps
your source tidy and *remains static dispatch*---no =dyn=, no boxing.

Other helpers: =strum=, =impl_enum::impl_enum=, =paste=.

--------------

***** When you really do want trait objects
:PROPERTIES:
:CUSTOM_ID: when-you-really-do-want-trait-objects
:END:
Some objects are /open-ended/ (user-defined classes, native calls, ...).
For them store a single pointer-sized variant:

#+begin_example rust
pub enum Value {
    …,
    Obj(NonNull<dyn Object>),   // 8 bytes (pointer) + tag
}
#+end_example

Where =Object= is your “everything lives on the heap” trait.

Operations that could land on either side look like

#+begin_example rust
fn length(v: &Value) -> Result<usize, Error> {
    match v {
        Value::Str(s)  => Ok(s.len()),
        Value::List(l) => Ok(l.len()),
        Value::Obj(o)  => unsafe { o.as_ref().length() }, // 1 indirect call
        _              => Err(Error::Type("len")),
    }
}
#+end_example

All built-ins are still zero-indirection; /only/ user objects pay the
dynamic-dispatch cost, which is usually acceptable.

--------------

***** Converting “back and forth”
:PROPERTIES:
:CUSTOM_ID: converting-back-and-forth
:END:
You almost never need /=Item= → =&dyn Trait= → =Item=/ again:

- read operands from the stack as =&Value=
- =match= once to pick the fast path
- push a /new/ =Value= on return

No allocation, no down-cast, no double bookkeeping.

If you /do/ want it (e.g. heterogeneous collections), use =Any=:

#+begin_example rust
if let Value::Obj(ptr) = value {
    let obj = unsafe { ptr.as_ref() };
    if let Some(s) = obj.as_any().downcast_ref::<String>() {
        …
    }
}
#+end_example

where every heap object implements
...
---still one pointer + tag in the interpreter core.

--------------

***** Recommendation in one sentence
:PROPERTIES:
:CUSTOM_ID: recommendation-in-one-sentence
:END:
Flatten the enum, keep scalars inline, store /one/ pointer for
everything that must live on the heap, and dispatch with =match=; use
crates like =enum_dispatch= only to cut down on repetitious code, not as
a replacement for the flat value enum.
...
ok, flatten the enum. but when would i use Any? seems like i don't need
it, since my collection is technically homogenous by storing Values (the
flat enum).
...
Exactly---if every run-time slot is already a =Value= you do *not* need
=Any= inside the core interpreter. =Any= (or any other
run-time-type-information mechanism) only becomes useful when /outside/
code needs to peek behind the “one pointer + tag” abstraction.

Typical scenarios where =Any= (or an equivalent down-cast) shows up

1. *Foreign-function interface / plug-ins*\\
   You keep a variant such as\\
   =Value::Native(NonNull<dyn NativeObject>)=.\\
   A host callback receives that =&dyn NativeObject= and wants to
   recover the original Rust type it registered with the VM:

   #+begin_example rust
fn host_callback(o: &dyn NativeObject) {
       // down-cast from the trait object to the concrete host type
       if let Some(sock) = o.as_any().downcast_ref::<TcpStream>() {
           // work with the socket …
       }
   }
   #+end_example

2. *Embedding* the VM in a larger Rust program where parts of the
   application share real Rust structs and want zero-copy access.\\
   Example: a GUI toolkit stores =Rc<Widget>= inside the VM so that Joy
   scripts can manipulate widgets. Host code still wants to see a
   =Widget=, not just an opaque pointer.

3. *Introspection / debugging tools* that need to print or log internal
   Rust-level details of heap objects.

If your VM

- already boxes every non-scalar value in =Value= variants, and
- never hands those boxes out as trait objects to arbitrary Rust code,

then you can skip =Any=. The flattened enum + direct =match= is all you
need.
...
Ok and what do you think the limits on performance are for this design?
In other words, what do i lose just by being stack-based? I was hoping
at the outset to at least be as fast as python, but so far it's much
much slower, like 50x in some cases.
...
“Stack-based” is *not* inherently 50 × slower than CPython.\\
Lua 5.4, Wren, Factor, and µPy are all stack or quasi-stack VMs and they
run at 1--3 × the speed of CPython, sometimes faster.\\
So the gap you see comes from three other layers:

--------------

***** 1 Instruction-dispatch cost
:PROPERTIES:
:CUSTOM_ID: instruction-dispatch-cost
:END:
A tight C loop with a =switch= over 100 byte-codes clocks at roughly\\
20--40 ns / byte-code on a 3 GHz desktop (≈ 25--50 M instr/s) when
branch-predicted.

• Rust =match= on an =enum= is the same speed if you keep it in one
function and compile with =-C opt-level=3 -C target-cpu=native=.

• If profiling shows >40 ns / instr, switch to\\
-- “direct threading” (computed gotos / =&&label= table)\\
-- or an array of function pointers (=for &f in tape { f(self) }=).\\
Both cut the dispatch cost by ~30 -- 40 %.

--------------

***** 2 Too many byte-codes per high-level operation
:PROPERTIES:
:CUSTOM_ID: too-many-byte-codes-per-high-level-operation
:END:
A pure stack VM needs extra =push/pop/swap/rot= that a register VM
encodes implicitly. That overhead is real but is usually a factor of
1.5--2, not 50.

Mitigations

- Peephole / super-instructions\\
  (=LOAD_CONST 0; LOAD_CONST 1; ADD= → one =ADD_KK 0 1=)
- Specialised variants (=ADD_III= for int + int, =ADD_FFF= for float +
  float)
- Hand-written hot path
  (=if tag_is_int(lhs, rhs) { …fast int add… } else { slow path }=)

--------------

***** 3 Heap traffic and reference counting
:PROPERTIES:
:CUSTOM_ID: heap-traffic-and-reference-counting
:END:
Every heap allocation or atomic ref-count increment costs 10--200 ns. If
every =Value= push allocates a =Box= or bumps an =Rc=, the program is
dead before dispatch even matters.

Make sure

- =Value= is *one word* for scalars, one word + tag for heap things.
- Lists / dicts kept on the heap use =Rc=/=Arc= /once/; pushing the
  pointer on the stack is just a copy of 8 bytes.
- No hidden =clone()= calls in hot loops (look for =#[derive(Clone)]=
  that the compiler cannot elide).

--------------

***** 4 Bounds checks, dynamic borrow checks, debug build
:PROPERTIES:
:CUSTOM_ID: bounds-checks-dynamic-borrow-checks-debug-build
:END:
A stack-loop written in safe Rust is still competitive, /if/:

- compile with =--release= (+ =lto = "fat"= and =codegen-units = 1=).\\
  Debug builds run at 10--40 × slower, which matches the gap you see.
- =unsafe { *ptr.add(sp) }= is not required; LLVM removes the =Vec=
  bounds check when it sees the invariant =sp < stack.len()=.

--------------

***** 5 Object system / dictionaries
:PROPERTIES:
:CUSTOM_ID: object-system-dictionaries
:END:
CPython spends half its time in attribute lookup and hashing strings. If
your benchmark exercises maps heavily and your implementation uses
=HashMap<Value, Value>= with per-lookup allocations, the cost explodes.

- Add a symbol table: intern strings once, compare/have hash on the
  interned =u32 id=.
- Cache the last lookup (=LOAD_GLOBAL=) next to the byte-code (inline
  cache) -- that is how PyPy and LuaJIT get their 3--5 × speed-ups.
...
:END:
Tracing GC pauses do not affect micro-benchmarks, but a naive /“walk
every =Rc= on every operation”/ will kill you. Use either

- plain =Rc= plus *copy-on-write* (Lua), or
- a generational tracing GC triggered every N allocations (CPython
  3.12).

--------------

****** What a realistic Rust interpreter can hit
:PROPERTIES:
:CUSTOM_ID: what-a-realistic-rust-interpreter-can-hit
:END:
#+begin_example
Copy code                                                        (desktop Core i7)
#+end_example

byte-codes / second | 25 M -- 50 M Fibonacci 40 µ-benchmark | 0.8 --
1.5× CPython Richards / DeltaBlue (object benchmarks)| 0.5 -- 1.2×
CPython Mandelbrot | 2 -- 4× CPython (tight numeric)

If you are at /50 × slower/ you are almost certainly measuring:

- debug build, or
- per-value allocation/=Rc::clone=, or
- front-insertion on the frame =Vec=, or
- an interpreter loop that calls out to dozens of small Rust functions
  (no inlining).

Fix those first; the stack vs. register decision at most halves the
speed once everything else is tight.

--------------

****** Checklist to reach “Python-class” speed
:PROPERTIES:
:CUSTOM_ID: checklist-to-reach-python-class-speed
:END:
1. =Value= = 8 bytes + tag, stack = =Vec<Value>=.
2. One tight =loop { match bytecode[ip] { … } }= function, compiled with
   =-O3=.
3. All literals and big objects allocated once, referenced by pointer.
4. No =Vec::insert(0, …)=; push/pop only at the back.
5. =perf=, =cargo flamegraph=, or =dhatu= to see where time goes.
6. Only after the profiler is clean: consider direct threading or a
   register VM to get the last 1.5--2 ×.

A stack machine can absolutely match or beat CPython; the 50 × gap comes
from avoidable layers above the stack abstraction, not from the stack
itself.
...
ok that's good to know, i may not do all those optimizations because i
don't *really* need to be as fast as python just yet, it would be nice.
I think i understand what to do for most of your recommendations. But
one i'm confused about: right now i look at the next instruction, it's
an interned string. It's either: an axiom instruction (with a rust
implementation fn), or derived (it's a program snippet to be appended to
the front of the program). Either way, i look it up in a hashmap. i
think i can figure out how to at least turn the axiom words (which are
all known at compile time) into a match. That should help a lot. but
what about the user-defined instructions?
...
“Look-the-word-up-in-a-hash-map-every-time” is the single largest
overhead most Joy / Forth interpreters have after they solve the value
representation.\\
Fortunately you only need to do that lookup *once per word*, then you
can patch the byte-code so that subsequent executions jump directly to
either

• a built-in primitive (Rust function pointer / enum tag), or\\
• a user quotation (a slice of instructions).

Below are three levels of sophistication; you can start with level-1 and
already save an order of magnitude.

***** Level 1 Resolve once, patch in place
:PROPERTIES:
:CUSTOM_ID: level-1-resolve-once-patch-in-place
:END:
Parsing time / first touch:

#+begin_example
Copy code"dup"   → lookup → Axiom(DUP_IDX)    → overwrite token
"user+" → lookup → Derived(ptr,len) → overwrite token
#+end_example
...
#+begin_example rust
match prog[ip] {
    Op::Axiom(idx)   => AXIOM_TABLE[idx](vm),        // no HashMap!
    Op::Derived(p,l) => vm.call(p, l),              // push frame
    _                => unreachable!(),
}
#+end_example
...
#[derive(Clone, Copy)]
enum Op {
    Axiom(u16),            // index into static axiom table
    Derived { start: u32, len: u16 }, // slice in code space
    Sym(InternId),         // unresolved (only appears before patching)
}

fn resolve_op(dict: &HashMap<InternId, Word>, op: &mut Op) {
    if let Op::Sym(id) = *op {
        match dict.get(&id).expect("unknown word") {
            Word::Axiom(ix)     => *op = Op::Axiom(*ix),
            Word::Derived(sl)   => *op = Op::Derived { start: sl.0, len: sl.1 },
        }
    }
}
#+end_example

Do =resolve_op= once per token when you load the source, or lazily the
first time you hit the =Sym=.

Cost per execution: *0 hash-map look-ups* -- just the =match=.

***** Level 2 Inline cache for late binding / redefinition
:PROPERTIES:
:CUSTOM_ID: level-2-inline-cache-for-late-binding-redefinition
:END:
If the language lets the user do

#+begin_example
Copy code: dup2 dup dup ;
...
add a /version counter/ to the dictionary:
...
struct OpCache { version: u32, op: Op }   // sits in the byte-code
#+end_example
...
#+begin_example rust
match prog[ip] {
    Op::Cached(c) if c.version == dict[c.sym].version => exec(c.op),
    Op::Sym(sym) | Op::Cached(c) /*stale*/            => {
        let entry = &dict[sym];
        let new_op = entry.kind.to_op();
        prog[ip] = Op::Cached(OpCache{version:entry.version, op:new_op});
        exec(new_op);
    }
}
#+end_example

A self-modifying inline cache: the extra branch predicts well and the
cost when the definition is /not/ redefined is still a single
indirect-call for axioms / direct call for quotations.

***** Level 3 Ahead-of-time “compile” to a register VM
:PROPERTIES:
:CUSTOM_ID: level-3-ahead-of-time-compile-to-a-register-vm
:END:
Later, when you add peephole optimisations, you can transform the whole
token stream into a compact numeric byte-code with register operands. At
that point the string → op translation happens once during “compilation”
and the runtime never even sees a =HashMap=.

But you can reach CPython-class performance already with level 1.

--------------

****** What about memory?
:PROPERTIES:
:CUSTOM_ID: what-about-memory
:END:
=Op::Axiom(u16)= = 4 bytes\\
=Op::Derived(u32,u16)= = 8 bytes\\
=Op::Sym(u32)= = 4 bytes (until patched)

If you store 100 000 words the opcode tape is < 1 MB.

--------------

****** Putting it in your current code
:PROPERTIES:
:CUSTOM_ID: putting-it-in-your-current-code
:END:
1. Replace =Item::Symbol(InternId)= in the program list with the =Op=
   enum above.

2. When the interpreter sees =Op::Sym(id)= call =resolve_op= (or do it
   in the loader).

3. Built-in words live in a static table:

   #+begin_example rust
type AxiomFn = fn(&mut VM) -> Result<(), Error>;
   const AXIOMS: &[AxiomFn] = &[ add, sub, dup, drop, … ];
   #+end_example

4. User definitions get stored once in =Vec<Op>= and referenced by slice
   (=start,len=).

You eliminate the per-step =HashMap= cost for both built-ins and user
words while keeping late binding and redefinition semantics.

[[data:image/svg+xml;base64,]]

[[data:image/svg+xml;base64,]]

*** TODO Flat enum hierarchy

** INPROGRESS Make juggle etc based on destructure
- State "INPROGRESS" from "TODO"       [2025-09-26 Fri 10:38]
Juggle is basically "destructure, run prog, restructure" on a nested map.

So the first part we already have. The last part, is probably NOT best
handled by pack, because we're not really creating a new structure,
we're probably updating the values in the existing one. -- Wait wtf
was i smoking here. yes we're updating but we're calculating values on
the stack. So yes we're just assigning values back. But the important
part is *not necessarily the same places we took from*. So we can't just
reuse the same template.

So we have a nested map and ... a list of pairs of [path value].

So let's say the args are:
+ the nested map original
+ the pattern to unpack with
+ a program to run
+ a list of paths.

So we run the unpack, then the program, then label the resulting
values and then we step through with assignment.
#+begin_src kcats
    [[a [[b [[c 1]]]]] [e f] [g 2]] association
    [[a [[b [[c 1️⃣]]]]] [g 2️⃣]] association
    [+ 👥]
    [[a z] [h]]
    [[•👥 #️⃣⛏️] •🪄
    [▶️] 🪄 label] 🛡️ ▶️ •🗑️ •🗑️ •🗑️ ;; ensure prog doesn't clobber map
    [🍫 assign] 🪜 ▶️
...
older: 
A simple way to handle it might be:

#+begin_src kcats
  [[a [[b [[c 1]]]]] [e f] [g 2]] association
  [[[a [[b [[c 1️⃣]]]]] [g 2️⃣]] association #️⃣⛏️
  [+]  ▶️
  ] 🛡️  ▶️
  [a x] 🔀 assign
#+end_src
...
it would be nice if pack actually did updates instead of pure assign?
Actually i don't think that's pack's job - it's just templating. To
update a nested structure we can take pairs of =[path program]= and iterate:

#+begin_src kcats
  [[a [[b [[c 1]]]]] [e f] [g 2]] association
  [[[a b c] [inc]] [[g] [3 *]] [[a c] [25]]]
  [🍫 update] 🪜 ▶️  
#+end_src
...
and we can even label values

** CANCELED Generating with an empty 'each' program causes infinite loop
- State "CANCELED"   from "TODO"       [2025-10-22 Wed 07:43] \\
  Not a bug: the issue was the generator was never producing [], not a problem with =each=
#+begin_src kcats
[[1 2] [2 2] [3 3]] liberator [] each collect
#+end_src
...
** TODO Invalid spec not caught
a spec of [program program] is not flagged as invalid. Needs to be a
pair of lists.

** INPROGRESS Change parse semantics to always return chunks
- State "INPROGRESS" from "TODO"       [2025-10-22 Wed 18:35]
The problem is when you try to chain parsers, there's a mismatch:
parsers want to ingest chunks of data (rather than say, single
characters or bytes) but then they produce generators of single items,
which can't be directly hooked up to another parser.

Solution: parser generators always return chunks. If the desired end
result is single item generator, you can apply the 'joiner' transform
(which currently isn't implemented but should be straightforward).
...
🔳 [[🔳] [🔳 [▶️] ••🐋] ↔️ [🔀 [👥] ••🐋 ▶️ [[🔳] [🔳 [▶️] ••🐋] ↔️] [🔀 [👥] ••🐋 ▶️] 🔳 [▶️]
   ...
   [[🔳] 🔀]
   ↔️]
🔳 " [ba" [parse-edn] [📤 [[📤] 🔳 [🔳] [🔀] 🪆 🔀] [🔳] ↔️] ["z] [quux" "] [[this] " "[that]]"
                                                     "[foo" "]"]
#+end_src

** DONE Implement 'joiner'
- State "DONE"       from "TODO"       [2025-10-22 Wed 18:35]
A generator that ingests list[item] and generates item.
...
#+begin_src kcats
  ...
  [🔀]
  🪆 collect
  ;; does NOT work with empty items in the list, but this is just how
  ;; generators work, nothing values are not supported. But this can
  ;; cause unexpected results with polling-type IO, which should not
  ;; exist in the stdlib (eg reading from a socket should block until
  ;; there's data)
#+end_src
...
#+end_src
#+begin_src kcats
 [1 2 +] stage stepper generator [[dictionary] unassign] each collect
#+end_src
...
does until need to keep pred result? seems like it can only be [].

let's fix that
** DONE Until doesn't need to keep pred result
- State "DONE"       from "INPROGRESS" [2025-10-22 Wed 17:34]
- State "INPROGRESS" from "TODO"       [2025-10-22 Wed 12:18]
it's always [] due to the negation logic, so we don't need to pass it to the body.
...
11
#+end_src
** INPROGRESS parse-edn should deliver chunks too
- State "INPROGRESS" from "TODO"       [2025-10-23 Thu 07:49]
** DONE Make axiom words testable separately
- State "DONE"       from "INPROGRESS" [2025-12-14 Sun 22:33]
- State "INPROGRESS" from "TODO"       [2025-12-12 Fri 19:17]
When these are broken, we shouldn't bother with the rest
** INPROGRESS Module loading by alias fixes
- State "INPROGRESS" from "TODO"       [2025-12-20 Sat 16:58]
#+begin_src kcats
  dictionary
  [[errors ✅]
   [crypto []]]
...
#+RESULTS:
#+begin_src kcats
[[modules [#b64 "core" #b64 "Vb9Om_94ZoKOutE83MbeH5EXoGWDjqrs9_xtGg8f4kM" #b64 "bCR4VmWPLte5CjqODcEHz4w01LoeKR-DfxyA0IAZ4Ro"
           #b64 "trOmyrhUW8QpePNjKQ6RGMkRl2Sd1u-lNrSL4Yg-5yc" #b64 "iNTmQssX0VW551Vnkcf_68I8elrtvUq2Xc7ZAupceDo"
           #b64 "PcFAzMQBF-ouey_2Oln7hv8ewah8EBFLH2W3u1cNIvg" #b64 "WIAWT-eHEuTY4Kvqg7t2FnOj0C784Wju266OzQGqSe8"
           #b64 "MOxo0jXIaLxOqyNpeD2PoF91dEs4Ez7nYbR89h4OlSQ" #b64 "Vb9Om_94ZoKOutE83MbeH5EXoGWDjqrs9_xtGg8f4kM"]]
 [words 286_entries]]
#+end_src

Gemini suggest fix
#+begin_src markdown
Yes, this approach works perfectly and satisfies all your constraints:

1.  **Security:** The specific (Name, Hash) pairing is enforced.
2.  **No Manual Sync:** `build.rs` does the dirty work.
3.  **Brittleness Fix:** The Runtime Merge step acts as a "Linker". If Rust has a function that defines `+`, but `build.rs` didn't find `+` in any EDN file, the linker panics at startup.

Here is the implementation strategy.

### 1. `build.rs` (The Mapper)

We need `build.rs` to scan your EDN files, parse out the function names, calculate the file hash, and generate a static map of `Function Name -> File Hash`.

*Assumption: Your function names are globally unique across the stdlib. If `+` appears in two different files, your current `BUILTIN_FUNCTIONS` flat map wouldn't work anyway.*
...
fn main() {
    ...
    let dest_path = Path::new(&out_dir).join("stdlib_manifest.rs");
    let mut f = fs::File::create(&dest_path).unwrap();

    let src_dir = Path::new("src/kcats/stdlib"); // Your path
    
    // We will generate a map: "function_name" -> "file_hash"
    writeln!(f, "pub fn get_manifest() -> std::collections::HashMap<&'static str, &'static str> {{").unwrap();
    writeln!(f, "    let mut m = std::collections::HashMap::new();").unwrap();

    for entry in fs::read_dir(src_dir).unwrap().flatten() {
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()) == Some("joy") { // or .edn
            
            let content = fs::read_to_string(&path).unwrap();
            
            // 1. Calculate Hash of the file content
            // (Using whatever caching logic you have)
            ...
            let names_found: Vec<String> = parse_names_from_edn(&content);

            for name in names_found {
                writeln!(f, "    m.insert(\"{}\", \"{}\");", name, file_hash).unwrap();
            }
        }
    }
...
```rust
// stdlib_manifest.rs
pub fn get_manifest() -> HashMap<&'static str, &'static str> {
    let mut m = HashMap::new();
    m.insert("postbox", "QmHashA...");
    m.insert("put", "QmHashA...");
    m.insert("+", "QmHashB...");
    m
}
```

### 2. Runtime (The Linker)

Now, inside your interpreter initialization, you merge your existing `BUILTIN_FUNCTIONS` (which has the logic) with the `manifest` (which has the security context).

```rust
// src/stdlib.rs or main.rs

// Import the generated manifest
include!(concat!(env!("OUT_DIR"), "/stdlib_manifest.rs"));

pub struct NativeRegistry {
    // Key: (FileHash, FunctionName) -> FunctionPointer
    // This is the secure table.
    registry: HashMap<(String, String), dict::Executable>,
}

impl NativeRegistry {
    pub fn build(builtins: &HashMap<Word, dict::Executable>) -> Self {
        let manifest = get_manifest(); // From generated code
        let mut registry = HashMap::new();

        for (word, exec) in builtins {
            let func_name = word.as_str(); // Assuming Word can give us &str

            // THE LINKER CHECK
            // We look up where this function *should* live according to the source files.
            match manifest.get(func_name) {
                Some(file_hash) => {
                    // Success! We found the file that claims this function.
                    // We lock this function pointer to THAT hash only.
                    registry.insert(
                        (file_hash.to_string(), func_name.to_string()), 
                        exec.clone() // or copy/reference
                    );
                },
                None => {
                    // BRITTLENESS CHECK:
                    // Rust defines a function, but no EDN file claims it.
                    // This panics at startup so you know you broke the build.
                    panic!("Orphaned Native Function: '{}' is defined in Rust BUILTIN_FUNCTIONS but not found in any stdlib file.", func_name);
                }
            }
        }
        
        // REVERSE CHECK (Optional):
        // Did we miss implementing something in Rust that the EDN expects?
        for (name, _) in manifest {
            // Logic depends on your Word type, simplified here:
            if !builtins.keys().any(|w| w.as_str() == *name) {
                println!("WARNING: stdlib defines '{}' but Rust has no implementation.", name);
            }
        }
    ...
    // This is called when the Interpreter is running a specific module
    pub fn resolve(&self, callers_file_hash: &str, func_name: &str) -> Option<&dict::Executable> {
        self.registry.get(&(callers_file_hash.to_string(), func_name.to_string()))
    }
}
```
...
1.  **Dangling Pointers (Brittleness):**
    If you rename `postbox` to `mailbox` in the text file, `manifest.get("postbox")` returns None. The `panic!("Orphaned Native Function...")` fires immediately when you run the project. You are forced to update the Rust string to match.

2.  **Security/Sandboxing:**
    If a user loads a script `malicious.joy` (Hash: `QmEvil`), and tries to call `postbox`:
    *   Interpreter asks: `registry.resolve("QmEvil", "postbox")`.
    *   Registry looks for key key `("QmEvil", "postbox")`.
    *   It only has `("QmHashA", "postbox")`.
    *   Returns `None`.
    *   The malicious script cannot access the raw memory pointer, even though it used the correct name.

3.  **Manual Work:**
    You only maintain `BUILTIN_FUNCTIONS` (which you have to do anyway to write the code) and the EDN files. The mapping of "Which file owns what" is automated.
#+end_src
*** TODO Inscribe should return module hash
Just in case the caller wants to add it to the resolution list
#+begin_src kcats
  dictionary [errors] 🍫 decache
  [[hashbytes] ;; calculate module hash
   [string read [words] 🔀 update]] ;; install the module in the dictionary
  [▶️] 🚜 ▶️ ;; fork 
  •🗑️ 🍫 •🔀 ••👥 dictmerge
...
[[modules [#b64 "core"]]
 [words 282_entries]]
#b64 "Vb9Om_94ZoKOutE83MbeH5EXoGWDjqrs9_xtGg8f4kM"
#+end_src
** TODO Make dict modules a sorted set.
#+begin_src toml
# In your Cargo.toml
[dependencies]
indexmap = "2.0"
...
fn main() {
    // --- OPTION 1: Insertion Order (indexmap crate) ---
    println!("--- IndexSet (Insertion Order) ---");
    let mut insertion_set = IndexSet::new();
    
    // 1. Basic insertion preserves order
    insertion_set.insert("Apple");
    insertion_set.insert("Banana");
    insertion_set.insert("Carrot");
    
    println!("Original: {:?}", insertion_set); 
    // Output: {"Apple", "Banana", "Carrot"}

    // 2. Handling Duplicates (The "No-op" scenario)
    // By default, inserting a duplicate returns false and changes/moves nothing.
    let is_new = insertion_set.insert("Apple");
    println!("Inserted duplicate 'Apple'? {}", is_new); // false
    println!("After duplicate: {:?}", insertion_set);
    // Output: {"Apple", "Banana", "Carrot"} (Apple is still at index 0)

    // 3. Handling Duplicates (The "Move to end" scenario)
    // To move a duplicate to the end, you must remove strict constraint 
    // by removing the old one first.
    if insertion_set.contains("Banana") {
        insertion_set.shift_remove("Banana"); // distinct from swap_remove
        insertion_set.insert("Banana"); // Re-insert at the end
    }
    println!("Moved Banana: {:?}", insertion_set);
    // Output: {"Apple", "Carrot", "Banana"}


    // --- OPTION 2: Sorted Order (Standard Library) ---
    ...
    // It ignores insertion order and sorts by value
    println!("Sorted: {:?}", sorted_set);
    // Output: {1, 2, 3}
}
#+end_src
** TODO Error handling design
*** Overview
I need a type solution where we get the properties we want in a way that isn't awkward to use.
*** Requirements
+ When the caller wants a fast conversion attempt from type A to B
  (he's probing to see what it can be converted to), if it fails he
  gets a thin wrapper around the instance of A very quickly
+ When the caller wants a detailed conversion (the user needs to know
  what went wrong if it fails), he can get a detailed Error
+ Conversions can be nested. Conversoins should do the right thing on
  the inner values - if the outer type is supposed to be a fast
  conversion, the inner value should do the same. And if it's supposed
  to be detailed, the inner failure should be passed back up so the
  user knows where it went wrong.
+ If possible avoid code duplication - the two different types of
  conversion both involve simlilar match statements. but re-use is
  difficult because you have to keep track of the original callers
  intent, if you have detailed impl call the fast one (for example).
+ The caller should be able to specify which method (fast/detailed) to
  use just by the return type, if possible. Separate methods are not
  as good, but acceptable. It should be possible to pass the target
  type down to inner conversions.
+ Trait objects might be ok, as long as the trait handles everything
  we might do with the error later: print it out, get the cause,
  convert it back to an Item etc. 
