...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl DemotingTake for Sized {
      type Output = Self;
      type Item = Item;
      fn take_demoting(self) -> (Option<Self::Item>, Self::Output) {
          //println!("Taking! {:?}", self);
          match self {
              Sized::List(mut l) => ((&mut l).take_simple(), Sized::List(l)),
              Sized::String(mut s) => ((&mut s).take_simple().map(Item::derive), Sized::String(s)),
              Sized::Bytes(mut b) => ((&mut b).take_simple().map(Item::derive), Sized::Bytes(b)),
              Sized::Set(mut s) => ((&mut s).take_simple(), Sized::Set(s)),
              Sized::Program(mut p) => ((&mut p).take_simple(), Sized::Program(p)),
              Sized::Assoc(mut a) => ((&mut a).take_simple().map(Fit::fit), Sized::Assoc(a)),
              s => {
                  let mut a = assoc::Associative::try_derive(s.clone()).unwrap();
  ...
  impl TryDerive<Sized> for List {
      fn try_derive(s: Sized) -> Result<Self, Error> {
          match s {
              ...
              i => Err(Error::expected(fit!("list"), i)),
          }
      }
  }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn is_association(i: Item) -> bool {
      Result::<assoc::Associative, DeriveError<Item>>::derive(i).is_ok()
  }
