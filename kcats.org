...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  pub trait RewrapExt: ToIterator + Sized {
      ...
      fn try_rewrap<Target, Inner, M: Mode>(self) -> Result<Target, M::Detail<Self>>
      where
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  impl<M: Mode> TryDerive<Item, M> for Associative {
      fn try_derive(i: Item) -> Result<Associative, M::Detail<Item>> {
          match i {
...
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  ...
  pub trait Mode: Sized + 'static {
      ...
      fn expected<Bounce>(expected: Word, src: Bounce) -> Self::Detail<Bounce>;
      fn nest_error<Bounce, BounceInner>(
          outer: Self::Detail<Bounce>,
          inner: Self::Detail<BounceInner>,
      ) -> Self::Detail<Bounce>;
  }
  ...
  impl Mode for Fast {
      ...
      #[inline(always)]
      fn expected<Src>(_expected: Word, src: Src) -> Src {
          src
      }

      #[inline(always)]
      fn nest_error<Src, Inner>(outer: Src, inner: Inner) -> Src {
  ...
  impl Mode for Detailed {
      ...
      fn expected<I: Fit<Item>>(expected: Word, src: I) -> Error {
          //TODO: get rid of default
          Error::expected(expected, src)
      }

      fn nest_error<Err, _Err>(outer: Error, inner: Error) -> Error {
          outer.nest(inner)
      }
  }
  ...
  impl Mode for Infallible {
      type Detail<X> = Infallible;

      fn expected<X>(_expected: Word, _src: X) -> Infallible {
          unreachable!("supposed to be infallible")
      }

      fn nest_error<X>(_outer: Infallible, _inner: Infallible) -> Infallible {
          unreachable!("supposed to be infallible")
      }
  }
  ...
  pub trait TryDerive<Src, M: Mode = Detailed>: Sized {
      fn try_derive(value: Src) -> Result<Self, M::Detail<Src>>;
  }
  ...
  pub trait TryFit<Target, M: Mode = Detailed>: Sized {
      fn try_fit(self) -> Result<Target, M::Detail<Self>>;
  }
  ...
  impl<Src, Target, M: Mode> TryFit<Target, M> for Src
  where
  ...
  impl<T, M: Mode> TryDerive<T, M> for T
  where
      T: Identity,
  {
      fn try_derive(value: T) -> Result<T, M::Detail<T>> {
          Ok(value)
      }
  }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  /// stack item.
  fn f_stack1<F, Arg1, Output, ArgMode: Mode, Err>(
      f: F,
