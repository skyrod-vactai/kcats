...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  pub struct DeriveError<T> {
      pub src: T,
      pub expected: types::Word,
  }

  impl<T> Debug for DeriveError<T>
  where
      T: Debug,
  {
      fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
          write!(
              f,
              "DeriveError[src: {:?}, expected: {:?}]",
              self.src, self.expected
          )
      }
  }
  impl<T> DeriveError<T> {
      pub fn expected(expected: types::Word, src: T) -> DeriveError<T> {
          DeriveError { src, expected }
      }

      pub fn swap<U>(self, updated: U) -> DeriveError<U> {
          DeriveError {
              src: updated,
  ...
  pub trait TryDerive<T>: Sized {
      fn try_derive(value: T) -> Result<Self, Error>;
  }
  ...
  impl<I> Derive<DeriveError<I>> for Error
  where
      I: Fit<types::Item>,
  {
      fn derive(de: DeriveError<I>) -> Error {
          Error::expected(de.expected, Some(de.src.fit()))
      }
  }

  impl<T, I> Derive<Result<T, DeriveError<I>>> for Result<T, Error>
  where
      I: Fit<types::Item>,
  {
      fn derive(r: Result<T, DeriveError<I>>) -> Self {
          r.map_err(Error::derive)
      }
  }

  /// Convert Error to DeriveError
  impl Derive<Error> for DeriveError<types::Item> {
      fn derive(e: Error) -> DeriveError<types::Item> {
          DeriveError::expected(fit!("fixme"), e.actual().cloned().unwrap_or_default())
      }
  }
  ...
  pub trait TryDeriveIterator<A>: Sized {
      ...
      where
          I: IntoIterator<Item = types::Item>;
  }
...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  pub type Ftr<'a, T> = Pin<Box<dyn std::future::Future<Output = T> + Send + 'a>>;
  pub enum Future<'a, T> {
      Now(T),
      Future(Ftr<'a, T>),
  }
  ...
  pub type StepFn = dyn Fn(env::Environment) -> Future<'static, env::Environment> + Sync + Send;
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  pub trait Take {
      type Output;
      type Item;
      fn take(self) -> Future<'static, (Result<Option<Self::Item>, Error>, Self::Output)>;
  }
      ...
      T: SimpleTake + Send + 'static,
  {
      ...
      fn take(mut self) -> Future<'static, (Result<Option<Self::Item>, Error>, Self::Output)> {
          let item = self.take_simple();
          Future::Now((Ok(item.map(Fit::fit)), self))
      }
  }
      ...
      fn has(&self, other: &Item) -> bool {
          ...
          match (self, other) {
              ...
              (s, other) => {
                  ...
                  //}
                  let c =
                      Result::<assoc::Associative, DeriveError<Sized>>::derive(s.clone()).unwrap();
      ...
      fn take(self) -> Future<'static, (Result<Option<Self::Item>, Error>, Self::Output)> {
          match self {
              Dispenser::Sized(mut s) => {
                  ...
                  Future::Now((Ok(v), Dispenser::Sized(s)))
              }
              Dispenser::Out(mut o) => Future::Future(Box::pin(async move {
                  (o.take_future().await, Dispenser::Out(o))
              })),
              Dispenser::Tunnel(mut t) => Future::Future(Box::pin(async move {
  ...
  impl Sized {
      ...
      pub fn pop(self) -> (Self, Option<Item>) {
          match self {
              ...
              s => {
                  ...
                  let mut c = Result::<assoc::Associative, DeriveError<Sized>>::derive(s).unwrap();
                  let x = c.take_simple();
                  (Sized::derive(c), x)
              }
          }
      ...
      pub fn put(&mut self, other: Item) -> Result<(), Error> {
          match (self, other) {
              (Sized::List(c), i) => {
                  c.push_back(i);
                  Ok(())
              }
              (Sized::Set(ref mut s), i) => {
                  s.mutate().insert(assoc::KeyItem::try_derive(i)?);
                  Ok(())
              }
              (Sized::Bytes(ref mut b), Item::Int(i)) => {
                  b.push(i as u8);
                  Ok(())
              }
              (Sized::Bytes(_), i) => Err(Error::expected(fit!("integer"), i)),
              (Sized::String(s), Item::Char(c)) => {
                  s.push(c);
                  Ok(())
              }
              (Sized::String(_), i) => Err(Error::expected(fit!("char"), i)),
              (Sized::Program(ref mut p), i) => {
                  p.extend(list![i]);
                  Ok(())
              }
              (s, i) => {
                  let mut c = assoc::Associative::try_derive(s.clone())?;
                  c.put(i)
              }
          }
      }
  ...
  impl Receptacle {
      ...
      /// the end.
      pub fn put(&mut self, i: Item) -> Future<Result<(), Error>> {
          match self {
              Receptacle::Sized(s) => Future::Now(s.put(i)),
              Receptacle::In(ref mut p) => p.put(i),
              Receptacle::Tunnel(ref mut t) => t.put(i),
          }
      }
  }
      ...
      fn into_iter(self) -> Self::IntoIter {
          match self {
              ...
              s => {
                  ...
                  let map = Result::<assoc::Associative, DeriveError<Sized>>::derive(s).unwrap();
  ...
  impl Derive<Item> for Result<Sized, DeriveError<Item>> {
      fn derive(i: Item) -> Self {
          match i {
              ...
              i => Err(DeriveError::expected(fit!("sized"), i)),
              //try_convert_enum!(item, Item => Sized, [Bytes, String, List, Associative, Set, Program], |i| Self::DeriveError::expected(fit!("sized"), i) )
          }
      }
  }
      ...
      impl Serialize for Sized {
              ...
              S: serde::Serializer,
          {
              match self {
                  ...
                  s => {
                      let a = Result::<assoc::Associative, DeriveError<Sized>>::derive(s.clone())
                          .unwrap();
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
  ...
  impl coll::Join<Association> for coll::List {
      ...
      fn join(self, other: Association) -> Result<Self::Output, Self::Error> {
          let la = Association::try_from_iter(self.iter().cloned())?;
  ...
  impl Associative {
      ...
      pub fn insert(&mut self, k: KeyItem, v: Item) -> Option<Item> {
          //println!("Insert! {:?}", self);

          //swap the original slot so we
          // have an owned value to work with, just make sure to swap it
          // back before finishing. This lets us replace with other
          // variants if necessary.
          let mut a = std::mem::replace(self, Associative::Nothing);

          match a {
              Associative::Assoc(mut a) => {
                  let am = coll::Arc::mutate(&mut a);
                  let replaced = am.insert(k, v);
                  ,*self = Associative::Assoc(a);
                  replaced
              }
                              ...
                              "stack" => {
                                  ...
                                  match l {
                                      Ok(l) => {
                                          ...
                                          e.stack = l.fit();
                                          ,*self = Associative::Env(e);
                                          Some(old.fit())
                                      }
                                      Err(_) => {
                                          let (a2, i) = demote(*e, k, v);
                                          ,*self = a2;
                                          i
                                      }
                                  }
              ...
              Associative::DictEntry(ref mut de) => match k {
                  KeyItem::Word(ref w) => {
                      let w: &str = w.fit();
                      if w == "definition" {
                          ...
                          match l {
                              Ok(l) => {
                                  de.examples = Some(l);
                                  None // TODO: return the old examples
                              }
                              Err(_) => None,
                          }
                      } else if w == "spec" {
              ...
              Associative::Dictionary(ref mut d) => match k {
                  KeyItem::Word(ref w) => {
                      let w: &str = w.fit();
                      if w == "words" {
                          ...
                          match e {
                              Ok(words) => {
                                  d.words = words;
                                  None // TODO: return the old entries
                              }
                              Err(_) => None,
                          }
                      } else if w == "modules" {
                          ...
                          match l {
                              Ok(modules) => {
                                  d.modules = modules;
                                  d.resolve();
                                  None // TODO: return the old modules
                              }
                              Err(_) => None,
                          }
                      } else {
      ...
      pub fn put(&mut self, other: Item) -> Result<(), Error> {
          match (self, other) {
              (Associative::Words(ref mut this), other) => {
  ...
  impl TryDerive<coll::Sized> for Associative {
      fn try_derive(s: coll::Sized) -> Result<Self, Error> {
          Result::<Associative, DeriveError<coll::Sized>>::derive(s).map_err(Error::derive)
      }
  }

  impl Derive<Item> for Result<Associative, DeriveError<Item>> {
      fn derive(i: Item) -> Result<Associative, DeriveError<Item>> {
          match i {
              ...
              Item::List(l) => Association::try_from_iter(l.iter().cloned())
                  .map(Associative::Assoc)
                  .map_err(DeriveError::derive),
              i => Err(DeriveError::expected(fit!("associative"), i)),
          }
      }
  }

  impl Derive<coll::Sized> for Result<Associative, DeriveError<coll::Sized>> {
      fn derive(i: coll::Sized) -> Result<Associative, DeriveError<coll::Sized>> {
          match i {
              ...
              coll::Sized::List(l) => Association::try_from_iter(l.iter().cloned())
                  .map(Associative::Assoc)
                  .map_err(|_| DeriveError::expected(fit!("associative"), coll::Sized::List(l))),
              i => Err(DeriveError::expected(fit!("associative"), i)),
          }
      }
  }
  ...
  impl TryDerive<Item> for Associative {
      fn try_derive(i: Item) -> Result<Self, Error> {
          Result::<assoc::Associative, DeriveError<Item>>::derive(i).map_err(Error::derive)
      }
  }
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  impl<T> Derive<DeriveError<T>> for Item
  where
      T: Fit<Item>,
  {
      fn derive(e: DeriveError<T>) -> Item {
          //return an Error
          Item::Error(e.fit())
      }
  }

  impl<T> From<DeriveError<T>> for Item
  where
      T: Fit<Item>,
  {
      fn from(e: DeriveError<T>) -> Item {
          //return an Error
          Item::Error(e.fit())
      }
  }

  impl<T> From<DeriveError<T>> for Error
  where
      T: Fit<Item>,
  {
      fn from(e: DeriveError<T>) -> Error {
          //return an Error
          Error::expected(e.expected.fit(), e.src.fit())
      }
  }
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  impl Derive<Environment> for Future<'static, Environment> {
      fn derive(env: Environment) -> Future<'static, Environment> {
          Future::Now(env)
      }
  }
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  /// stack item.
  fn f_stack1<F, Output, SpecInput, E>(f: F) -> impl Fn(Environment) -> Future<'static, Environment>
  where
  ...
  fn f_stack2<F, Output, SpecInputY, SpecInputX, E>(
      f: F,
  ) -> impl Fn(Environment) -> Future<'static, Environment>
  where
  ...
  fn f_stack2_async(
      f: fn(Item, Item) -> Future<'static, ItemResult>,
  ) -> impl Fn(Environment) -> Future<'static, Environment> {
      move |mut env: Environment| {
          let x = env.pop();
          let y = env.pop();
          match f(x, y) {
              ...
              Future::Future(r) => Future::Future(Box::pin(r.map(|r| {
                  if r.is_ok() {
                      env.pop_prog();
                  }
                  env.push(r);
  ...
  fn f_result<F>(f: F) -> impl Fn(Environment) -> Future<'static, Environment>
  where
  ...
  pub fn put(mut env: Environment) -> Future<'static, Environment> {
      let i = env.pop();
      match coll::Receptacle::try_derive(env.pop()) {
          Ok(mut p) => {
              ...
              match r {
                  ...
                  Future::Future(fu) => Future::Future(Box::pin(fu.map(|f| {
                      match f {
                          Ok(_) => {
                              env.pop_prog();
                              env.push(Item::derive(p))
  ...
  pub fn clone(mut env: Environment) -> Future<'static, Environment> {
  ...
  fn swap2(mut env: Environment, offset: usize) -> Future<'static, Environment> {
      env.stack.swap(offset, offset + 1);
      env.fit()
  }

  pub fn swap(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
      swap2(env, 0)
  }

  pub fn swapdown(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
      swap2(env, 1)
  }

  pub fn swapdeep(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
      swap2(env, 2)
  }

  pub fn sink(mut env: Environment) -> Future<'static, Environment> {
      env.stack.swap(0, 2);
      env.stack.swap(0, 1);
      env.pop_prog();
      env.fit()
  ...
  pub fn float(mut env: Environment) -> Future<'static, Environment> {
      env.stack.swap(0, 2);
      env.stack.swap(1, 2);
      env.pop_prog();
      env.fit()
  ...
  pub fn drop(mut env: Environment) -> Future<'static, Environment> {
      env.pop();
      env.pop_prog();
      env.fit()
  }

  pub fn eq(mut env: Environment) -> Future<'static, Environment> {
      let i = env.pop();
      let j = env.pop();
      env.pop_prog();
      env.push(i == j);
  ...
  pub fn execute(mut env: Environment) -> Future<'static, Environment> {
      let i = env.pop();
  ...
  pub fn wrap(mut env: Environment) -> Future<'static, Environment> {
      let item = env.pop();
      env.pop_prog();
      env.push(list!(item));
      env.fit()
  ...
  pub fn unwrap(mut env: Environment) -> Future<'static, Environment> {
  ...
  pub fn dip(mut env: Environment) -> Future<'static, Environment> {
      match coll::List::try_derive(env.pop()) {
          Ok(program) => {
  ...
  pub fn dipdown(mut env: Environment) -> Future<'static, Environment> {
      match coll::List::try_derive(env.pop()) {
          Ok(program) => {
  ...
  pub fn take(mut env: Environment) -> Future<'static, Environment> {
      ...
      match coll::Dispenser::try_derive(env.pop()) {
          Ok(d) => match d.take() {
              ...
              Future::Future(r) => Future::Future(Box::pin(async move {
                  let (i, c) = r.await;
                  finish(env, i, c)
              })),
          },
  ...
  pub fn pop(mut env: Environment) -> Future<'static, Environment> {
  ...
  pub fn branch(mut env: Environment) -> Future<'static, Environment> {
      match (
  ...
  pub fn step(mut env: Environment) -> Future<'static, Environment> {
      fn finish(
          mut env: Environment,
      ...
      match dispenser.take() {
          Future::Now((r, dispenser)) => finish(env, r, dispenser, p).fit(),
          Future::Future(f) => Future::Future(Box::pin(async move {
  ...
  pub fn evert(mut env: Environment) -> Future<'static, Environment> {
  ...
  pub fn snapshot(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
  ...
  fn assoc_in(i: Option<Item>, ks: &[assoc::KeyItem], v: Item) -> Result<Item, Error> {
      ...
      fn assoc_map(
          ...
          if ks.is_empty() {
              ...
              a.insert(k.clone(), assoc_in(inner, ks, v)?);
              Ok(a.fit())
          }
      }
  ...
  fn remove(i: coll::Sized, k: Item) -> Result<Item, Error> {
      match i {
          ...
          coll::Sized::Env(e) => {
              let a = assoc::Association::derive_iter(*e);
  ...
  pub fn eval_step(mut env: Environment) -> Future<'static, Environment> {
      //println!("{:?}", env.stack);
  ...
  pub fn eval_step_outer(mut env: Environment) -> Future<'static, Environment> {
      let tos = env.pop();
          ...
          Ok(inner) => {
              ...
              if inner.program.is_empty() {
                  ...
                  match eval_step(inner) {
                      ...
                      Future::Future(finner) => Future::Future(Box::pin(finner.map(|e| {
                          env.push(e);
                          env
                      }))),
                  }
  ...
  pub fn evaluate(mut env: Environment) -> Future<'static, Environment> {
      match Environment::try_derive(env.tos().unwrap()) {
          Ok(inner) => Future::Future(Box::pin(eval(inner).map(|inner_done| {
              env.pop_prog();
              env.pop();
              env.push(inner_done);
              env
  ...
  pub fn dictionary(mut env: Environment) -> Future<'static, Environment> {
  ...
  /// no-op.
  fn handle(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
  ...
  pub fn self_insert(mut env: Environment) -> Future<'static, Environment> {
      let t = env.pop_prog();
      env.push(t);
      env.fit()
  }
  ...
  pub fn animate(mut env: Environment) -> Future<'static, Environment> {
      let tos = env.pop();
      let inner_env = Environment::try_derive(tos);
      match inner_env {
          Ok(inner) => {
  ...
  pub fn timestamps(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
      env.push(Item::Time);
      env.fit()
  }

  pub fn standard(mut env: Environment) -> Future<'static, Environment> {
      env.pop_prog();
  ...
  fn unnamespace(mut env: Environment) -> Future<'static, Environment> {
  ...
  fn resolve(mut env: Environment) -> Future<'static, Environment> {
      if let Some(i) = env.stack.pop_front() {
          match Word::try_derive(i) {
              Ok(w) => {
                  env.pop_prog();
  ...
  fn unpack(env: &mut Environment) -> Result<(), Error> {
      ...
      fn destructure(
          ...
          let data: Result<coll::Sized, DeriveError<Item>> = data.clone().fit();
          let pattern: Result<coll::Sized, DeriveError<Item>> = pattern.clone().fit();
          match (data, pattern) {
              (Ok(data), Ok(pattern)) => {
                  // We got containers
                  match (data, pattern) {
                      ...
                      (a, b) => {
                          ...
                          match (
                              Result::<assoc::Associative, DeriveError<coll::Sized>>::derive(a),
                              Result::<assoc::Associative, DeriveError<coll::Sized>>::derive(b),
                          ) {
                              (Ok(data), Ok(pattern)) => {
...
#+begin_src rust :tangle src/types/container/pipe.rs :mkdirp yes
  ...
  impl In {
      ...
      pub fn put(&mut self, i: Item) -> types::Future<Result<(), Error>> {
          match self {
              In::StaticFile(f) => {
                  let f = f.clone();
                  types::Future::Future(Box::pin(async move { f.write().await.put(i).await }))
              }
              In::Socket(f) => {
                  let f = f.clone();
                  types::Future::Future(Box::pin(async move { f.write().await.put(i).await }))
              }
              In::Handoff(ref mut h) => h.put(i), //_ => Err(Error::expected("foo")),
  ...
  impl Tunnel {
      ...
      /// the tunnel is full.
      pub fn put(&mut self, i: Item) -> types::Future<'static, Result<(), Error>> {
          match self {
              Tunnel::StaticFile(f) => {
                  let f = f.clone();
                  types::Future::Future(Box::pin(async move { f.write().await.put(i).await }))
              ...
              Tunnel::Handoff(ref mut h) => h.put(i),
...
#+begin_src rust :tangle src/types/container/pipe/net.rs :mkdirp yes
  ...
  pub fn server_socket(i: Item, j: Item) -> types::Future<'static, ItemResult> {
      match socket_addr(i, j) {
          Ok(addr) => types::Future::Future(Box::pin(TcpListener::bind(addr).map(|r| match r {
              Ok(s) => Ok(
                  super::Out::ServerSocket(Arc::new(RwLock::new(ServerSocket { socket: s }))).fit(),
              ),
              Err(e) => Err(e.into()),
          }))),
          Err(e) => types::Future::Now(Err(e)),
      }
  }

  pub fn socket(i: Item, j: Item) -> types::Future<'static, ItemResult> {
      match host_addr(i, j) {
          Ok(addr) => types::Future::Future(Box::pin(TcpStream::connect(addr.clone()).map(
              move |r| match r {
                  Ok(s) => Ok(
...
#+begin_src rust :tangle src/types/container/pipe/standard.rs :mkdirp yes
  ...
  pub fn put(i: Item) -> types::Future<'static, Result<(), Error>> {
      let bs = Bytes::try_derive(i);
      match bs {
          Ok(bs) => {
              let f = io::stdout().write(&bs);
              types::Future::Future(Box::pin(future::ready(f.map_err(|e| e.into()).map(|_| ()))))
          }
          Err(e) => types::Future::Now(Err(e)),
      }
  }
...
#+begin_src rust :tangle src/types/container/pipe/channel.rs :mkdirp yes
  ...
  impl Handoff<Item> {
      ...
      pub fn put(&mut self, i: Item) -> types::Future<'static, Result<(), error::Error>> {
          // println!(
          ...
          if let Some(ch) = self.sender.clone() {
              if axiom::is_truthy(&i) {
                  types::Future::Future(Box::pin(async move {
                      ch.send_async(i)
                          .await
              ...
              types::Future::Now(Err(pipe::closed_error(false)))
          }
      }
  ...
  pub fn handoff(mut env: Environment) -> types::Future<'static, Environment> {
      env.pop_prog();
