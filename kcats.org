...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
          ...
          match &*self.node {
              ...
              ProgramNode::Recur {
                  ...
                  if self.next_idx == 0 {
                      ...
                      true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame {
                          node: pred.clone(),
                          next_idx: 0,
                      }]))));
                      true_branch_list.push_back(Item::Program(Box::new(Program(vec![Frame {
                      ...
                      if let ProgramNode::Seq(s) = &**pred {
                          ...
                          l.push_back(Item::Program(Box::new(Program(vec![Frame {
                              node: pred.clone(),
                              next_idx: 0,
                          }]))));
                      }
                      ...
                      l.push_back(Item::Program(Box::new(Program(vec![Frame {
                          node: std::sync::Arc::new(ProgramNode::Seq(true_branch_list)),
                          next_idx: 0,
                      }]))));
                      l.push_back(Item::Program(Box::new(Program(vec![Frame {
                          node: then.clone(),
  ...
  impl Program {
      ...
      pub fn advance_control_flow(
          &mut self,
          stack: &mut crate::types::container::stack::Stack,
      ) -> bool {
          self.clean();
      ...
      pub fn stacktrace(&self) -> cont::List {
          ...
          if let Some(first) = s.get(0) {
              let is_empty = first.is_empty()
                  || matches!(first, crate::types::Item::Nothing)
                  || match first {
      ...
      pub fn count(&self) -> usize {
          self.0.iter().fold(0, |acc, f| match &*f.node {
      ...
      pub fn last(&self) -> Option<&Item> {
          self.0.first().and_then(|f| match &*f.node {
