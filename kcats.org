...
#+begin_src rust :tangle src/traits.rs :mkdirp yes
  ...
  pub trait TryDerive<T>: Sized {
      fn try_derive(value: T) -> Result<Self, DeriveError<T>>;
  ...
  pub trait TryFit<T>: Sized {
      fn try_fit(self) -> Result<T, DeriveError<Self>>;
  }
  ...
  where
      U: TryDerive<T>,
  {
      fn try_fit(self) -> Result<U, DeriveError<T>> {
          U::try_derive(self)
      }
  }
...
#+begin_src rust :tangle src/types.rs :mkdirp yes
  ...
  use crate::types::container::error::Error;
  use crate::types::container::pipe;
  ...
  // these are wrong, not every string is a valid word
  impl TryDerive<String> for Word {
      fn try_derive(s: String) -> Result<Self, DeriveError<String>> {
  ...
  impl TryDerive<&str> for Word {
      fn try_derive(s: &str) -> Result<Self, DeriveError<&str>> {
          Word::try_derive(s.to_string()).map_err(|e| e.swap(s))
      }
  }
  ...
  impl Word {
      ...
      pub fn strip_prefix(&self, prefix: &str) -> Option<Word> {
          self.data.strip_prefix(prefix).map(|d| Word {
  ...
  pub enum Item {
      ...
      In(pipe::In),
      /// A pipe supporting both in and out
      Tunnel(pipe::Tunnel),
  }
  ...
  impl fmt::Debug for Item {
      fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
          match self {
              ...
              Item::Out(o) => write!(f, "{:?}", o),
              Item::Tunnel(t) => write!(f, "{:?}", t),
  ...
  impl Item {
      ...
      pub fn is_empty(&self) -> bool {
          coll::Sized::try_derive(self.clone()).map_or(false, |s| s.is_empty())
      }
  }
  ...
  impl TryDerive<Item> for String {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match i {
              ...
              i => {
                  ...
                  match s {
                      ...
                      i => Err(DeriveError::expected(fit!("string"), i)),
                  }
              }
          }
      }
      ...
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match i {
              ...
              i => {
                  ...
                  match s {
                      coll::Sized::String(s) => s.try_fit().map_err(|e| e.swap(i)),
                      s => {
                          ...
                          match l {
                              ...
                              Ok(mut l) => {
                                  if l.len() == 1 {
                                      ...
                                      Err(DeriveError::expected(fit!("word"), i))
                                  }
                              }
                              Err(_) => Err(DeriveError::expected(fit!("word"), i)),
                          }
                      }
                  }
              }
  ...
  impl TryDerive<Item> for Bytes {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          ...
          match s {
              ...
              b => Err(DeriveError::expected(fit!("bytes"), b)),
          }
      }
  }
      ...
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match i {
              ...
              Item::Int(i) => match char::from_u32(i as u32) {
                  Some(c) => Ok(c),
                  None => Err(DeriveError::expected(fit!("character-encoding"), i)),
              },
              b => Err(DeriveError::expected(fit!("character"), b)),
          }
      }
  }
  ...
  impl TryDerive<&str> for Item {
      fn try_derive(i: &str) -> Result<Self, DeriveError<&str>> {
          Ok(Item::Word(Word::try_derive(i).map_err(|e| e.swap(i))?))
      }
  }
  ...
  where
      T: TryDerive<Item>,
  {
      fn try_derive(value: &Item) -> Result<Self, DeriveError<&Item>> {
          T::try_derive(value.clone()).map_err(|e| e.swap(value))
      }
  }
      ...
      impl Serialize for Item {
              ...
              S: Serializer,
          {
              ...
              match self {
                  ...
                  Item::Out(ref out) => (*out).representation().serialize(serializer),
                  Item::Tunnel(ref t) => t.representation().serialize(serializer),
                  Item::In(ref i) => i.representation().serialize(serializer),
              }
          }
      }
  }
...
#+begin_src rust :tangle src/types/number.rs :mkdirp yes
  ...
  impl TryDerive<Number> for Float {
      fn try_derive(i: Number) -> Result<Self, DeriveError<Number>> {
          match i {
              Number::Float(i) => Ok(i),
              i => Err(DeriveError::expected(fit!("float"), i)),
          }
      }
  }
  ...
  impl TryDerive<Number> for Int {
      fn try_derive(i: Number) -> Result<Self, DeriveError<Number>> {
          match i {
              Number::Int(i) => Ok(i),
              i => Err(DeriveError::expected(fit!("integer"), i)),
          }
      }
  }
      ...
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          let fromstr = |s: String| {
              let r = s
                  ...
                  .parse::<i64>()
                  .map(Number::Int)
                  .map_err(|e| DeriveError::expected(fit!("integer"), s));

              r.or_else(|_| s.as_str().parse::<Float>().map(Number::Float))
                  .map_err(|e| DeriveError::expected(fit!("float"), s))
          };
          match i {
              ...
              i => Err(DeriveError::expected(fit!("number"), i)),
          }
      }
  }
  ...
  impl TryDerive<Item> for Int {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match Number::try_derive(i)? {
              Number::Int(i) => Ok(i),
              n => Err(DeriveError::expected(fit!("integer"), i.fit())),
          }
      }
  }
  ...
  impl TryDerive<Item> for Float {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match Number::try_derive(i)? {
              Number::Float(i) => Ok(i),
              i => Err(DeriveError::expected(fit!("float"), i.fit())),
          }
      }
  }
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
      ...
      fn join(self, mut other: Set) -> Result<Self::Output, Self::Error> {
          ...
          bm.extend(
              self.iter()
                  ...
                  .collect::<Result<Vec<assoc::KeyItem>, DeriveError<dict::Entry>>>()
                  .map_err(|e| Error::derive)?,
          );
          Ok(other)
      }
  }
  ...
  impl TryDerive<Dispenser> for Sized {
      fn try_derive(c: Dispenser) -> Result<Self, DeriveError<Dispenser>> {
  ...
  impl TryDerive<Receptacle> for Sized {
      fn try_derive(c: Receptacle) -> Result<Self, DeriveError<Receptacle>> {
          match c {
  ...
  impl TryDerive<Sized> for List {
      fn try_derive(s: Sized) -> Result<Self, DeriveError<Sized>> {
          match s {
              Sized::List(l) => Ok(*l),
              Sized::Associative(a) => Ok(List::derive_iter(a.to_iter().map(Item::derive))),
              i => Err(DeriveError::expected(fit!("list"), i)),
          }
      }
  }
  ...
  impl TryDerive<List> for Vec<dict::Namespace> {
      fn try_derive(l: List) -> Result<Self, DeriveError<List>> {
  ...
  impl TryDerive<Item> for Sized {
      fn try_derive(item: Item) -> Result<Self, DeriveError<Item>> {
          match item {
              Item::Bytes(b) => Ok(Sized::Bytes(b)),
              Item::String(s) => Ok(Sized::String(s)),
              Item::List(l) => Ok(Sized::List(l)),
              Item::Assoc(a) => Ok(Sized::Associative(assoc::Associative::Assoc(a))),
              Item::Error(a) => Ok(Sized::Associative(assoc::Associative::Error(a))),
              Item::Dictionary(a) => Ok(Sized::Associative(assoc::Associative::Dictionary(a))),
              Item::DictEntry(a) => Ok(Sized::Associative(assoc::Associative::DictEntry(a))),
              Item::Words(a) => Ok(Sized::Associative(assoc::Associative::Words(a))),
              Item::Env(a) => Ok(Sized::Associative(assoc::Associative::Env(a))),
              Item::Nothing => Ok(Sized::Associative(assoc::Associative::Nothing)),
  ...
  impl TryDerive<Item> for Receptacle {
      fn try_derive(item: Item) -> Result<Self, DeriveError<Item>> {
          match item {
              Item::In(i) => Ok(Receptacle::In(i)),
              Item::Tunnel(t) => Ok(Receptacle::Tunnel(t)),
              ...
              Item::Assoc(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Assoc(a),
              ))),
              Item::Error(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Error(a),
              ))),
              Item::Dictionary(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Dictionary(a),
              ))),
              Item::DictEntry(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::DictEntry(a),
              ))),
              Item::Words(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Words(a),
              ))),
              Item::Env(a) => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Env(a),
              ))),
              Item::Nothing => Ok(Receptacle::Sized(Sized::Associative(
                  assoc::Associative::Nothing,
              ))),
              Item::Set(s) => Ok(Receptacle::Sized(Sized::Set(s))),
              Item::Program(p) => Ok(Receptacle::Sized(Sized::Program(p))),
              i => Err(DeriveError::expected(fit!("receptacle"), i)),
          }
      }
  }
  ...
  impl TryDerive<Receptacle> for Dispenser {
      fn try_derive(c: Receptacle) -> Result<Self, DeriveError<Receptacle>> {
          match c {
              ...
              i => Err(DeriveError::expected(fit!("iterable"), i)),
          }
      }
  }
  ...
  impl Derive<Dispenser> for Item {
      fn derive(d: Dispenser) -> Self {
          match d {
              Dispenser::Sized(Sized::Bytes(b)) => Item::Bytes(b),
              Dispenser::Sized(Sized::String(s)) => Item::String(s),
              Dispenser::Sized(Sized::List(l)) => Item::List(l),
              Dispenser::Sized(Sized::Set(s)) => Item::Set(s),
              Dispenser::Sized(Sized::Associative(assoc::Associative::Assoc(a))) => Item::Assoc(a),
              Dispenser::Sized(Sized::Associative(assoc::Associative::Dictionary(a))) => {
                  Item::Dictionary(a)
              }
              Dispenser::Sized(Sized::Associative(assoc::Associative::DictEntry(a))) => {
                  Item::DictEntry(a)
              }
              Dispenser::Sized(Sized::Associative(assoc::Associative::Words(a))) => Item::Words(a),
              Dispenser::Sized(Sized::Associative(assoc::Associative::Env(a))) => Item::Env(a),
              Dispenser::Sized(Sized::Associative(assoc::Associative::Error(a))) => Item::Error(a),
              Dispenser::Sized(Sized::Associative(assoc::Associative::Nothing)) => Item::Nothing,
              Dispenser::Sized(Sized::Program(p)) => Item::Program(p),
              Dispenser::Out(o) => Item::Out(o),
              Dispenser::Tunnel(t) => Item::Tunnel(t),
          }
      }
  }
  ...
  impl Derive<Receptacle> for Item {
      fn derive(r: Receptacle) -> Self {
          match r {
              Receptacle::Sized(Sized::Bytes(b)) => Item::Bytes(b),
              Receptacle::Sized(Sized::String(s)) => Item::String(s),
              Receptacle::Sized(Sized::List(l)) => Item::List(l),
              Receptacle::Sized(Sized::Set(s)) => Item::Set(s),
              Receptacle::Sized(Sized::Program(p)) => Item::Program(p),
              Receptacle::Sized(Sized::Associative(assoc::Associative::Assoc(a))) => Item::Assoc(a),
              Receptacle::Sized(Sized::Associative(assoc::Associative::Dictionary(a))) => {
                  Item::Dictionary(a)
              }
              Receptacle::Sized(Sized::Associative(assoc::Associative::DictEntry(a))) => {
                  Item::DictEntry(a)
              }
              Receptacle::Sized(Sized::Associative(assoc::Associative::Nothing)) => Item::Nothing,
              Receptacle::Sized(Sized::Associative(assoc::Associative::Words(a))) => Item::Words(a),
              Receptacle::Sized(Sized::Associative(assoc::Associative::Env(a))) => Item::Env(a),
              Receptacle::Sized(Sized::Associative(assoc::Associative::Error(a))) => Item::Error(a),
              Receptacle::In(o) => Item::In(o),
              Receptacle::Tunnel(t) => Item::Tunnel(t),
          }
      }
  }
  ...
  impl TryDerive<Item> for Dispenser {
      fn try_derive(item: Item) -> Result<Self, DeriveError<Item>> {
          match item {
              ...
              Item::Assoc(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Assoc(a),
              ))),
              Item::Error(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Error(a),
              ))),
              Item::Dictionary(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Dictionary(a),
              ))),
              Item::DictEntry(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::DictEntry(a),
              ))),
              Item::Words(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Words(a),
              ))),
              Item::Env(a) => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Env(a),
              ))),
              Item::Nothing => Ok(Dispenser::Sized(Sized::Associative(
                  assoc::Associative::Nothing,
              ))),
              Item::Program(p) => Ok(Dispenser::Sized(Sized::Program(p))),
              Item::Out(s) => Ok(Dispenser::Out(s)),
              Item::Tunnel(s) => Ok(Dispenser::Tunnel(s)),
              i => Err(DeriveError::expected(fit!("iterable"), i)),
          }
      }
  }
...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Program {
      fn try_derive(i: Item) -> Result<Program, DeriveError<Item>> {
          let s: cont::Sized = i.try_fit()?;
          match s {
              cont::Sized::Program(p) => Ok(p),
              s => Ok(Program::derive(cont::List::try_derive(s).map_err(|e| {
                  DeriveError {
                      src: e.src.fit(),
                      expected: e.expected,
                  }
...
#+begin_src rust :tangle src/types/container/associative.rs :mkdirp yes
          ...
          match i {
              ...
              i => match coll::Sized::try_derive(i)? {
                  ...
                  s => {
                      ...
                      Err(DeriveError::expected(fit!("KeyItem"), s))
                  }
              },
          }
      }
  ...
  impl TryDerive<KeyItem> for Word {
      fn try_derive(k: KeyItem) -> Result<Self, DeriveError<KeyItem>> {
          match k {
              KeyItem::Word(w) => Ok(w),
              KeyItem::String(s) => s.try_fit(),
              i => Err(DeriveError::expected(fit!("word"), i)),
          }
      }
  }
  ...
  impl TryDerive<coll::Sized> for Associative {
      fn try_derive(s: coll::Sized) -> Result<Self, DeriveError<coll::Sized>> {
          match s {
              coll::Sized::Associative(a) => Ok(a),
              coll::Sized::String(i) => Err(DeriveError::expected(fit!("associative"), i)),
              coll::Sized::Bytes(i) => Err(DeriveError::expected(fit!("associative"), i)),
  ...
  impl TryDerive<Item> for Entry {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          let s = coll::Sized::try_derive(i)?;
          if s.count() != 2 {
              Err(DeriveError::expected(fit!("pair"), s))
          } else {
...
#+begin_src rust :tangle src/types/container/error.rs :mkdirp yes
  ...
  impl TryDerive<Box<dyn Iterator<Item = Item>>> for Error {
      ...
      ) -> Result<Self, DeriveError<Box<dyn Iterator<Item = Item>>>> {
  ...
  impl TryDerive<assoc::Associative> for Error {
      fn try_derive(a: assoc::Associative) -> Result<Self, DeriveError<assoc::Associative>> {
          match a {
...
#+begin_src rust :tangle src/types/container/dictionary.rs :mkdirp yes
  ...
  impl TryDerive<Item> for SpecElement {
      fn try_derive(i: Item) -> Result<SpecElement, DeriveError<Item>> {
          match i {
  ...
  impl TryDerive<coll::List> for StackSpec {
      fn try_derive(s: coll::List) -> Result<StackSpec, DeriveError<coll::List>> {
          s.iter()
              .cloned()
  ...
  impl TryDerive<coll::List> for Spec {
      fn try_derive(s: coll::List) -> Result<Spec, DeriveError<coll::List>> {
          if s.len() != 2 {
              Err(Error::list_count(2))
          } else {
              Ok((
  ...
  impl TryDerive<Item> for Spec {
      fn try_derive(i: Item) -> Result<Spec, DeriveError<Item>> {
  ...
  impl TryDerive<Item> for Namespace {
      fn try_derive(i: Item) -> Result<Namespace, DeriveError<Item>> {
          match i {
              ...
              i => {
                  ...
                  if s.is_empty() {
                      ...
                      Err(DeriveError::expected(fit!("namespace"), s))
                  }
              }
          }
      }
  ...
  impl TryDerive<Item> for Dictionary {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          ...
          match s {
              ...
              a => Err(DeriveError::expected(fit!("dictionary"), a)),
          }
      }
  }
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  impl Environment {
      ...
      /// Use when a stack item fails the type check, pushes the item
      /// (which is stored in the error)
      pub fn push_err_with_src<T>(&mut self, err: DeriveError<T>)
      where
          T: Clone + Fit<Item>,
      {
          self.push(err.src.clone());
          self.push(err);
      }
  ...
  fn check_type(i: &Item, w: &Word) -> bool {
      ...
      match (w, i) {
          ...
          (w, Item::Out(_) | Item::Tunnel(_) | Item::In(_)) if *w == *S_PIPE => true,
...
#+begin_src rust :tangle src/crypto.rs :mkdirp yes
      ...
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          ...
          if a.get(&fit!("type")) == Some(fit!("elliptic-curve-key")) {
              let sk = signing::SecretKey::from_bytes(
                  &Bytes::try_derive(a.get(&fit!("secret")).map_err(
                      |_e| DeriveError::expected(i, fit!("secret")), //Error::expected(fit!("secret"), None::<Item>)
                  )?)?[..],
              )
              .map_err(|_e| DeriveError::expected(i, fit!("valid-secret-key")))?;
              Ok(sk)
          } else {
              Err(DeriveError::expected(i, fit!("elliptic-curve-key")))
          }
      }
  }
  ...
  impl TryDerive<Item> for signing::PublicKey {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          let a = assoc::Associative::try_derive_impl(i)?;
          if a.get(&fit!("type")) == Some(fit!("elliptic-curve-key")) {
              let pk = signing::PublicKey::from_bytes(
                  &Bytes::try_derive(
                      a.get(&fit!("public"))
                          .ok_or_else(|| DeriveError::expected(fit!("public"), None::<Item>))?,
                  )?[..],
              )
              .map_err(|_e| DeriveError::expected(fit!("valid-public-key"), None::<Item>))?;
              Ok(pk)
          } else {
              Err(Error::expected(fit!("public-key"), a))
          }
      }
  }
  ...
  impl From<&Item> for edn_format::Value {
      fn from(item: &Item) -> Self {
          ...
          match item {
              ...
              Item::Out(o) => (&o.representation()).into(),
              Item::Tunnel(t) => (&t.representation()).into(),
              Item::In(i) => (&i.representation()).into(),
...
#+begin_src rust :tangle src/axiom.rs :mkdirp yes
  ...
  pub fn swapdeep(mut env: Environment) -> Future<Environment> {
      env.pop_prog();
      swap2(env, 2)
  }
  ...
  pub fn is_pipe(i: Item) -> bool {
      matches!(i, Item::Out(_) | Item::Tunnel(_) | Item::In(_))
  }
  ...
  fn assoc_in(i: Option<Item>, ks: &[assoc::KeyItem], v: Item) -> Result<Item, Error> {
      ...
      if let [k, ks @ ..] = ks {
          match (i, k) {
              ...
              (Some(i), k) => {
                  let a = assoc::Associative::try_derive(i).map_err(Error::derive)?;
                  assoc_map(a, ks, k, v)
              }
          }
      } else {
  ...
  fn remove(i: coll::Sized, k: Item) -> Result<Item, Error> {
      match i {
          ...
          coll::Sized::Set(mut s) => {
              s.mutate()
                  .remove(&assoc::KeyItem::try_derive(k).map_err(Error::derive)?);
              Ok(s.fit())
          }
          coll::Sized::Associative(a) => {
              let (a, _) = a.remove(&assoc::KeyItem::try_derive(k).map_err(Error::derive)?);
              Ok(a.fit())
          }
  ...
  fn unassoc_in(i: Item, ks: &[assoc::KeyItem]) -> Result<Item, Fit<Error>> {
      if let [k, ks @ ..] = ks {
          if ks.is_empty() {
              let a = assoc::Associative::try_derive(i).map_err(DeriveError::derive)?;
              Ok(a.remove(k).0.fit())
          } else {
              match (i, k) {
  ...
  pub fn timestamps(mut env: Environment) -> Future<Environment> {
      env.pop_prog();
      env.push(Item::Out(pipe::Out::Time));
      env.fit()
  }

  pub fn standard(mut env: Environment) -> Future<Environment> {
      env.pop_prog();
      env.push(Item::Tunnel(pipe::Tunnel::Standard));
      env.fit()
  }
...
#+begin_src rust :tangle src/types/container/pipe.rs :mkdirp yes
  ...
  impl TryDerive<Item> for In {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match i {
              Item::In(i) => Ok(i),
              Item::Tunnel(t) => Ok(t.fit()),
              i => Err(DeriveError::expected(fit!("pipe"), i)),
          }
      }
  }
...
#+begin_src rust :tangle src/types/container/pipe/net.rs :mkdirp yes
  ...
  impl Derive<Socket> for Item {
      fn derive(ss: Socket) -> Item {
          Item::Tunnel(pipe::Tunnel::Socket(Arc::new(RwLock::new(ss))))
      }
  }
#+end_src
***** Time
...
#+begin_src rust :tangle src/types/container/pipe/channel.rs :mkdirp yes
              ...
              i => {
                  ...
                  match (p, i) {
                      (Some(p), Some(i)) => match (p, i) {
                          (Item::In(pipe::In::Handoff(p)), i) => Ok(ChannelOp::Send(
                              Arc::new(p.sender.clone().unwrap()),
                              i.clone(),
                          )),
                          (Item::Tunnel(pipe::Tunnel::Handoff(p)), i) => Ok(ChannelOp::Send(
                              Arc::new(p.sender.clone().unwrap()),
                              i.clone(),
                          )),
  ...
  impl TryDerive<Item> for Handoff<Item> {
      fn try_derive(i: Item) -> Result<Self, DeriveError<Item>> {
          match i {
              Item::Out(pipe::Out::Handoff(p)) => Ok(p),
              Item::In(pipe::In::Handoff(p)) => Ok(p),
              Item::Tunnel(pipe::Tunnel::Handoff(p)) => Ok(p),
              i => Err(DeriveError::expected(fit!("handoff"), i)),
          }
      }
  }
  ...
  pub fn receiver(i: Item) -> axiom::ItemResult {
      let mut h = Handoff::try_derive(i).map_err(Error::derive)?;
      h.close_put();
      Ok(Item::Out(pipe::Out::Handoff(h)))
  }
  ...
  pub fn timer(i: Item) -> axiom::ItemResult {
      ...
      Ok(Timer::new(Duration::from_millis(ms)).fit())
  }
#+end_src
***** Database
#+begin_src rust :tangle src/types/container/pipe/db.rs :mkdirp yes
  ...
  impl Db {
      ...
      pub fn insert_item(&self, i: Item, id: uuid::Uuid) -> axiom::ItemResult {
          ...
          match s {
              ...
              s => return Err(Error::expected(fit!("db-object"), s)),
          }
          Ok(Item::default())
      }
  }
  ...
  pub fn query(q: Item, params: Item) -> axiom::ItemResult {
      let query: String = q.try_fit().map?;
  ...
  impl TryDerive<ValueRef<'_>> for Item {
      fn try_derive(value: ValueRef) -> Result<Self, DeriveError<ValueRef>> {
          match value {
              ...
              ValueRef::Blob(b) => Ok(Item::Bytes(b.to_vec())),
  ...
  fn decode_string(s: String) -> Result<Item, DeriveError<String>> {
      if let Some(w) = s.strip_prefix("w|") {
          w.try_fit()
      } else if let Some(s) = s.strip_prefix("s|") {
          Ok(Item::String(s.to_string()))
...
** TODO Module loading by alias fixes
#+begin_src kcats
  dictionary
  [[errors ✅]
   [crypto []]]
