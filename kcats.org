...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  impl Program {
      ...
      pub fn unwind_to_handle(&mut self, handle: &Item) -> (Program, bool) {
          println!("unwind_to_handle called!");
          ...
          while let Some(f) = self.0.last_mut() {
              ...
              match &*f.node {
                  ProgramNode::Seq(items) => {
                      ...
                      if local_found {
                          found = true;
                          let end_idx = f.next_idx - 1;
                          let mut temp = items.clone();
                          let unwound_items = temp.slice(0..end_idx);
                          let unwound_frame = Frame {
                              node: Arc::new(ProgramNode::Seq(unwound_items)),
                              next_idx: start_idx,
                          };
                          ...
                          unwound_frame.next_idx = start_idx;
                          unwound.0.push(unwound_frame);
                          self.0.pop();
                      }
                  }
                  ProgramNode::Recur { .. } => {
                      unwound.0.push(f.clone());
                      self.0.pop();
                  }
              }
          }
      ...
      pub fn stacktrace(&self) -> cont::List {
          ...
          if let Some(first) = s.get(0) {
              let is_empty = first.is_empty() || matches!(first, crate::types::Item::Nothing) || match first {
                  crate::types::Item::Program(p) => p.count() == 0,
                  _ => false,
              };
              if is_empty {
                  s = s.slice(1..s.len());
