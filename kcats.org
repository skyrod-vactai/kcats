  ...
  impl PartialEq for Item {
      fn eq(&self, other: &Self) -> bool {
          match (self, other) {
              ...
              (Item::Sized(a), Item::Receptacle(coll::Receptacle::Sized(b))) => a == b,
              (Item::Receptacle(coll::Receptacle::Sized(a)), Item::Sized(b)) => a == b,
...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl TryDerive<Item> for Sized {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              Item::Sized(s) => Ok(s),
              Item::Receptacle(Receptacle::Sized(s)) => Ok(s),
              i => {
  ...
  impl TryDerive<Item> for Dispenser {
      ...
      fn try_derive(item: Item) -> Result<Self, Self::Error> {
          match item {
              ...
              Item::Receptacle(Receptacle::Sized(s)) => Ok(Dispenser::Sized(s)),
              Item::Receptacle(Receptacle::Tunnel(t)) => Ok(Dispenser::Tunnel(t)),
...
#+begin_src rust :tangle src/types/container/environment.rs :mkdirp yes
  ...
  fn check_type(i: &Item, w: &Word) -> bool {
      ...
      match (w, i) {
          ...
          (w, Item::Out(_) | Item::Tunnel(_) | Item::Sized(_) | Item::Receptacle(_))
              if *w == *S_DISPENSER =>
          {
              true
          }
          (w, Item::Receptacle(_) | Item::Tunnel(_) | Item::Sized(_)) if *w == *S_RECEPTACLE => true,
          ...
          (
              w,
              Item::Sized(coll::Sized::List(_))
              | Item::Receptacle(coll::Receptacle::Sized(coll::Sized::List(_)))
              | Item::Sized(coll::Sized::Program(_))
              | Item::Receptacle(coll::Receptacle::Sized(coll::Sized::Program(_))),
          ) if *w == *S_LIST || *w == *S_PROGRAM => true,

          (
              w,
              Item::Sized(coll::Sized::Associative(_))
          ...
          (
              w,
              Item::Sized(coll::Sized::Associative(assoc::Associative::Dictionary(_)))
              | Item::Receptacle(coll::Receptacle::Sized(coll::Sized::Associative(
                  assoc::Associative::Dictionary(_),
              ))),
          ) if *w == *S_DICTIONARY => true,

          (w, Item::Sized(_) | Item::Receptacle(coll::Receptacle::Sized(_)))
              if *w == *S_SIZED || *w == *S_ORDERED =>
          {
              true
          }
