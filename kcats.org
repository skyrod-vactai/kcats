...
#+begin_src rust :tangle src/types/container.rs :mkdirp yes
  ...
  impl Join<List> for assoc::Associative {
      ...
      fn join(self, other: List) -> Result<Self::Output, <Self as Join<List>>::Error> {
          ...
          let la = assoc::Associative::Assoc(Arc::new(
              other.try_rewrap::<assoc::AssociationContent, Detailed>(&Detailed)?,
          ));
  ...
  impl Join<assoc::Associative> for List {
      ...
      fn join(self, other: assoc::Associative) -> Result<Self::Output, Self::Error> {
          ...
          let sa = assoc::Associative::Assoc(Arc::new(
              self.try_rewrap::<assoc::AssociationContent, Detailed>(&Detailed)?,
          ));
  ...
  impl TryDerive<Dispenser> for Sized {
      fn try_derive_with<M: Mode>(c: Dispenser, _mode: &M) -> Result<Self, M::Err<Dispenser>> {
  ...
  impl TryDerive<Receptacle> for Sized {
      fn try_derive_with<M: Mode>(c: Receptacle, mode: &M) -> Result<Self, Error> {
          match c {
  ...
  impl TryDerive<Sized> for List {
      fn try_derive_with<M: Mode>(s: Sized, mode: M) -> Result<Self, M::Err<Sized>> {
          match s {
              Sized::List(l) => Ok(*l),
  ...
  impl TryDerive<List> for Vec<dict::Namespace> {
      fn try_derive_with<M: Mode>(l: List, mode: &M) -> Result<Self, M::Err<List>> {
          // l.iter().cloned().map(dict::Namespace::try_derive).collect()
          l.try_rewrap::<Self, M>(mode)
      }
  }
  ...
  impl TryDerive<List> for String {
      fn try_derive_with<M: Mode>(l: List) -> Result<Self, Error> {
          let v = l
              .iter()
              .cloned()
              .map(Char::try_derive)
  ...
  impl TryDerive<List> for Bytes {
      fn try_derive_with<M: Mode>(l: List) -> Result<Self, M::Err<List>> {
          let v = l
              .iter()
              .cloned()
              .map(|i| number::Int::try_derive::<M>(i).map(|i| i as u8))
...
#+begin_src rust :tangle src/derivation.rs :mkdirp yes
  ...
  pub trait Mode: Sized + 'static {
      ...
      fn expected<Src: Fit<Item>>(expected: Word, src: Src) -> Self::Err<Src>;
  ...
  impl Mode for Detailed {
      ...
      fn expected<Src: Fit<Item>>(expected: Word, src: Src) -> Error {
          Error::expected(expected, src.fit())
      }

      fn nest_error<Outer: Fit<Item>, Inner: Fit<Item>>(outer: Error, inner: Error) -> Error {
          outer.nest(inner)
      }

      fn revert<Src, Dst>(err: Error, _f: impl FnOnce(Src) -> Dst) -> Error {
          err
      }
  }
  ...
  pub trait TryDerive<Src>: Sized {
      ...
      fn try_derive(value: Src) -> Result<Self, Error> {
          Self::try_derive_with::<Detailed>(value)
      }
  }
  ...
  pub trait TryFit<Target>: Sized {
      ...
      fn try_fit(self) -> Result<Target, Error> {
          Self::try_fit_with::<Detailed>(self)
      }
  }
  ...
  where
      T: Identity + Fit<Item>,
  {
      fn try_derive_with<M: Mode>(value: T, _mode: &M) -> Result<T, M::Err<T>> {
          Ok(value)
      }
  }
...
#+begin_src rust :tangle src/serialize.rs :mkdirp yes
  ...
  impl TryDerive<edn_format::Value> for Item {
      fn try_derive_with<M: Mode>(item: edn_format::Value, _mode: &M) -> Result<Item, Error> {
