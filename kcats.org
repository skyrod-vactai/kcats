...
#+begin_src rust :tangle src/types/container/program.rs :mkdirp yes
  ...
  impl Frame {
      ...
      pub fn remaining(&mut self) -> cont::List {
          match &*self.node {
              ...
              ProgramNode::Recur { .. } => {
                  // Cannot easily represent remaining state of Recur as a flat list.
                  cont::List::new()
              }
          }
      }
  ...
  impl Program {
      ...
      pub fn advance_control_flow<F>(&mut self, mut pop_stack: F) -> bool
      where
          F: FnMut() -> Option<Item>,
      {
          self.clean();
          ...
          if let ProgramNode::Recur { pred, then, rec1, rec2, self_ref } = &*frame.node {
              if frame.next_idx == 0 {
                  ...
                  let pred_result = pop_stack();
                  
                  // Determine truthiness. For now, empty list/🔳/error is false, anything else true.
                  let mut is_true = true;
                  if let Some(res) = pred_result {
                      match res {
                          Item::Nothing => is_true = false,
                          Item::List(l) if l.is_empty() => is_true = false,
                          Item::Program(p) if p.is_empty() => is_true = false,
                          Item::Error(_) => is_true = false,
                          Item::Char(c) if c == '\0' => is_true = false,
                          _ => {} // default true
                      }
                  } else {
                      is_true = false;
