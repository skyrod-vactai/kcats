...
type EffectResult = Result<(Option<List>, List), Error>;
...
pub fn first(i: Item) -> EffectResult {
    if let Item::List(mut l) = i {
        Ok((None, VecDeque::from(vec![
            if let Some(i) = l.pop_front() { i } else { Item::Nothing}])))
        
    }
    else { Err(Error::from("Invalid type for first")) }
}
