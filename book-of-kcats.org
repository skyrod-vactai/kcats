...
There will be no compiled binary code, at least, not a standard one
that users can rely on. All code will be source and run via the
interpreter.
* Non-goals
** Fast performance
...
Next we're going to need some utility functions that operate on
environments to do common things like moving items between the stack
and the expression.
