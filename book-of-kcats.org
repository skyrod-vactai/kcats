...
pub trait Step {
    fn step(self, env: Environment) -> StepResult;
}
...
fn act_axiom(word: Word, e: Environment) -> StepResult {
    match word.as_str() {
        ...
        "=" => axiom::env_effect2(e, axiom::eq),
        "count" => axiom::env_effect1(e, axiom::count),
        "string?" => axiom::env_effect1(e, axiom::is_string),
        "number?" => axiom::env_effect1(e, axiom::is_number),
        "list?" => axiom::env_effect1(e, axiom::is_list),
...
pub fn is_number(i: Item) -> EffectResult {
    Ok((
        None,
        Some(List::from(vec![Item::Boolean(
            if let Item::Int(_) | Item::Float(_) = i { true } else { false },
        )])),
    ))
}
...
pub fn is_list(i: Item) -> EffectResult {
    Ok((
        None,
        Some(List::from(vec![Item::Boolean(
            if let Item::List(_) | Item::Nothing = i { true } else { false },
        )])),
    ))
}
...
Test hash of fn
#+begin_src rust :crates '(("radix_trie" . "0.2.1"))
use radix_trie::Trie;
use std::collections::HashMap;

struct Env<'a> {
    number: Option<i32>,
    dict: Trie<String, &'a StepFn>
}
...
type StepFn = dyn Fn(Env) -> Env;

fn main () {
    let mut dict = Trie::<String, &StepFn>::new();
    let i =  &make_thing(inc);
    dict.insert("inc".to_string(), i);
    let mut env = Env { number: Some(12), dict: dict };
    let method = env.dict.get(&"inc".to_string()).unwrap().clone();
    println!("{}", method(env).number.unwrap());
