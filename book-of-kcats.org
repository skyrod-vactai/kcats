...
pub fn to_value_item(i: KeyItem) -> ItemResult {
    match i {
...
fn f_stack1(f: fn(Item) -> ItemResult, env: &mut Environment) -> StepResult {
    if let Some(x) = env.stack.pop_front() {
        env.stack.push_front(f(x)?);
        Ok(env)
    } else {
        axiom::stack_underflow()
    }
}

fn f_stack2(f: fn(Item, Item) -> ItemResult, env: &mut Environment) -> StepResult {
...
fn act_axiom(word: Word, e: &mut Environment) -> StepResult {
    match word.as_str() {
        ...
        _ => Err(Error {message: (format!("unimplemented axiom word: {}", word)),
                        environment: Some(e.clone())}),
    }
}
    ...
    if let Some(val) = next_item {
        if let Item::Word(word) = val {
            if let Some(dfn) = env.dictionary.get(&word) {
                ...
                Err(Error::from(format!("Unknown word: {}", word)))
            }
        } else {
...
#+RESULTS:
: error: Could not compile `cargo72cA7r`.
...
pub fn stack_underflow() -> StepResult<'static> {
...
pub fn plus(i: Item, j: Item) -> ItemResult {
    match (i, j) {
        ...
        _ => Err(Error::from("Invalid type for +")),
    }
}
...
pub fn inc(i: Item) -> ItemResult {
    if let Item::Int(i) = i {
        ...
        Err(Error::from("Invalid type for inc"))
    }
}
...
pub fn execute(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop_front() {
        match item {
            ...
            _ => Err(Error::from("Invalid type for execute")),
        }
    } else {
        stack_underflow()
    }
...
pub fn unwrap(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop_front() {
        if let Item::List(mut l) = item {
            ...
            Err(Error::from(format!("Can only unwrap on list, found {:?}", item)))
        }
    } else {
        stack_underflow()
    }
...
pub fn dip(env: &mut Environment) -> StepResult {
    ...
    match (s1, s2) {
        (Some(item1), Some(item2)) => match item1 {
            ...
            _ => Err(Error::from("Invalid type for dip")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn branch(env: &mut Environment) -> StepResult {
    ...
    match (f, t, b) {
        (Some(f), Some(t), Some(b)) => match (f, t) {
            ...
            _ => Err(Error::from("Invalid type for branch")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn step(env: &mut Environment) -> StepResult {
    ...
    match (p, l) {
        (Some(p), Some(l)) => match (p, l) {
            ...
            _ => Err(Error::from("Invalid type for step")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn range(env: &mut Environment) -> StepResult {
    ...
    match (from, to) {
        (Some(from), Some(to)) => match (from, to) {
            ...
            _ => Err(Error::from("Invalid type for range")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn recur(env: &mut Environment) -> StepResult {
    ...
    match (rec2, rec1, then, pred) {
        (Some(rec2), Some(rec1), Some(then), Some(pred)) => match (rec2, rec1, then, pred) {
            ...
            _ => Err(Error::from("Invalid type for recur")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn evert(env: &mut Environment) -> StepResult {
    ...
    match l {
        Some(l) => match l {
            ...
            _ => Err(Error::from("Invalid type for evert")),
        },
        _ => stack_underflow(),
    }
}
...
pub fn lexicon(env: &mut Environment) -> StepResult {
    ...
    //                     },
    //                     _ => { return Err(Error::from("Word definition has invalid type"))}
    //                 }
    //             }
