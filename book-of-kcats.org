...
(def axiom-words
  {'execute (effect [p] [p []])
   ...
   'describe (fn [{[word & others] ::stack dict ::dictionary :as env}]
               (if-let [dfn (get dict word)]
   ...
   'dictionary (fn [{::keys [dictionary] :as env}]
                 (update env ::stack conj (repr dictionary))) 
   'lexicon (fn [env]
              (reduce (fn [env dfn]
                        (update env ::dictionary
                                inscribe dfn))
                      env
                      (-> "src/kcats/lexicon.kcats"
                          io/file
                          io/reader
