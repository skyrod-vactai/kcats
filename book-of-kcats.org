  ...
  (def builtin-words
    (merge
     ...
     {'discard {::spec ::stack-min-depth-1
      ...
      'pack {::spec (spec/cat :x ::item
                              :aggregate ::aggregate
                              :others (spec/* ::item))
             ::fn (f-stack 2 (fn [x a]
                               (conj (vec a) x)))}
      'unpack {::spec (spec/cat :aggregate ::aggregate
                                :others (spec/* ::item))
               ::fn (partial update-stack (fn [[[x & rest] & others]]
                                            (apply list x (vec rest) others)))}
      'join {::spec (spec/cat :agg1 ::aggregate
                              :agg1 ::aggregate
                              :others (spec/* ::item))
    ...
    (merge builtin-words
            {'assoc {::spec (spec/cat :item ::pair, :alist ::association-list
             ...
             'cond {::spec (spec/cat :test-expr-pairs ::association-list
                                     :other (spec/* ::item))
                    ::fn (fn [{[[[test expr] & other-clauses] & others] ::stack
...
#+begin_src clojure :tangle src/kcats
...
+ Clojure-like dispatch function and value-labeled methods
Tried and true, but requires the implementation of a type hierarchy to
do dispatch by type prope
+ A series of predicate/method pairs
Iterate over the pairs, and the first one where the pred returns true,
execute the method. Performance-wise, it could be slower if there's
lots of methods defined. However there's no dispatch function to
execute so in some cases it could be faster.
