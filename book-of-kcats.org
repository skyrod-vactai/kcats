...
produced something). That will signal =collect= to stop.
...
Finally we have =reduce= which will consume what a generator
produces. You provide a program that takes 2 arguments, and =reduce=
will generate all the items, and pass to your program: the result so
far and the next item generated:
