...
fn to_key_item(i: Item) -> Result<KeyItem, String> {
    match i {
...
pub fn lexicon(env: &mut Environment) -> StepResult {
    if let Ok(s) = fs::read_to_string("src/kcats/lexicon.kcats") {
        ...
        let h: HashMap<KeyItem, Item> = items
            .into_iter()
            .map(| i |   {
                if let Item::List(l) = i {
                    Ok(l)
                } else {
                    Err("Lexicon item isn't a list".to_string())
                }
    ...
    else { Err("Could not read from lexicon file".to_string())  }
...
#+RESULTS:
: Ok(Symbol(Symbol { namespace: None, name: "ced" }))
