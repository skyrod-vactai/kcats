...
fn print_result(r: StepResult) {
    match r {
        ...
        Ok(env) => {
            ...
            println!("{}", serialize::emit(&Item::List(env.stack.clone())));
        }
    }
}
...
fn append_expression(env: &mut Environment, mut p: Vec<Item>) -> StepResult {
    p.reverse();
    env.expression.append(&mut p);
    Ok(env)
}
...
pub fn execute(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop() {
        match item {
            Item::List(mut program) => append_expression(env, program),
...
pub fn step(env: &mut Environment) -> StepResult {
    ...
    match (p, l) {
        (Some(p), Some(l)) => match (p, l) {
            (Item::List(p), Item::List(l)) => {
                if !l.is_empty() {
                    let litem = l[0].clone();
                    let lrest = &l[1..];
                    let mut expr = vec![word("execute")];
                    if !lrest.is_empty() {
                        expr.push(Item::List(lrest.to_vec()));
                        expr.push(Item::List(p.clone()));
                        expr.push(word("step"))
                    }
                    append_expression(env, expr);
    ...
    match (from, to) {
        (Some(from), Some(to)) => match (from, to) {
            (Item::Int(from), Item::Int(to)) => {
                env.stack
                    .push(Item::List(
                        (from..to).map(|i| Item::Int(i)).collect::<Vec<Item>>()));
                return Ok(env);
            }
...
fn to_item (item: &edn_format::Value) -> Item {
    match item {
        ...
        edn_format::Value::Vector(v) => { Item::List(v.iter().map(|i| to_item(i)).collect::<Vec<Item>>())},
...
pub fn parse(s: String) -> Vec<Item> {
...
pub fn emit_all(items: &Vec<Item>) -> String {
