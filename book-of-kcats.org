...
fn act_axiom(word: Word, e: &mut Environment) -> StepResult {
    match word.as_str() {
        ...
        "recur" => axiom::recur(e),
        "evert" => axiom::evert(e),
        "unpack" => axiom::unpack(e),
        "lexicon" => axiom::lexicon(e),
        _ => Err(Error {
            message: (format!("unimplemented axiom word: {}", word)),
...
fn main() {
    // TODO: probably not needed, we can read the lexicon as part of bootstrapping
    // TODO: there's a bug here where words like + aren't in the lexicon and they get overwritten
    let words = vec![
        "+".to_string(),
        "-".to_string(),
        "clone".to_string(),
...
fn append_expression(env: &mut Environment, p: VecDeque<Item>) -> StepResult {
    //env.expression.extend(p.into_iter());
    for i in p.into_iter() {
        env.expression.push_front(i);
    }
    Ok(env)
}
...
pub fn unpack(env: &mut Environment) -> StepResult {
    if let Some(mut s1) = env.stack.front_mut() {
        if let Item::List(l) = s1 {
            ...
            Err(Error::from("Invalid type for unpack - must be list"))
        }
    } else {
        stack_underflow()
    }
...
fn to_item(item: &edn_format::Value) -> Item {
    match item {
        ...
        edn_format::Value::Vector(v) => {
            Item::List(v.iter().map(|i| to_item(i)).collect::<VecDeque<Item>>())
        }
...
pub fn parse(s: String) -> List {
    ...
    parser
        .map(|r| to_item(&r.expect("expected valid element")))
        .collect()
}
