...
;;
;; +**foo : clojure fn foo, consume 2 args
;; .Bar. : Java no-arg construct Bar instance (consume zero, add one)
;; .*Baz. : construct with one arg (consume one, add one)
;; .!quux : call void instance method quux, (consume one, add none, mutating 2nd item)
;; .*Foo/bar : call static bar on class Foo. (consume one, add one)
(defn invoke*
  [{[word & others] ::expression
    ::keys [dictionary stack]
    :as env}]
  {::stack
   (let [[_ fn-type arity fn-name] (->> word
                                        str
                                        (re-find #"([\\+\\.])([\\!\\*]*)(.*)\.?"))
     ...
     (conj others
           (nil->nothing
            (cond
              ...
              (.startsWith arity "!") ;; void instance method (keep object)
              (let [obj (last args)]
  ...
  {'discard {::spec (stack-spec :a ::item)
   ...
   'pack {::spec (stack-spec :x ::item
          ...
          ::definition '[[[[swap bytes?]
                          ...
                          [[swap string?]
                           [.*java.lang.String/valueOf swap .**concat]]                            
  ...
  {'assign {::spec (stack-spec :value ::item
   ...
   'lookup {::spec (stack-spec :key ::item
            ...
            ::examples '[[[[[a b] [c d]] a lookup] [b]]
                         [[[[a b] [c d]] e lookup] [[]]]
                         [[[[outer [[a b] [c d]]]] [outer c] [lookup] step] [d]]]}
       ...
       {'hash {::core/spec (core/stack-spec :input ::core/bytes)
        ...
        'key {::core/spec (core/stack-spec :seed ::item)
              ::core/definition '[bytes
                                  ...
                                  "SHA1PRNG" .*java.security.SecureRandom/getInstance
                                  .!!(k '[1 [1 2 3] [+] map])setSeed

                                  256 "EC" .*java.security.KeyPairGenerator/getInstance
                                  .!!initialize 
