...
fn f_stack1(f: fn(Item) -> ItemResult, mut env: Environment) -> StepResult {
...
fn f_stack2(f: fn(Item, Item) -> ItemResult, mut env: Environment) -> StepResult {
...
fn act_axiom(word: Word, e: Environment) -> StepResult {
    match word.as_str() {
        "+" => f_stack2(axiom::plus, e),
        "-" => f_stack2(axiom::minus, e),
        "*" => f_stack2(axiom::mult, e),
        "/" => f_stack2(axiom::div, e),
        ">" => f_stack2(axiom::gt, e),
        "<" => f_stack2(axiom::lt, e),
        ">=" => f_stack2(axiom::gte, e),
        "<=" => f_stack2(axiom::lte, e),
        "clone" => axiom::clone(e),
        "swap" => axiom::swap(e),
        "swapdown" => axiom::swapdown(e),
        "inc" => f_stack1(axiom::inc, e),
        "dec" => f_stack1(axiom::dec, e),
        ...
        "loop" => axiom::env_effect2(e, axiom::loop_),
...
mod tests {
    ...
    fn test_words() {
        ...
        let words = vec![
            "+", "-", ">", "clone", "swap", "swapdown", "inc", "discard", "execute", "wrap",
            "unwrap", "dip", "dipdown", "branch", "step", "range", "recur", "evert", "unpack",
            "lexicon", "first", "recur", "snapshot", "loop",
        ];

        for word in words {
            assert_eq!(test_word(e.clone(), word.to_string()), Vec::<Error>::new())
        }
    }
...
pub fn dec(i: Item) -> ItemResult {
    if let Item::Int(i) = i {
        ...
        Err(Error::from("Invalid type for dec"))
    }
}
...
pub fn env_effect1(mut env: Environment, f: fn(Item) -> EffectResult) -> StepResult {
...
pub fn env_effect2(mut env: Environment, f: fn(Item, Item) -> EffectResult) -> StepResult {
    if let (Some(i), Some(j)) = (env.stack.pop_front(), env.stack.pop_front()) {
