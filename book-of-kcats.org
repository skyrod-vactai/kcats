...
pub trait ToHash<T: Eq + Hash> {
    fn to_hash(self) -> Result<HashMap<T, T>, NotDuple>;
}

impl<It: Iterator<Item = ItIt>, ItIt: Iterator<Item = T>, T: Eq + Hash> ToHash<T> for It {
    fn to_hash(self) -> Result<HashMap<T, T>, NotDuple> {
        self.map(|mut it_it| {
...
#+RESULTS:
: error: Could not compile `cargom2yLm4`.
...
pub fn lexicon(env: &mut Environment) -> StepResult {
    if let Ok(s) = fs::read_to_string("src/kcats/lexicon.kcats") {
        ...
        let h: Result<Vec<List>, String> = items
            .into_iter()
            .map(| i |   {
