...
There is only one type in the system that has state (and therefore
would lose meaning being sent to a remote machine): a pipe. See [[*Pipes][this
later section]] for more detail. In kcats, all state is isolated in
pipes. Nothing on the stack can be stateful, except pipes. Examples of
stateful things: files, network sockets, singletons, etc. 

A tricky part of kcats implementation is representing anything
stateful as a pipe without putting any intermediate stateful things on
the stack. For example, we can represent a network socket connection
as a pipe, but we have to construct it all at once from values (the
host and port). No java Socket objects can go on the stack. The tricky
part is dropping down out of kcats into the platform language (clojure
in this case) just long enough to construct the pipe.
#+BEGIN_SRC clojure
...
(def base-words
  {'invoke {::spec (stack-spec :program ::program)
            ...
            functions (listed in an aggregate) with part of stack as
            arguments. eg [..+ .inc] will take top 2 args (because of
            two dots) and add them, then increment the top item (one
            dot)"
