...
#+begin_src kcats
  ...
  handoff ;; a pipe to distribute work
  ["Darlene" "Thomas" "Ethel"] ;; the names of the librarians
  ;; here is their program. They'll find the stack with:
  ;; their name, <-library, ->library, a timestamp pipe.
  [[take] dip ;; n book pi po tsp
   ;"got book\n" print
   [last-checked-in-by] swap assign ;; book pi po tsp
   [take] divedeep ;; ts book p tsp
   [last-checkin-time] swap assign ;; book pi po tsp
   swap [put] dip] ;; the book back in the pipe to the library
  ;; Now we want to create an environment for each librarian
  swap [swap spawn animate] map 
  ;; now that this is done we don't need the names or program or
  ;; timestamp pipe anymore
  drop drop dropdeep
  ;; read in our books
  "examples/books.kcats" file-out slurp read
  ;; feed the db into the in pipe
  [[put] step] spawn animate
  drop drop ;; drop the db and in-pipe
  [take] collect ;; from the out pipe
#+end_src

#+RESULTS:
#+begin_src kcats
Taking from channel: Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
Taking from channel: Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(5)
Taking from channel: Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
[[reason "type mismatch"]
 [unwound [collect]]
 [type error]
...
[[type pipe]
 [handoff 0]]
Putting into channel: [[author-first "George"]
 [author-last "Orwell"]
 [title "1984"]
 [year 1949]
 [subjects [government dystopia surveillance totalitarianism freedom]]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
Sent: Ok(()) on ThreadId(4)
Putting into channel: [[author-first "Aldous"]
 [author-last "Huxley"]
 [title "Brave New World"]
 [year 1932]
 [subjects [society technology dystopia happiness drugs]]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
Received: [[author-first "George"]
 [author-last "Orwell"]
 [title "1984"]
 [year 1949]
 [subjects [government dystopia surveillance totalitarianism freedom]]] on ThreadId(5)
Putting into channel: [[subjects [government dystopia surveillance totalitarianism freedom]]
 [last-checkin-time 1692994374076]
 [year 1949]
 [title "1984"]
 [last-checked-in-by "Darlene"]
 [author-last "Orwell"]
 [author-first "George"]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 0 } on ThreadId(5)
Received: [[author-first "Aldous"]
 [author-last "Huxley"]
 [title "Brave New World"]
 [year 1932]
 [subjects [society technology dystopia happiness drugs]]] on ThreadId(5)
Putting into channel: [[last-checkin-time 1692994374076]
 [author-first "Aldous"]
 [title "Brave New World"]
 [year 1932]
 [author-last "Huxley"]
 [subjects [society technology dystopia happiness drugs]]
 [last-checked-in-by "Ethel"]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 0 } on ThreadId(5)
Sent: Ok(()) on ThreadId(4)
Putting into channel: [[author-first "F. Scott"]
 [author-last "Fitzgerald"]
 [title "The Great Gatsby"]
 [year 1925]
 [subjects [wealth love obsession american-dream tragedy]]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
Received: [[author-first "F. Scott"]
 [author-last "Fitzgerald"]
 [title "The Great Gatsby"]
 [year 1925]
 [subjects [wealth love obsession american-dream tragedy]]] on ThreadId(4)
Putting into channel: [[author-last "Fitzgerald"]
 [last-checkin-time 1692994374076]
 [author-first "F. Scott"]
 [title "The Great Gatsby"]
 [year 1925]
 [last-checked-in-by "Thomas"]
 [subjects [wealth love obsession american-dream tragedy]]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 0 } on ThreadId(4)
Sent: Ok(()) on ThreadId(4)
Putting into channel: [[author-first "J.D."]
 [author-last "Salinger"]
 [title "The Catcher in the Rye"]
 [year 1951]
 [subjects [adolescence alienation innocence society adulthood]]] into Handoff { receiver: Some(Receiver), sender: Some(Sender), id: 1 } on ThreadId(4)
#+end_src
...
It's important that we create the 
* Example programs
