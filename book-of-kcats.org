...
environment expressed as kcats data.
#+begin_src clojure
(in-ns 'kcats.core)
...
Here's an example showing how this functionality can be used. Let's
say we want to count the number of times each word is executed while
running a program.

#+begin_src kcats
[[] ;; empty list to put word counts in
 [lexicon ;; the inner environment, load the lexicon - probably should make this automatic
  10 [0 >] [clone dec] while]  ;; the sample program to run
 environment 
 [[expression] lookup nothing? not] ;; something still in the expression, keep running
