...
- list :: A list of items, which can be interpreted either as
  data or executable code.
- program :: an list intended to be executable.
...
Now we'll spec a list, which is just a list. We'll use
clojure's vectors (which are indexed lists). We'll also spec out
programs, which are lists but intended to be executed (otherwise
...
(defn inscribe
  "Define a new word. Takes a list off the stack which should be of the
  form:
  [new-word program]"
