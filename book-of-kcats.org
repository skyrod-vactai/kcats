...
information. So in this format, =clone= spec would be =[[a] [a a]]=. A
list of two items: what is taken from the stack, and what's left
afterward. Only the top stack items are relevant here, there could be
any number of items underneath, but they aren't touched. The order of
items in each list is that the first item is the top of the stack.
...
stack changes". The spec for =execute= is =[[program][*]]=. 
...
(defmacro spec-effect
  ...
  [[in out :as spec] other-props]
  (let [->body (fn [spec-item]
                 (if (vector? spec-item)
                   (first spec-item)
                   spec-item))]
    `(merge ~other-props
            {::spec (quote ~spec)
             ::fn (stack-effect ~(mapv ->body in)
                                ~(mapv ->body out))})))
#+END_SRC
...
(defn from-spec
  ...
  [{::keys [spec] :as dfn}]
  (let [[in out] spec]
    (assoc dfn ::fn (clojure.core/eval `(stack-effect ~in ~out)))))
...
(def axiom-words
  {'execute (effect [p] [p []])
   ...
   'inscribe (fn [{[word-def word & others] ::stack
                   ::keys [stack expression dictionary]}]
               {::dictionary (inscribe dictionary [word word-def])
                ::stack others
                ::expression expression})
   'branch (effect [f t b]
                   [(if (nothing->nil b) t f) []])
   'step (effect [p [agg-item & agg-rest :as agg]]
                 (if (seq agg)
                   [(cond-> ['execute]
   ...
   'unpack (stack-effect [[l & others]] [(nil->nothing l) (vec others)])
   'join (stack-effect [a b]
                       (if (and (string? a) (string? b))
                         [(str b a)]
                         [(vec (concat b a))]))
   'range (f-stack 2 (comp vec range))
   'evert (fn [{[l & others] ::stack ::keys [dictionary expression]}]
            {::stack (apply list (vec others) l)
             ::expression expression
             ::dictionary dictionary})
   'some? (f-stack 1 (comp some? nothing->nil))
   ...
   'not (f-stack 1 (fn [a]
                     (not (nothing->nil a))))
   'dictionary (fn [{::keys [dictionary] :as env}]
...
(defonce k-env (atom (eval (default-env))))

(defn prepare-env
  "Adds the lexicon and expression and returns the uneval'd env"
  [expr]
  (update @k-env
          ::expression
          concat expr))
...
(defn k
  ...
  ([opts p]
   ;; avoid having to constantly reset k-env manually during development
   (reset! k-env (eval (default-env)))
   (repr (::stack (eval opts (prepare-env p)))))
...
;; TODO: remove the word from the end of all the examples inputs, and
;; have the test runner add it at test-time
...
[swapdown [[spec [[a b c] [a c b]]]
           [examples [[[1 2 3 swapdown] [3 1 2]]]]]]

[float [[spec [[a b c] [c a b]]]
        [examples [[[1 2 3 float] [1 3 2]]]]]]

[sink [[spec [[a b c] [b c a]]]
...
[branch [[spec [[[false-branch program]
                 [true-branch program]
                 [condition item]]
                [*]]]
...
;; Words that have pure kcats definitions start here
...
[shielddowndown [[spec [[[p program] [consumed item] [consumed item]]
                        [[result item]]]]
...
[update [[spec [[program [keys list] association-list]
                [association-list]]]
...
[lookup [[spec [[keys list] association-list] [item]]
...
[unassign [[spec [[[key item] association-list] [association-list]]]
           [examples [[[[[a b] [c d]] a unassign]
                       [[[c d]]]]
                      
                      [[[[a b] [c d]] e unassign]
                       [[[a b] [c d]]]]]]]]
[decide [[spec [[[test-expr-pairs association-list] [*]]]]
...
[value [[spec [[association-list] [item]]]
        ...
        [examples [[[[[foo 1]] value] [1]]
                   ...
                   [[[[type url] [value "http://foo.com"]] value]
                    ["http://foo.com"]]]]]]
[zip [[spec [[[values list] [keys list]] [association-list]]]
...
(swap! core/core-words merge
       {'atom {::core/spec (core/stack-spec :value ::core/item)
        ...
        'filled {::core/spec (core/stack-spec :values ::core/list)
                 ::core/fn (core/stack-effect [l] [(->filled l)])}
        ...
        'put {::core/spec (core/stack-spec :value ::core/item
                                           :pipe ::in)
              ;; TODO: not appropriate to call `bytes` here.  Some
              ;; pipes (from one env to another) take any value, not
              ;; just bytes.
              ::core/definition '[++kcats.pipe/put!]}
        ...
        'pipe-in {::core/spec (core/stack-spec :obj ::item)
                  ...
                  '[[[[type [file] unwrap =]
                    ...
                    decide]}
        'tunnel {::core/spec (core/stack-spec :obj ::item)
                  ::core/definition
                  '[[[[type [ip-port] unwrap =]
                      [clone
                       [port] lookup
        ...
        'assemble {::core/spec (core/stack-spec :pipe ::core/pipe
                   ...
                   ::core/doc "Takes a large value out of a pipe by
        ...
        'disassemble {::core/spec (core/stack-spec :splitter ::core/program
                                                   :stuff ::core/item
                                                   
                                                   :pipe ::core/pipe)
                      ::core/doc "Puts a large value into a pipe by
                      ...
                      ::core/definition '[;; build a while body that includes splitter
                                          [swap [put] dip] join
                                          [nothing? not] swap
                                          while
                                          discard ;; drop the 'nothing' value
                                          ]}
        'pump {::core/spec (core/stack-spec :program ::core/program
                                            :input ::core/pipe
                                            :output ::core/pipe)
               ;; while neither pipe is closed, take from input, run
               ;; program, put into output
               ::core/definition '[wrap [shield] join
        ...
        'spit
        {::core/spec (core/stack-spec :contents ::core/item
                                            :target ::core/item)
...
(defconst kcats-font-lock-keywords
	...
	(,(regexp-opt '("swap" "swapdown" "discard" "sink" "float" "clone" "snapshot" "evert") 'symbols) . (0 font-lock-builtin-face))
	(,(regexp-opt '("true" "false" "[]") 'symbols) . (0 font-lock-keyword-face))
        (,(regexp-opt '("first" "second" "pack" "unpack" "step" "filter"
			"map" "count" "join" "rest" "wrap" "unwrap") 'symbols) . (0 font-lock-function-name-face))
	(,(regexp-opt '("execute" "dip" "dipdown" "shield" "shielddown" "shielddowndown" "inject"
			"loop" "while" "if" "branch" "recur") 'symbols) . '(0 font-lock-preprocessor-face))
