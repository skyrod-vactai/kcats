...
#+begin_src clojure
(defn kw->sym
  "Converts keywords to symbols, dropping any namespace. Non-keywords
  are unchanged"
  [k]
  (if (keyword? k)
    (-> k name symbol)
...
(defn kws->syms
  "Converts map keys that are keywords, to symbols"
  [m]
  (zipmap (->> m keys (map kw->sym))
          (vals m)))
