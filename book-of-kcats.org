...
(def axiom-words
  {'execute (effect [p] [p []])
   ...
   'reverse (f-stack 1 (comp vec reverse))
...
pub type StepResult<'a> = Result<&'a mut Environment, Error>;
...
// pub trait ToHash<K: Eq + Hash, V> {
//     fn to_hash(self) -> Result<HashMap<K, V>, NotDuple>;
// }

// impl<It: Iterator<Item = ItIt>, ItIt: Iterator<Item = K>, K: Eq + Hash, V> ToHash<K, V> for It {
//     fn to_hash(self) -> Result<HashMap<K, V>, NotDuple> {
//         self.map(|mut it_it| {
//             let a = it_it.next().ok_or(NotDuple::ZeroElements)?;
//             let b = it_it.next().ok_or(NotDuple::OneElement)?;
//             if !it_it.next().is_none() {
//                 return Err(NotDuple::TooMany);
//             }
//             Ok((a, b))
//         })
//         .collect()
...
pub fn to_key_item(i: Item) -> Result<KeyItem, String> {
    match i {
        ...
        _ => Err("item is not allowed as a hashmap key".to_string()),
    }
}

pub fn to_value_item(i: KeyItem) -> Result<Item, String> {
    match i {
        KeyItem::Int(i) => Ok(Item::Int(i)),
        KeyItem::String(i) => Ok(Item::String(i)),
        KeyItem::List(l) => Ok(Item::List(
            l.iter()
                .map(|i| to_value_item(i.clone()))
                .collect::<Result<List, String>>()?,
        )),
        KeyItem::Word(w) => Ok(Item::Word(w)),
        KeyItem::Boolean(b) => Ok(Item::Boolean(b)),
        KeyItem::Bytes(bs) => Ok(Item::Bytes(bs)),
        KeyItem::Nothing => Ok(Item::Nothing),
        
    }
}

pub fn to_entry(mut i: Item) -> Result<(KeyItem, Item), String> {
    match i {
        Item::List(mut l) => {
            let v = l.pop_back();
            let k = l.pop_back();
        ...
        _ => Err("Hash entry must be a list (of length 2)".to_string()),
    }
}

pub fn to_hash(mut l: List) -> Result<HashMap<KeyItem, Item>, String> {
    l.iter()
        .map(|i| to_entry(i.clone()))
        .collect::<Result<HashMap<KeyItem, Item>, String>>()
}
#+end_src
...
// fn err(env: &mut Environment, s: String) -> Result<&mut Environment, String> {
//    Err(EnvErr {Environment: env,
//            Error: s})
// }

fn f_stack1(f: fn(Item) -> Result<Item, Error>, env: &mut Environment) -> StepResult {
...
fn f_stack2(f: fn(Item, Item) -> Result<Item, Error>, env: &mut Environment) -> StepResult {
...
fn act_axiom(word: Word, e: &mut Environment) -> StepResult {
    match word.as_str() {
        "+" => f_stack2(axiom::plus, e),
        "-" => f_stack2(axiom::minus, e),
        "clone" => axiom::clone(e),
        "swap" => axiom::swap(e),
        "swapdown" => axiom::swapdown(e),
        "inc" => f_stack1(axiom::inc, e),
        "discard" => axiom::discard(e),
        "execute" => axiom::execute(e),
        "wrap" => axiom::wrap(e),
        "unwrap" => axiom::unwrap(e),
        "dip" => axiom::dip(e),
        "branch" => axiom::branch(e),
        "step" => axiom::step(e),
        "range" => axiom::range(e),
        "recur" => axiom::recur(e),
        "evert" => axiom::evert(e),
        "lexicon" => axiom::lexicon(e),
        _ => Err(format!("unimplemented axiom word: {}", word)),
    }
}

fn eval_step(env: &mut Environment) -> StepResult {
    let next_item = env.expression.pop_front();

    if let Some(val) = next_item {
        if let Item::Word(word) = val {
            if let Some(dfn) = env.dictionary.get(&word) {
                match dfn {
                    WordDef::Axiom(..) => act_axiom(word, env),
                    WordDef::Derived(d) => {
                        // has a definition, put it into expression (in reverse)
                        let items = d.definition.clone();
                        env.expression.extend(items.into_iter().rev());

                        Ok(env)
                    }
                }
            } else {
                println!("{:?}", env.dictionary.keys());
        ...
        Err("expression is empty".to_string())
    }
}

fn eval(env: &mut Environment) -> StepResult {
...
fn main() {
    ...
    let dict: Dictionary = HashMap::from_iter(words.iter().map(|w| {
        return (
            w.clone(),
    ...
    match io::stdin().read_line(&mut program) {
        Ok(_) => {
            let parsed = serialize::parse(program);
...
#+RESULTS:
: error: Could not compile `cargoxVWZQV`.
...
fn stack_underflow() -> StepResult<'static> {
    Err("Stack underflow".to_string())
}

pub fn plus(i: Item, j: Item) -> Result<Item, String> {
    match (i, j) {
...
pub fn minus(i: Item, j: Item) -> Result<Item, String> {
    match (i, j) {
        (Item::Int(i), Item::Int(j)) => Ok(Item::Int(i - j)),
        _ => Err("Invalid type for -".to_string()),
    }
...
pub fn inc(i: Item) -> Result<Item, String> {
    if let Item::Int(i) = i {
        Ok(Item::Int(i + 1))
    } else {
        Err("Invalid type for inc".to_string())
    }
}
...
fn swap2(env: &mut Environment, offset: usize) -> StepResult {
    ...
    if len >= 2 {
        env.stack.swap(len - offset - 1, len - offset - 2);
        Ok(env)
    } else {
        stack_underflow()
    }
...
pub fn discard(env: &mut Environment) -> StepResult {
...
fn append_expression(env: &mut Environment, p: VecDeque<Item>) -> StepResult {
    env.expression.extend(p.into_iter().rev());
    Ok(env)
}
...
pub fn execute(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop_front() {
        match item {
            ...
            _ => Err("Invalid type for execute".to_string()),
        }
    } else {
        stack_underflow()
    }
...
pub fn unwrap(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop_front() {
        if let Item::List(l) = item {
            env.stack.extend(l.into_iter().rev());
            Ok(env)
        } else {
            Err(format!("Invalid type for unwrap: {:?}", item))
        }
    } else {
        stack_underflow()
    }
...
pub fn dip(env: &mut Environment) -> StepResult {
    ...
    match (s1, s2) {
        (Some(item1), Some(item2)) => match item1 {
            Item::List(program) => {
                env.expression.push_front(Item::Word("unwrap".to_string()));
                let mut v = List::new();
                v.push_front(item2);
            ...
            _ => Err("Invalid type for dip".to_string()),
        },
        _ => stack_underflow(),
    }
}
...
pub fn branch(env: &mut Environment) -> StepResult {
    ...
    match (f, t, b) {
        (Some(f), Some(t), Some(b)) => match (f, t) {
            (Item::List(false_branch), Item::List(true_branch)) => {
                let p = if is_truthy(b) {
                    true_branch
                } else {
                    false_branch
                };
                append_expression(env, p)
            }
            _ => Err("Invalid type for branch".to_string()),
        },
        _ => stack_underflow(),
    }
}
...
pub fn step(env: &mut Environment) -> StepResult {
    ...
    match (p, l) {
        (Some(p), Some(l)) => match (p, l) {
            ...
            _ => Err("Invalid type for step".to_string()),
        },
        _ => stack_underflow(),
    }
}

pub fn range(env: &mut Environment) -> StepResult {
    ...
    match (from, to) {
        (Some(from), Some(to)) => match (from, to) {
            ...
            _ => Err("Invalid type for range".to_string()),
        },
        _ => stack_underflow(),
    }
}
...
pub fn recur(env: &mut Environment) -> StepResult {
    ...
    match (rec2, rec1, then, pred) {
        (Some(rec2), Some(rec1), Some(then), Some(pred)) => match (rec2, rec1, then, pred) {
            (Item::List(rec2), Item::List(rec1), Item::List(then), Item::List(pred)) => {
                ...
                let mut b = List::new();
                b.push_front(Item::List(pred.clone()));
                b.push_front(Item::List(then.clone()));
                b.push_front(Item::List(rec1.clone()));
                b.push_front(Item::List(rec2.clone()));
                b.push_front(word("recur"));
                let mut e = List::new();
                e.push_front(Item::List(rec1));
                e.push_front(Item::List(b));
                e.push_front(Item::List(rec2));
                env.stack.push_front(Item::List(e));
                env.stack.push_front(Item::List(then));
                env.stack.push_front(Item::List(pred));
                Ok(env)
            }
            _ => Err("Invalid type for recur".to_string()),
        },
        _ => stack_underflow(),
    }
}
...
pub fn evert(env: &mut Environment) -> StepResult {
    ...
    match l {
        Some(l) => match l {
            ...
            _ => Err("Invalid type for evert".to_string()),
        },
        _ => stack_underflow(),
    }
}
...
fn to_dict(def: HashMap<KeyItem, Item>) -> WordDef {
    let v = as_list(def.get(&key_item("definition")));
    let docs = WordDocs {
    ...
    if let Some(d) = v {
        ...
        WordDef::Axiom(AxiomDef { docs: docs })
    }
}

pub fn lexicon(env: &mut Environment) -> StepResult {
    if let Ok(s) = fs::read_to_string("src/kcats/lexicon.kcats") {
        let items = serialize::parse(s);

        let vitems = List::from(items);

        // let mut h = HashMap::new();
        // let entries = items
        //     .iter()
        //     .map(|item| to_entry(item.clone()))
        //     .collect::<Result<Vec<(KeyItem, Item)>, String>>()?;
        // for (k, v) in entries {
        //     h.insert(k, v);
        // }

        let d: Dictionary = HashMap::from_iter(to_hash(vitems)?.iter().map(|(k, v)| {
            let v = to_dict(to_hash(as_list(Some(v)).unwrap()).ok().unwrap());
            (as_word(k).unwrap_or("".to_string()), v)
        }));
        ...
        Err("Could not read from lexicon file".to_string())
    }

    // for [w, dfn] in items {

    //     match (w, dfn) {
    //         (Item::Word(w), Item::List(dfn)) => {
    //             let mut m: HashMap<Word, List>::new();
    //             for [k, v] in dfn {
    //                 match (k, v) {
    //                     (Item::Word(k), Item::List(v)) => {
    //                         m.insert(Item::Word(k), Item::List(v));
    //                     },
    //                     _ => { return Err("Word definition has invalid type".to_string())}
    //                 }
    //             }
    //             env.dictionary.insert(Item::Word(w),
    //                                   DerivedDef{definition: Item::List(m.get(word("definition"))),
    //                                              docs: WordDocs{examples: m.get(word("examples")),
    //                                                             spec: m.get(word("spec"))}}
    //             );
    //         }
    //     }
    // }
...
fn to_item (item: &edn_format::Value) -> Item {
    match item {
        edn_format::Value::Integer(i) => { Item::Int(*i) },
        edn_format::Value::Vector(v) => { Item::List(v.iter().map(|i| to_item(i)).collect::<VecDeque<Item>>())},
        edn_format::Value::Symbol(s) => { Item::Word(s.name().to_string()) },
...
fn from_item (item: &Item) -> edn_format::Value { 
    match item {
        ...
        Item::List(v) => { edn_format::Value::Vector(v.iter().map(|i| from_item(i)).collect::<Vec<edn_format::Value>>()) },
...
pub fn parse(s: String) -> List {
    let parser = edn_format::Parser::from_iter(s.chars(), edn_format::ParserOptions::default());
    parser.map(|r| { to_item(&r.expect("expected valid element"))}).collect()
}
...
#+RESULTS:
: error: Could not compile `cargo75nfdJ`.
