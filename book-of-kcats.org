...
fn standard_dictionary() -> Dictionary {
    HashMap::from([axiom_entry("lexicon", Rc::new(axiom::lexicon)),
                   axiom_entry("+", Rc::new(f_stack2(axiom::plus)))])
}
...
pub fn lexicon(mut env: Environment) -> StepResult {
    if let Ok(s) = fs::read_to_string("src/kcats/lexicon.kcats") {
        ...
        let d: Dictionary = HashMap::from_iter(to_hash(vitems)?.iter().map(|(k, v)| {
            ...
            (as_word(k).unwrap_or(Intern::new("".to_string())), v)
        }));
...
fn to_item(item: &edn_format::Value) -> Item {
    match item {
        ...
        edn_format::Value::Symbol(s) => Item::Word(Intern::new(s.name().to_string())),
