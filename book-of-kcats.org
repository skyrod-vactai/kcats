...
#+BEGIN_SRC clojure
  ...
  (defn f-stack-list
    "Apply nitems from stack to f, which returns a list to be concat'd
    onto the stack"
    [nitems f]
    (partial update-stack (partial s-apply nitems f)))
...
(defmacro stack-effect
  ...
  f-stack-list"
  [in out]
  `(f-stack-list ~(count in) (fn ~in ~out)))
  ...
  (merge
   ...
   {'discard {::spec ::stack-min-depth-1
    ...
    'branch {::spec (stack-spec :false-branch ::aggregate
                                ...
                                :condition ::item)
             ::fn (fn [{[f t b & others] ::stack ::keys [expression dictionary]}]
                    {::stack others
                     ::expression (concat (if (nothing->nil b) t f) expression)
                     ::dictionary dictionary})}
    ...
    'pack {::spec (stack-spec :x ::item
                              :aggregate ::aggregate)
           ::fn (stack-effect [a x] [(conj (vec a) x)])
           ::examples '[[[[] 1 pack] [[1]]]
...
(defn k
  ...
  ([p {:keys [max-steps] :as opts}]
   (let [e {::stack '()
            ::dictionary builtin-words
            ::expression (list* p)}
         steps (cond->> (iterate eval-step e)
                 max-steps (take max-steps))
         [unfinished finished] (split-with (comp seq ::expression) steps)]
     (::stack (if (seq finished)
                (first finished)
                (last unfinished)))))
  ([p] (k p {})))
