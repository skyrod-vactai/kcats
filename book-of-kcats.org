...
You can also use a =while= loop to run the program until an arbitrary
...
where =step= is about to be executed. =stepper= is a generator that takes
an environment and generates all the steps of execution. Note =toe=
means "top of expression", so it's keeping the states where =step= is
the next item in the expression.
#+begin_src kcats
  [[[expression [0 1 3 inc 1 range [+] step]]] environment
   stepper
   [toe wrap [step] =] keep
