...
information. So in this format, =clone= spec would be =[[[item a]] [a a]]=. A
...
(defn build-spec
  ...
  [spec]
  (clojure.core/eval `(spec/tuple ~@(for [s spec]
...
(defn cache-spec
  ...
  [{::keys [dictionary] :as env} word]
  (let [[in-spec out-spec] (get-in dictionary [word ::spec])
...
(defn onto-stack
  ...
  (if (word? item)
    (let [{::keys [dictionary] :as env} (cache-spec env item)
...
[clone [[spec [[[item a]] [a a]]]
        [examples [[[1 2 3 clone] [3 3 2 1]]]]]]

[swap [[spec [[[item a] [item b]] [b a]]]
       [examples [[[1 2 3 swap] [2 3 1]]]]]]

[swapdown [[spec [[[item a] [item b] [item c]] [a c b]]]
           [examples [[[1 2 3 swapdown] [3 1 2]]]]]]

[float [[spec [[[item a] [item b] [item c]] [c a b]]]
        [examples [[[1 2 3 float] [1 3 2]]]]]]

[sink [[spec [[[item a] [item b] [item c]] [b c a]]]
