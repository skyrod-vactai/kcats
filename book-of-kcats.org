...
type Stack = Vec<Item>;
type Error = String;
type Dictionary = HashMap<Word, WordDef>;
...
type StepResult<'a> = Result<&'a mut Environment, Error>;
...
fn inc(i: Item) -> Result<Item, String> {
    if let Item::Int(i) = i{
        Ok(Item::Int(i+1))
    } else {
...
fn clone(env: &mut Environment) -> StepResult {
...
fn discard(env: &mut Environment) -> StepResult {
...
fn main() {
    ...
    let dict: Dictionary = HashMap::from_iter(
        words.iter().map(|w| {
            return (w.clone(), WordDef::Axiom(AxiomDef
                               {docs: WordDocs {examples: None,
                                                spec: None}}))}));
