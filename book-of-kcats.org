...
(def standard-words
  {'if {::spec (stack-spec :false-branch ::program
                           :true-branch ::program
                           :condition ::program)
        ::definition '[[nullary] dipdown branch]
   ...
   'assert {::spec (stack-spec :p ::program)
            ::definition '[snapshot ;; save stack to print in err message
                           [nullary] dip swap ;; run the assertion under the saved stack
...
that will be run with =nullary= before the actual word and if it
...
(k 
 '[[1 23 9 9 9 9 9 9 9 9 9  9]          ; items exceptions? empty? continue?
   false ;; any exceptions found
   [[clone nothing?] dip swap [or] unary]
   [not]
   [[unpack [odd?] unary not] dip or]
   [execute]
   recur
...
the above doesn't work because when the program is executing we can't
access anything below the list - because the predicate is going the
find the remaining list below the list item it's currently working
on. That shouldn't be there from the predicate's point of view.

The only way to make this work is with snapshotting the stack and then
doing injections. Step also doesn't work here because we need to short
circuit. 
