...
(expound/def ::environment ::association-list) ;; TODO: specifically call ou:syms
...
(defn eval
  ([:syms [max-steps before-step histogram]} env]
   (let [hist (when histogram (atom {}))
         eval-step (if before-step
     ...
     (loop [{:syms [expression] :as env} env
       ...
       (if (or (not (seq expression)) ;; nothing left to run
         ...
         (do (when histogram (println (sort-by val @hist))) env)
         (do (when histogram
               (swap! hist update (first expression) (fnil inc 0)))
...
(defn k
  ...
  ([opts p]
   (->> p
        (update (default-env) 'expression concat)
        (eval opts)
        'stack
        repr))
...
[join [[spec [[item item] [item]]]
       [definition [[[[list?] [++concat +vec]]
                     [[[string?] both?] [++str]]
                     ;; could also just check if it's an array 
                     [[[bytes?] both?] [++concat +into-array]]]
                    decide]]
...
[dipdown [[spec [[program
          ...
          ;;[definition [wrap [dip] unwrap pack dip]]
...
(defn env->k
  ...
  [['stack (-> e 'stack vec)]
   ['dictionary (into {}
                      (for [[k {:syms [definition]}] ('dictionary e)
...
namespaced keywor:syms. But kcats would say it's a list of pairs with
words a:syms. This matters in nested envs because each "step" of
...
(defrecord Reader [rdr closed?]
  ...
  (poll [this] (take this))
  Close
  (close [:syms [rdr closed?]}]
    (.close rdr)
    (reset! closed? true))
  (closed? [:syms [closed?]}] @closed?))
...
[encode [[spec [[[item data] [item as]] [item]]]
         [definition [[[[swap type base64 =]
                        [bytes
...
[decode [[spec [[[item data] [item as]] [item]]]
         [definition [[[[swap type base64 =]
                        [bytes
                         [!java.util.Base64/getEncoder] dip
                         !**encodeToString
