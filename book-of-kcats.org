...
pub type InnerFloat = f64;

pub type Float = OrderedFloat<InnerFloat>;

pub type Associative = BTreeMap<Item, Item>;
...
pub fn to_entry(mut i: Item) -> Result<(Item, Item), Error> {
    match i {
        Item::List(mut l) => {
            let v = l.pop_back();
            let k = l.pop_back();
...
pub fn to_hash(mut l: List) -> Result<Associative, Error> {
    l.iter()
        .map(|i| to_entry(i.clone()))
        .collect::<Result<HashMap<Item, Item>, Error>>()
}
#+end_src
...
pub fn plus(i: Item, j: Item) -> ItemResult {
    match (i, j) {
        ...
        (Item::Int(i), Item::Float(j)) => Ok(Item::Float(OrderedFloat(i as InnerFloat) + j)),
        (Item::Float(i), Item::Int(j)) => Ok(Item::Float(i + OrderedFloat(j as InnerFloat))),
        _ => Err(invalid_type("+")),
    }
}
...
pub fn gt(i: Item, j: Item) -> ItemResult {
    match (i, j) {
        ...
        (Item::Int(i), Item::Float(j)) => Ok(Item::Boolean(OrderedFloat(i as InnerFloat) > j)),
        (Item::Float(i), Item::Int(j)) => Ok(Item::Boolean(i > OrderedFloat(j as InnerFloat))),
...
pub fn lte(i: Item, j: Item) -> ItemResult {
    match (i, j) {
        (Item::Int(i), Item::Int(j)) => Ok(Item::Boolean(i <= j)),
        (Item::Float(i), Item::Float(j)) => Ok(Item::Boolean(i <= j)),
        (Item::Int(i), Item::Float(j)) => Ok(Item::Boolean((OrderedFloat(i as InnerFloat)).le(&j))),
        (Item::Float(i), Item::Int(j)) => Ok(Item::Boolean(i <= OrderedFloat(j as InnerFloat))),
...
fn as_word(i: &Item) -> Option<Word> {
    match i {
        Item::Word(w) => Some(w.clone()),
        Item::Builtin(b) => Some(b.word.clone()),
        _ => None,
    }
}

fn to_lexicon_entry(w: Word, def: Associative) -> WordDef {
    //println!("{:?}", def);
    let docs = WordDocs {
        examples: as_list(def.get(&word("examples"))),
        spec: as_list(def.get(&word("spec"))),
    };
    if let Some(d) = as_list(def.get(&word("definition"))) {
...
fn to_item(item: &edn_format::Value, standard_dictionary: &Option<Dictionary>) -> Item {
    match item {
        ...
        edn_format::Value::Float(f) => Item::Float(OrderedFloat(f.into_inner())),
        _ => Item::Int(0),
    }
}
...
fn main() {
    let h: HashMap<String, i32> = HashMap::from([("foo".to_string(), 1)]);
    let v: Vec<_> = h.into_iter().map(|(k, v)| vec![Item::Str(k), Item::Num(v)]).collect();
    println!("{:?}", v);
}
#+end_src
