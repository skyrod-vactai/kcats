...
fn eval_step(env: &mut Environment) -> StepResult {
    let nextItem = env.expression.pop();

    if let Some(val) = nextItem {
...
fn print_result(r: StepResult) {
    match r {
        ...
        Ok(env) => {
            ...
            println!("{}", serialize::emit(&Item::List(List::from(env.stack.clone()))));
        }
    }
}
...
fn main() {
    ...
    match io::stdin().read_line(&mut program) {
        Ok(_) => {
            let mut parsed = serialize::parse(program);
            let mut env = Environment {
                stack: Vec::new(),
                expression: Vec::from_iter(parsed.into_iter().rev()),
                dictionary: dict,
            };
...
pub fn clone(env: &mut Environment) -> StepResult {
    if let Some(last) = env.stack.last().map(|v| (*v).clone()) {
        env.stack.push(last);
        Ok(env)
    } else {
        stack_underflow()
...
pub fn execute(env: &mut Environment) -> StepResult {
    if let Some(item) = env.stack.pop() {
        match item {
...
pub fn dip(env: &mut Environment) -> StepResult {
    let s1 = env.stack.pop();
    let s2 = env.stack.pop();
    match (s1, s2) {
        (Some(item1), Some(item2)) => match item1 {
            Item::List(program) => {
                env.expression.push(Item::Word("unwrap".to_string()));
                let mut v = List::new();
                v.push_front(item2);            

                env.expression.push(Item::List(v));
        ...
        (Some(p), Some(l)) => match (p, l) {
            (Item::List(p), Item::List(mut l)) => {
                if let Some(litem) = l.pop_front() {
                    if !l.is_empty() {
                        env.expression.push(word("step"));
                        env.expression.push(Item::List(p.clone()));
                        env.expression.push(Item::List(l));
                    }
                    env.expression.push(word("execute"));
                    env.stack.push(litem);
                    env.stack.push(Item::List(p));
                }
                Ok(env)
            }
...
pub fn range(env: &mut Environment) -> StepResult {
    let to = env.stack.pop();
    let from = env.stack.pop();
    match (from, to) {
...
pub fn recur(env: &mut Environment) -> StepResult {
    let rec2 = env.stack.pop();
    let rec1 = env.stack.pop();
    let then = env.stack.pop();
    let pred = env.stack.pop();
    match (rec2, rec1, then, pred) {
        (Some(rec2), Some(rec1), Some(then), Some(pred)) => match (rec2, rec1, then, pred) {
            (Item::List(rec2), Item::List(rec1), Item::List(then), Item::List(pred)) => {
                ...
                env.stack.push(Item::List(e));
                env.stack.push(Item::List(then));
                env.stack.push(Item::List(pred));
                Ok(env)
            }
...
pub fn evert(env: &mut Environment) -> StepResult {
    ...
    match l {
        Some(l) => match l {
            Item::List(l) => {
                //TODO: this clone is a performance problem
                let s = env.stack.clone();
                env.stack = Vec::<Item>::new();
                env.stack.push(Item::List(List::from(s)));
                env.stack.push(Item::List(l));
                Ok(env)
            },
...
//     let s = fs::read_to_string("src/kcats/lexicon.kcats")?;
//     let items = serialize::parse(s);
//     for (w, dfn) in items {
//         let item = from
//         env.dictionary.insert()
//     }
// }
#+end_src

#+RESULTS:
: error: Could not compile `cargoYFTjBG`.
...
pub fn parse(s: String) -> VecDeque<Item> {
