...
fn ontoStack (env: &mut Environment) -> Result<&mut Environment, String> {
    ...
    match nextItem {
        ...
        Some(val) => {
            match val {
                Item::Int(_) | Item::Float(_) | Item::Boolean(_) | Item::Bytes(_) | Item::String(_) |
                Item::List(_) => { env.Stack.push(val);
                                   Ok(env) },
                
                Item::Word(word) => {
                    let opt = env.Dictionary.get(&word);
                    match opt {
                        Some(dfn) => { let f = dfn.action;
                                       f(env) },
                        None => { Err("unknown word ".to_string()) }
                    }
                }
            }
        }
...
fn main() {
    ...
    let mut result: Result<&mut Environment, String> = Result::Ok(&mut env);
