...
that sense it adds impurities to functional programming by introducing
state, but quite often real programs need to deal with state. That
state could just be something introduced by, and consumed by, the
local environment, or it could be received from another machine
...
*** Sockets
A socket has an in and out pipe that go to, and come from
(respectively) a remote machine or process.
...
(swap! core/core-words merge
       {'atom {::core/spec (core/stack-spec :value ::core/item)
        ...
        'put {::core/spec (core/stack-spec :value ::core/item
                                           :pipe ::in)
              ::core/definition '[bytes [++kcats.pipe/put] shielddown]}
        ...
        'assemble {::core/spec (core/stack-spec :pipe ::core/pipe
                   ...
                   ::core/doc "Takes from the pipe and runs reducer,
                   until the pipe closes, discards the pipe"
