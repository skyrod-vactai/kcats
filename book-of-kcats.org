...
developers and if you want anything that's even a slight variation of
what's on the menu, well, forget it. You're at their mercy.
...
brackets, like =[1 2 3]=). You can see below the word =pack= adds an
item to a list.
#+begin_example
[1 2 3] [4] join
=> [1 2 3 4]
...
"Tell Alice 'bring your tennis racket'", you meant for him to pass the
message on, not act on it himself. Even though you're talking to Bob,
Alice is supposed to bring the racket, not Bob. Similarly, when you
put a program on the stack, it's a message to be passed on now, and
executed later.
...
items can be list, which we haven't specced yet. That's ok,
...
(defn eval
  ([{:keys [max-steps before-step]} env]
   (println "evaling"  (dissoc ::dictionary env))
