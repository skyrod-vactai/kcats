...
(def axiom-words
  {'discard {::spec (stack-spec :a ::item)
   ...
   'unwrap {::spec (stack-spec :l ::aggregate)
            ::fn (stack-effect [l] l)
            ::examples '[[[[1] unwrap] [1]]]}
   ...
   'unpack {::spec (stack-spec :l ::aggregate)
            ::fn (stack-effect [[l & others]] [l (vec others)])
            ::examples '[[[["a" "b" "c"] unpack] ["a" ["b" "c"]]]]}
   'join {::spec (stack-spec :l ::aggregate
                             :m ::aggregate)
   ...
   'evert {::spec (stack-spec :l ::aggregate)
            ...
            ::fn (fn [env]
                   #_(update-stack (fn [[p a & others :as stack]]
                                     (->> a
                                          ...
                                          (conj others)))
                                   env))}
   'intersection {::spec (stack-spec :l ::aggregate
                                     :m ::aggregate)
                  ::fn (f-stack 2 (fn [x y]
                                    (into []
...
Next we can add some more words that are defined in terms of the axiom words, or calling clojure functions.
#+begin_src clojure
(def standard-words
  {'if {::spec (stack-spec :false-branch ::program
   ...
   '= {::spec (stack-spec :x ::item, :y ::item)
       ...
       ::examples '[[[1 1 =] [true]]
                    ...
                    [[[] false =] [false]]
                    [[1 "hi" "hi" =] [true 1]]]}
   'count {::spec (stack-spec :l ::aggregate)
           ::definition '[.count]
   ...
   'first {::spec (stack-spec :l ::aggregate)
           ::definition '[.first]
           ::examples '[[[[4 5 6] first] [4]]]}
   'second {::spec (stack-spec :l ::aggregate)
            ::definition '[.second]
