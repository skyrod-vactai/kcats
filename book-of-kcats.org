...
fn main() {
    ...
    let mut env = Environment {Stack : Vec::new(),
                               Expression : stack};
    let mut result = Result::Ok(&mut env);

    result = ontoStack(&mut result.ok());
    // while let Some(ref e) = result.ok() {
    //     if ! e.Expression.is_empty() {
    //         result = ontoStack(&mut e);
    //     }
    //     else { break }
    // }
