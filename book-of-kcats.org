...
Even though the following sections will explain a lot of these words
and how they work, the above program does the following:
...
+ =[] sort=: sort takes a program to transform each item in the list
  ...
  so we don't need to transform it at all, hence the empty program.
