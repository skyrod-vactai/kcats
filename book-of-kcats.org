...
#+end_src
** Stack
Let's try a hello world to make sure the toolchain is working
#+begin_src rust :tangle src/helloworld.rs
fn main() {
    println!("Hello world!");   
}
#+end_src
...
trait WordDef {
    fn act(self, e: &mut Environment) -> Result<&mut Environment, String>;
}

impl WordDef for AxiomDef {
    fn act(self, e: &mut Environment)  -> Result<&mut Environment, String> {
        match self.word {
            "+" => { f_stack2(&plus, e)},
            _ => { Err("unknown axiom word")}    
        }
    }
}
...
struct Environment {
    Stack: Stack,
    Expression: Stack,
    Dictionary: HashMap<Word, DictionaryEntry>
}
...
fn plus(i: Item, j: Item) -> Result<Item, String> {
    match (i, j) {
...
fn f_stack1(f: fn(Item) -> Result<Item, String>, &mut Environment) -> Result<&mut Environment, String> {
    if let Some(x) = env.Stack.pop() {
        env.Stack.push(f(x)?);
        Ok(env)
    }
    else { Err("Stack underflow".to_string())}
}

fn f_stack2(f: fn(Item)-> Result<Item, String>, &mut Environment) -> Result<&mut Environment, String> {
    let i = env.Stack.pop();
    let j = env.Stack.pop();
    match (i, j) {
        (Some(i), Some(j)) => {
...
fn eval_step (env: &mut Environment) -> Result<&mut Environment, String> {
    ...
    if let Some(val) = nextItem {
        if let Item::Word(word) = val {
            ...
            if let Some(dfn) = env.Dictionary.get(&word){
                ...
                if let Some(f) = &dfn.action {
                    
                    f(env)
                }
                else {
                    if let Some(d) = &dfn.definition {
                        // has a definition, put it into expression (in reverse)
                        let mut items = d.clone();
                        items.reverse();
                        ...
                        Err("Word has no definition or action".to_string())
                    }
                }
            }
            else {
...
fn eval(env: &mut Environment) -> Result<&mut Environment, String> {
    let mut result: Result<&mut Environment, String> = Result::Ok(env);
    loop {
...
fn print_result(r: Result<&mut Environment, String>) {
    match r {
        Err(e) => { println!("{}", e) },
        Ok(env) => { println!("{:#?}", env.Stack); }
    }
}

fn word(s: String) -> Item {
    Item::Word(Word::from(s))
}

fn main() {
    let mut stack = vec![
        Item::String("foo".to_string()),
        Item::Int(3),
        Item::Int(4),
        Item::Int(9),
        word("++".to_string())
    ...
    let mut env = Environment {Stack : Vec::new(),
                               Expression : stack,
                               Dictionary: HashMap::from(
                                   [("+".to_string(),
                                     DictionaryEntry {
                                         action: Some(Box::new(f_stack2(plus))),
                                         examples: None,
                                         spec: None,
                                         definition: None
                                     }),
                                    ("++".to_string(),
                                     DictionaryEntry {
                                         ...
                                         examples: None,
                                         spec: None,
                                         definition: Some(vec![word("+".to_string()),
                                                               word("+".to_string())])
                                     })
                                   ])};
...
Now let's see if we can do some basic stack operations, hardcoded
...
//fn ageUp(mut p: Person) -> Person {
//    p.age = p.age + 1;
//    return p;
//}

fn ageUp(p: &mut Person) {
    p.age = p.age + 1;
}

fn main() {
    let mut p = Person{age: 1,
                       names: vec!["Abe".to_string(), "Lincoln".to_string()]};
    let mut r: Result<&mut Person, String> = Ok(&mut p);
    match r {
        Ok(ref mut x) => { ageUp(x); },
        Err(_) => { }
    }
    println!("{:#?}", r.unwrap());
    //p = ageUp(p);
...
impl Word for Add {
    fn act(self, e: &mut Env) -> &Env {
        let x = e.stack.pop().unwrap();
        let y = e.stack.pop().unwrap();
        e.stack.push(x + y);
        e
    }
}

fn eval (e: &mut Env) -> &Env {
    e.word.act(e)
}
fn main() {
    println!("{}", eval(&mut Env{stack: vec![5,3],
                                 word: Add{}}));
