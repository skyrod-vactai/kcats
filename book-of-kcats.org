...
#+BEGIN_SRC clojure :tangle src/kcats/core.clj
  ...
  (expound/def ::program-definition (spec/keys :req [::fn ::definition]))

  (expound/def ::dictionary (spec/map-of ::word (spec/or ::axiom-definition
                                                      ::program-definition)))
#+END_SRC
...
#+RESULTS:
: #'kcats.core/update-stack#'kcats.core/s-apply#'kcats.core/s-apply-one#'kcats.core/f-stack#'kcats.core/inscribe#'kcats.core/nothing#'kcats.core/nothing?#'kcats.core/nothing->nil#'kcats.core/nil->nothing
...
#+BEGIN_SRC clojure :tangle src/kcats/core.clj
  ...
  (defn roll*
    [depth distance stack]
    (let [[top rest] (split-at depth stack)
  ...
  (defn discard*
    [depth stack]
    (let [[top [_ & rest]] (split-at (dec depth) stack)]
      (concat top rest)))
    ...
    (merge
     ...
     {'discard {::spec ::stack-min-depth-1
      ...
      'swap {::spec ::stack-min-depth-2
             ::fn (fn [{[a b & others :as stack] ::stack :as env}]
                    (assoc env ::stack (-> others (conj a) (conj b))))
      ...
      'branch {::spec (stack-spec :false-branch ::aggregate
               ...
               ::fn (fn [{[f t b & others] ::stack expression ::expression :as env}]
                      (-> env
                          (assoc ::stack others)
                          (update ::expression concat (if (nothing->nil b) t f))))}
      ...
      'some {::spec (stack-spec :program ::aggregate
                                :aggregate ::aggregate)
             ;; implementing this requires infrastructure to restore
             ;; stacks a la 'map', since the predicate gets run more than once.
             }
...
#+RESULTS:
: :kcats.core/stack-min-depth-1:kcats.core/stack-min-depth-2:kcats.core/binary-arithmetic:kcats.core/predicate:kcats.core/quoted-word#'kcats.core/arithmetic-words#'kcats.core/predicates#'kcats.core/cardinality#'kcats.core/roll*#'kcats.core/clone*#'kcats.core/discard*#'kcats.core/dip*#'kcats.core/recur*#'kcats.core/stack-spec#'kcats.core/builtin-words#'kcats.core/knil#'kcats.core/test
...
#+begin_src clojure tangle src/kcats/core.clj
  (defn ->env
    [e]
    (let [ec (into {} e)]
      {::stack (-> ec (get 'stack) list*)
       ::dictionary (-> ec (get 'dictionary) (merge builtin-words))
       ::expression (list* (get ec 'expression))}))
#+end_src
