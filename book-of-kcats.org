...
fn act_axiom(word: Word, e: &mut Environment) -> StepResult {
    match word.as_str() {
        ...
        "first" => axiom::env_effect1(e, axiom::first),
        _ => Err(Error {
...
fn append_expression(env: &mut Environment, mut items: VecDeque<Item>) -> StepResult {
...
pub fn env_effect1(env: &mut Environment, f: fn(Item) -> EffectResult) -> StepResult {
    if let Some(i) = env.stack.pop_front() {
        let (expr_items, stack_items) = f(i)?;
        for i in expr_items.into_iter() {
            env.expression.push_front(Item::List(i));
        }
        for i in stack_items.into_iter() {
            env.stack.push_front(Item::List(i));
        }
        Ok(env)
    } else {
        stack_underflow()
...
pub fn first(i: Item) -> EffectResult {
    if let Item::List(mut l) = i {
        Ok((
            None,
            Some(VecDeque::from(vec![if let Some(i) = l.pop_front() {
                i
            } else {
                Item::Nothing
            }])),
        ...
        Err(Error::from("Invalid type for first"))
    }
}

pub fn loop_(p: Item, f: Item) -> EffectResult {
    if let Item::List(p) = p {
        Ok({
            if is_truthy(f) {
                (
                    Some(List::from([Item::List(p.clone()), word("loop")])),
                    None,
                )
            } else {
                (None, None)
        ...
        Err(Error::from("Invalid type for loop"))
    }
}
...
pub fn dipdown(env: &mut Environment) -> StepResult {
    let s1 = env.stack.pop_front();
    let s2 = env.stack.pop_front();
    ...
    match (s1, s2, s3) {
        (Some(item1), Some(item2), Some(item3)) => match item1 {
            ...
            _ => Err(Error::from("Invalid type for dipdown")),
        },
        _ => stack_underflow(),
    }
}
