...
struct DictionaryEntry {
    action: Option<fn(&mut Environment) -> Result<&mut Environment, String>>,
    examples: Option<List>,
    spec: Option<List>,
    definition: Option<List>
}
...
fn plus (env: &mut Environment) -> Result<&mut Environment, String> {
    let i = env.Stack.pop();
    let j = env.Stack.pop();
    match (i, j) {
        (Some(i), Some(j)) => {
            match (i, j) {
                (Item::Int(i), Item::Int(j)) => { env.Stack.push(Item::Int(i+j));
                                                  Ok(env) },
