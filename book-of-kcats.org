...
Comments are prefixed with a semicolon and extend to the end of the
line.
...
processed with the language itself. See [[Get started using introspection]]
** Powerful
Express yourself succinctly and without repetition.
** Get straight to the point
There is no ceremony. The program can easily be a single line
consisting of a few words:

#+begin_src kcats
  "contacts.txt" slurp read [[age] lookup 25 >] filter  
#+end_src

That reads a database file of contacts, and prints only those older
than 25. The program is 8 words. 

You don't need a "main" method, or class declaration.  You only need
to specify what the program is actually supposed to *do*.
...
- program :: a list of instructions intended to be carried out by a machine.
...
=1= is beneath it). Then we put the word =+= onto the stack and it will
consume the =2= and the =1= and replace them with the sum, which is =3=.
...
most interested in the top items. That can be a source of confusion
for beginners.
...
#+begin_src kcats :results code  :exports both
  [[file "bar"]] association pipe-in ;; create the pipe to the given file "foo"
...
We can also use the word =file-out= as a shortcut to get a pipe.
#+begin_src kcats :results code :exports both
...
need. It would be wasteful to over-produce, so generators allow the
consumer to tell the producer when to produce.
...
transformations and you want to re-use it, but someone's already
produced all the source data! Our transformation stack needs a
*generator*, not a list! How are we supposed to use it? Never fear,
there is a simple way to adapt transformations to work on a plain
list. You can use the word =liberate= to convert a list to a
generator. (It's just an alias for =[take]= which is even shorter than
=liberate= so feel free to just use =[take]=).

Do you see why =[take]= converts a list to a generator? Remember,
generators are a state and a program. If we already have a list, we
can just treat that as the state. And =[take]= as the program does
exactly what we want, removes an item from the list and returns it,
leaving the state with one fewer item.
* Implementations
+ [[file:production.org::*Production implementation][Production implementation (start here)]]
...
** Get started using introspection
*** Overview
