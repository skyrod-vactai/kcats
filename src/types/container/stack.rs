...
impl StackData {
    ...
    pub fn shuffle(&mut self, pops: u8, pushes: &[u8]) -> Result<(), crate::types::Error> {
        ...
        let mut final_push = Vec::with_capacity(pushes.len());
        for &idx in pushes {
            final_push.push(items[idx as usize].clone());
        }
        // Push elements from first to last, so the last element pushed becomes TOS
        for item in final_push {
...
impl TryDerive<Item> for Stack {
    fn try_derive(i: Item) -> Result<Stack, Error> {
...
impl StackData {
    ...
    pub fn swap(&mut self, a: usize, b: usize) {
        ...
        let cached = self.tos.is_some() as usize + self.nos.is_some() as usize + self.nnd.is_some() as usize;
